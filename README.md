...
- **Verified checkpoint:** exact `0.171.0-antenna-status-leds` at source `c2413c9e31b89efd646a0ca15d2eb2b574d90fe5` is physically accepted in `E-BUILD-148`/`E-AUTO-119`/`E-HIL-178`/`E-UX-053`, then closes the 15/15 periodic current-capability checkpoint in `E-AUTO-120`/`E-HIL-179`/`E-CADENCE-001`. The four GPIO1 pixels again follow the 0.x hardware model: CC1101, nRF24 slots 1/2/3; raw brightness defaults to 2/255 and is adjustable through 0/2/3/5/8/12; green means active RX, red means receiver fault and inactive/Safe Mode is off. The full no-reflash matrix covers all eight Home sections, 20 automatic TFT states, 224 real one-pixel rows on both nRF24 and CC1101 waterfalls, seven Targets with List/Compare/Detail, and read-only USB pagination of two Sessions, all 16 catalog Targets and seven comparisons. Exact CID/generation/observations remain `FE34…9CB7`/161/59, heap returns to 91,068 B, TX/writes/input drops/discovery/Cardputer opens remain zero, and every lane finishes Home/none/lease 0. This resets physical cadence to 0/15; it is not release promotion or the deferred S5 RF-positive gate.
- **Next gate:** expose confirmed Target mutations through the same scoped companion contract with the same preview/confirmation and optimistic revision rules as the on-device UI. No transport-only mutation, direct storage/driver access or ambient grant is allowed. The physical S5 gate remains postponed, not waived, until the replacement DIV arrives and passes its read-only profile.
