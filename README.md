...
- **Verified checkpoint:** `E-BUILD-178`/`E-AUTO-152`/`E-HIL-193`/`E-UX-057`/`RB-M189` physically accept exact `1.0.0-dev.250` on original board-01 at source `bfe646e4d9408b4cd0ec1dc58c7c4e9c38a4ac0d`. The shared Bluetooth signal-card renderer now has a delta path, while list/detail screens own bounded invalidation: one live list interval repaints exactly 2 of 4 visible rows with zero whole-content clears; the selected device card changes 929 radar pixels and zero static/chrome pixels with one delta repaint and no new full repaint or content clear. Passive BLE, zero driver drops, exact CID, automatic screenshots and final Home/none/lease 0 all pass. This candidate also includes the accepted dev.249 CAP-049 persistence/export implementation, but this focused repaint delta does not claim its physical Save/reopen/export acceptance.
- **Next gate:** reuse the exact `1.0.0-dev.250` image already on the original DIV and prove explicit Save, atomic SD commit, exact-generation cold reopen and PCAP export; `.hc22000` readiness must additionally be demonstrated by useful bounded PMKID or replay-consistent M1→M2 evidence, while an ambient no-evidence run must remain honest and non-exportable. Product Survey Stop/commit/cold-recovery remains separately pending after its earlier `running_degraded` attempt failed closed before commit. Physical HTTP parity remains deferred to a dedicated client that cannot touch active Mac Wi-Fi.
