...
- **Verified checkpoint:** exact `0.129.0-pre-app-watchdog` completes the physical two-board NEC receive → save → cold Library CSV path in 33/33 steps. Exact `0.130.0`/`0.131.0` changes board-02 shared MISO/GPIO13 from LOW with its RF carrier attached to HIGH with the carrier absent. Exact `0.132.0-carrier-csn-characterization` then reattaches the carrier and observes all four receiver selects HIGH for 32/32 samples (nRF GPIO4/48/21 and CC1101 GPIO5) while MISO returns LOW for 0/32 HIGH samples under both pulls. It performs zero SPI bytes, receiver reads, CE-high events, strobes or TX commands and returns Home/none/lease 0. This rejects an accidentally selected receiver and localizes the fault to a carrier module or the carrier's shared-MISO net; it does not identify one module or prove an antenna/U.FL fault. Exact `0.136.0-capture-store-deadline` extends accepted S5.5 safety through the public Wi-Fi Capture Store worker on no-PSRAM board-01: a normal save stores 2 frames/433 payload bytes and advances generation 98→99 with 93,544 B free heap, a 32,756 B largest block and mount error zero. A second save stalls 10 s before storage hardware; its 8,000 ms worker deadline trips at 8,001 ms with zero physical writes, quiesces outputs/releases lease, retains Safe Mode across restart, blocks recovery writes, requires the two-action clear and returns exact CID/catalog 99/0, Home and lease 0.
- **Next gate:** restore the physical IR station path, then rerun the one-command exact `0.137.0-pulse-store-deadline` flow with fixture `0.2.5-shared-pin-safe`: two fixed NEC emissions, normal IR save, injected 10 s pre-storage stall, 8 s trip, retained restart, recovery block, two-action clear and final zero lease. Three fail-closed A/B runs now see zero IR transitions: current product/current fixture, current product/retained known-good fixture, and the exact product+fixture pair that previously passed 0.129. All fixtures complete one bounded 68 ms emission and both boards clean up safely. This excludes a 0.137 product regression and fixture-image regression as the current cause and blocks automatic reruns on the physical station path: line of sight, emitter/receiver, lower-board interconnect or power. The valid 0.2.5 shared-GPIO ownership correction remains, but is not the root cause. The Sub-GHz worker is source/native-covered only; its positive physical gate and paused S5.3 nRF gate still require a repaired or replacement RF carrier/device with plausible read-only identities.
...
| S5.5 | Runtime completeness: exact 0.136 accepts Product Survey preparation/admission, calibrated Wi-Fi+BLE workers and Wi-Fi Capture Store; 0.137 IR/Sub-GHz Store source/native/automation is ready; three-way exact A/B excludes current firmware as the zero-transition cause and blocks the IR gate on restoration of the physical station path; low-voltage safe-write, sleep/resume and applicable explicit GPS/PN532 assembly profiles remain | 🟡 in progress |
