...
- **Verified checkpoint:** `E-BUILD-185`/`E-AUTO-160`/`E-HIL-199`/`RB-M196` accept the exact `1.0.0-dev.262` Field Survey visit lifecycle on original board-01 at source `99aacd01336a065e18b52035ec243e2eb47abd92`. Preflight and both committed visits each auto-pause after exactly one Wi-Fi+BLE pass with zero drops; generations advance 170→171→172, the revisit compares 52 current identities with the 46-identity baseline as 42 seen again, 10 new and 4 missing, and a deterministic incomplete input remains visibly uncommittable. A separate no-flash/no-scan cold delta recovers exact generation 172/52 read-only with zero physical or blocked writes and final Home/none/lease 0. The retained dev.260 81→64/17-drop precursor is now correctly classified as cumulative pressure from 11 repeated Wi-Fi cycles while BLE was unavailable, not one dense scan.
- **Next gate:** route the already accepted bounded Field Survey result into native Library/export and truthful local WiGLE 1.6 output, then add live passive Wi-Fi station observations. Optional trusted GPS/UTC remains a separate final CAP-050 slice. Physical HTTP parity remains deferred to a dedicated client that cannot touch active Mac Wi-Fi.
...
| **Passive audit** | **Field Survey (CF-003):** Wi-Fi AP/station and BLE observations with deduplication, revisit comparison, an optional GPS track, and local WiGLE-compatible export | 🟡 active, S7: setup/result UI, bounded catalog and exact Wi-Fi→BLE preflight exist; first/revisit persistence, export routing, stations and GPS remain open |
