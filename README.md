...
- **Verified checkpoint:** `E-BUILD-195`/`E-AUTO-170`/`E-HIL-207`/`E-UX-064`/`RB-M206` accept the exact physical `1.0.0-dev.278` CAP-052 Device Lock UI/KDF slice. On original board-01, the non-persistent status/PIN editor runs two exact 120,000-round PBKDF2-HMAC-SHA-256 known-vector checks; the repeated 7.467 s KDF has byte-invariant 67,488 B warm heap, acknowledges UI input in 19.929 ms and repaints only the changed PIN cell. Credential generation remains zero, storage stays generation 176 with zero physical writes, radio is untouched, input errors/drops are zero and cleanup returns Home/none/lease 0. This accepts the responsive UI and production KDF/watchdog boundary only, not enrollment, cold retry/recovery, protected-action admission or encrypted data at rest.
- **Next gate:** enroll a physical PIN, cold-restore the exact credential, prove reset-resistant retry delay and recovery-only transition on original board-01, then route safe Stop/panic/update/factory reset and protected actions through the access matrix without exposing protected content. CAP-050 located/timed export still needs a separately owned non-conflicting GPS profile; physical HTTP parity still needs a dedicated client that cannot touch active Mac Wi-Fi or the Cardputer; the deferred S5 RF carrier gate remains blocked on replacement hardware.
...
| **Active-confirmed** | **Device Lock (CF-006):** local PIN, bounded retry/recovery, and protection for secrets/evidence without blocking Stop, panic, or recovery | 🟡 exact dev.278 physically accepts responsive status/PIN UI and production KDF/watchdog; enrollment, cold retry/recovery, action admission and encryption remain, S7 |
