...
- **Verified checkpoint:** `E-AUTO-220`/`E-HIL-238`/`E-EVIDENCE-002`/`RB-M254` accept the complete supported `WF-11` chain on reused exact physical `1.0.0-dev.369`: a novice-facing password-check explanation leads through bounded owned Wi-Fi capture, atomic save, cold Library reopen and canonical `WPA*02` export; `check_my_wifi_password.py` then performs the local common/vendor-default corpus check with explicit authorization, finite budgets, checkpoint/resume and a privacy-safe report. The anonymized physical fixture correctly remains a negative control, while the official public Hashcat PMKID vector is the independent positive control. Neither report retains plaintext, raw evidence or network identity; computer-side device/network/radio side effects are zero, and cleanup reaches Home/none/lease 0. Exact dev.357 remains the periodic full anchor.
