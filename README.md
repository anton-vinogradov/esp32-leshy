...
- **Verified checkpoint:** `E-BUILD-184`/`E-AUTO-159`/`E-HIL-198`/`RB-M195` retain the exact `1.0.0-dev.261` receive-only Field Survey preflight on original board-01 at source `b38464f93cdb7807734a24b1e1a08f03d4bbae24`. Moving the 4,560-byte CAP-049 HIL-only raw capture out of resident product RAM lowers static use to 231,624 B and restores the real Wi-Fi→BLE lifecycle: 12 Wi-Fi plus 33 BLE records are read/accepted/forwarded with zero drops or writes, exact generation 170 is unchanged, and cleanup reaches Home/none/lease 0 with HIL inactive. The retained dev.260 precursor records the former NimBLE `host_sync` timeout and a separate dense-input 81→64/17-drop boundary. This is a regression checkpoint, not the CAP-050 visit gate.
- **Next gate:** make dense source input above 64 records bounded and complete without stealing the BLE startup budget, then run one exact first-visit/revisit/commit/cold-recovery delta with a deterministic incomplete-input negative. Native/WiGLE artifact routing, live station capture and optional trusted GPS/time remain separate CAP-050 slices. Physical HTTP parity remains deferred to a dedicated client that cannot touch active Mac Wi-Fi.
