...
- **Verified checkpoint:** `E-BUILD-197`/`E-AUTO-172`/`E-HIL-209`/`E-UX-065`/`E-STORAGE-036`/`RB-M208` accept the exact physical `1.0.0-dev.281` CAP-052 Device Lock recovery/admission slice. On original board-01, one isolated run executes all five wrong PIN attempts with measured post-KDF retry bounds 4,931/14,931/59,930/299,930 ms, cold-restores generation-6 `recovery_only`, denies all seven protected operation classes and admits all seven safe classes in every relevant state. A real Library launch stays on opaque Home and a real export returns no protected content. Reset preview is non-destructive; confirmed reset proves protected erase occurs while the credential still exists, then clears it. Explicit fixture cleanup and a final cold boot return the untouched product namespace to generation 0/unconfigured, Home/none/lease 0 with zero input errors/drops.
- **Next gate:** add authenticated encryption at rest for protected data; this is the only remaining CAP-052 implementation gate. CAP-050 located/timed export still needs a separately owned non-conflicting GPS profile; physical HTTP parity still needs a dedicated client that cannot touch active Mac Wi-Fi or the Cardputer; the deferred S5 RF carrier gate remains blocked on replacement hardware.
...
| **Active-confirmed** | **Device Lock (CF-006):** local PIN, bounded retry/recovery, and protection for secrets/evidence without blocking Stop, panic, or recovery | 🟡 exact dev.281 physically accepts cold recovery-only, all protected/safe action classes and erase-before-credential destructive recovery; authenticated encryption at rest alone remains, S7 |
