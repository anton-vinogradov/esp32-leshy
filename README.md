...
- **Verified checkpoint:** host/build checkpoint `E-CORR-002` extends the explainable S6.2 correlation slice with deterministic schema-v2 persistence. The Target catalog and its complete bounded accept/reject decision history are encoded as one CRC-bound CBOR state and published through one isolated dual-head journal, so recovery cannot pair a new graph with an old history or the reverse. Native fault injection interrupts every one of the six durable commit boundaries; recovery yields either the complete previous pair or the complete new pair, while corruption of the newest generation falls back to the previous valid pair. Deterministic codec/reopen, exact proposal explanations, decisions and revisions survive the round trip; native, ASan/UBSan and production builds pass. Runtime wiring/migration and reversible merge/split, compare and UI remain explicitly unclaimed. Exact `0.145.0-interface-settings` remains the latest physically accepted board-01 product baseline; the deferred S5 RF-positive gate remains open.
- **Next gate:** implement bounded reversible Target merge/split history and prove merge→split restores the byte-equivalent prior identity/evidence graph without editing source Observations, then include that history in the same atomic state generation. The physical S5 gate is postponed, not waived: when the ordered replacement DIV arrives, its read-only profile must pass before the retained one-command IR→nRF24→OOK→FSK matrix can close S5.3/S5.4/S5.6.
