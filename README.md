...
- **Verified checkpoint:** host/build checkpoint `E-CORR-004` completes S6.3 with bounded schema-v1 read-only Action `target.compare`. It compares two distinct, available stopped Sessions through exact source ID/generation/sequence/time references, selects the latest retained Observation per Target identity and deterministically reports added, removed, changed and unchanged Targets. Changed results expose an explicit identity/radio/frequency/channel/signal/label/Wi-Fi/BLE mask; RSSI noise below 6 dB is intentionally stable. Every result retains the exact baseline/current evidence it used. Missing sources, unavailable records, malformed observations or identity/source mismatches clear the whole result rather than returning a partial diff. Golden real-Session, negative, ASan/UBSan and production-build tests pass; the bounded result is 7,736 B with zero static-RAM growth. Exact `0.145.0-interface-settings` remains the latest physically accepted board-01 baseline; the deferred S5 RF-positive gate remains open.
- **Next gate:** wire the lifecycle-owned Target state and two-Session comparison into the product without permanent-static overflow, then expose a useful-first on-device Target List/Detail and Compare summary whose rows open the exact evidence while preserving the accepted stable navigation/no-flicker shell. The physical S5 gate is postponed, not waived: when the ordered replacement DIV arrives, its read-only profile must pass before the retained one-command IR→nRF24→OOK→FSK matrix can close S5.3/S5.4/S5.6.
