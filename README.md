...
- **Verified checkpoint:** host/build checkpoint `E-CORR-005` starts S6.4 with the first executable on-device Targets slice. Home enables Targets only when a saved Session source exists; opening it re-identifies the exact enrolled CID, mounts the product store read-only and recovers the two atomic heads without a write path. A foreground-only 22,544 B workspace plus an 11,272 B transient admission scratch derives at most the 16 strongest current identities and fills spare rows with the strongest baseline-only identities, while leaving both source Sessions immutable. The ordinary four-row list puts Compare visits first, sorts Targets by current signal, opens full-screen detail with an exact source generation, and exposes deterministic added/removed/changed/unchanged counts. Dense-air 28-identity, monotonic-reset, negative, ASan/UBSan and the complete host suite pass. Production `0.146.0-targets` builds at 211,296 B static RAM and 3,108,996 B linked flash. Exact `0.145.0-interface-settings` remains the latest physically accepted board-01 baseline until the focused delta below passes; the deferred S5 RF-positive gate remains open.
- **Next gate:** bind exact `0.146.0-targets` to a committed source, flash board-01 once, record two bounded real passive Wi-Fi Sessions, then automatically capture Targets List→Compare→Detail and prove exact generations/CID, bounded strongest-first projection, read-only zero-write behavior, released heap, inactive outputs, zero input drops and final Home/lease 0. The physical S5 gate is postponed, not waived: when the ordered replacement DIV arrives, its read-only profile must pass before the retained one-command IR→nRF24→OOK→FSK matrix can close S5.3/S5.4/S5.6.
