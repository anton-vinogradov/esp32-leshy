...
- **Verified checkpoint:** `E-BUILD-174`/`E-AUTO-148`/`E-HIL-191`/`RB-M185` retain exact `1.0.0-dev.246` at source `54cf455810c15753220fb2bd0f497381dfabde48` (`firmware 8cdd2c01b9e3c8423d665e39b7a0581d0f5039ae4e96fa617fee934dc7ea3b6e`, embedded app identity `3c4643e014262722c3ef3ce640e2d9964f0df1431ee49cd897d331291927a221`) as the first physical CAP-049 workflow checkpoint that survives both same-boot product mounts. The fail-closed dev.245 precursor mounted once at a 31,732 B largest block, then reached the still-available drive with SPI/bus error zero but failed the second VFS allocation at 17,396 B (`ESP_ERR_NO_MEM` 257, three attempts). Exact ESP-IDF 5.5.4 analysis showed that unused `max_files=5` required a contiguous 29,512 B VFS workspace; dev.246 pins the product's one serialized direct-FatFs file to `max_files=1`/12,968 B, and both mounts complete on attempt 1 with error zero. The retained receive-only run observes no EAPOL/PMKID event, so its user result is honestly `inconclusive`/evidence-incomplete and FUNC-49 remains active; it nevertheless independently passes exact candidate/CID, fixed-target/channel, content-only repaint, privacy, no-write/no-connect/no-TX cleanup and final Home/none/lease 0.
- **Next gate:** complete a focused Wi-Fi-only Product Survey Stop/commit/cold-recovery run on the same dev.246 bytes, proving the fixed second mount through a generation-advancing write and exact-CID reopen. The current mixed-source attempt reached `running_degraded` because BLE was unavailable, then failed closed before Stop/commit with cleanup Home/none/lease 0; it is not gate evidence. A known or naturally observed EAPOL/PMKID event is still required before CAP-049 can claim useful authentication evidence, while physical HTTP parity remains deferred to a dedicated client that cannot touch active Mac Wi-Fi.
