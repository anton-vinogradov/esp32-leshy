...
- **Verified checkpoint:** `E-BUILD-190`/`E-AUTO-165`/`E-HIL-203`/`E-UX-060`/`RB-M201` physically accept the receive-only product path of CAP-051 BLE Inspector in exact `1.0.0-dev.270` at source `cd542afac79f696a2cdcf3d7abe4bd18f3bc4a82`. A selected Bluetooth card opens Raw packets, retains only that exact identity in a bounded 32-record capture, repaints rows without clearing the content, freezes explicitly or at capacity, and streams a versioned local export. One fresh-flash gate remains on the Bluetooth route for 15,265 ms, captures two selected records/36 payload bytes with zero invalid/drop/allocation/fallback counters, checks four TFT frames automatically and exits Home/none/lease 0 with the buzzer inactive. The rejected dev.269 predecessor is retained fail closed: its resident 1,856 B capture made the route bounce Home before a live scan; dev.270 shares that storage with the mutually exclusive Airspace Guard event and restores the NimBLE headroom.
- **Next gate:** implement the live platform GATT enumeration adapter and the visible selected-target permission/confirmation/cleanup UI, then physically prove success plus wrong-peer, timeout, resource-conflict and failed-cleanup paths. Pairing, characteristic read/write and subscription remain outside this slice. The passive path is accepted, but live BLE free-heap telemetry reaches only 572 B/500 B largest block after catalog fill, so the connected adapter must first release or reuse passive-scan memory and re-measure its own bounded lifecycle. CAP-050 physical located/timed export remains deferred until a separately owned non-conflicting GPS profile exists; physical HTTP parity still requires a client that cannot touch active Mac Wi-Fi.
...
| BLE Inspector preserves raw compatible packets and enters connected GATT only after explicit target/permission/lease confirmation | S7 | 🟡 in progress |
...
| **Active-confirmed** | **BLE Inspector, GATT path (CF-004):** connects only to an explicitly selected device and enumerates services/characteristics under separate permission and lease | 🟡 host/build foundation in exact `1.0.0-dev.268`: the enumeration-only permission/lease/cleanup state machine is accepted; live transport, UI and physical HIL remain open after the dev.270 passive-path acceptance |
