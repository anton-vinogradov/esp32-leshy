...
- **Verified checkpoint:** `E-BUILD-207`/`E-AUTO-182`/`RB-M218` accept exact host/build `1.0.0-dev.304` at firmware source `96a55a4d46f3171690cb3778d99acc2a95cbd0c2`. The passive product Inspector now uses a real mbedTLS ECDSA-P256/SHA-256 verifier backed by a canonical, CRC-protected, generation-counted NVS trust store for at most four public keys. Missing state restores empty; corruption and persistence failure fail closed. Enrollment/revocation requires Device Lock plus fresh confirmation, but no product UI can invoke it yet. A protected GitHub environment derives and publishes only a 128-byte public `LHAK` bundle and public metadata; private key material stays in GitHub secrets. Execution remains disconnected. Exact physical dev.303 remains the latest focused physical delta and dev.302 the periodic full anchor.
- **Next gate:** connect the owner-visible trust-store import/list/revoke UI, validate the public point and derived key ID before enrollment, then physically prove authenticated enrollment, cold restore, trusted/unknown/invalid inspection and revocation while keeping execution disconnected. Positive Serial Console traffic still waits for an explicitly reviewed no-RF `mux56-3v3` fixture; optional GPS, physical HTTP parity and the deferred S5 RF carrier gate remain externally blocked.
...
| **Passive audit** | **BadUSB inspection (CF-008):** parses a signed Automation/HID script and previews its target, permissions, actions, and bounds without executing it | ✅ exact physical dev.303 accepts malformed/unsigned packages from real exact-CID SD in stable EN/RU Inspector views with zero Action/HID/resource/RF output and complete cleanup; real trust and execution remain disconnected |
...
| **Active-confirmed** | **Automation/HID execution (CF-008):** runs only a signed, permissioned script after previewing the target, actions, ceilings, and finite duration | ⬜ planned, S7; accepted passive dev.303 deliberately still provides no HID/Action output path |
