...
- **Current phase:** `S6.3 — two-Session baseline/diff and evidence-backed change classification`.
- **Verified checkpoint:** host/build checkpoint `E-CORR-003` completes the S6.2 foundation with explicit reversible `target.merge`/`target.split` Actions. A caller-selected 128-bit operation ID and optimistic revisions make replay idempotent and collisions/conflicting state fail closed. Merge retains complete bounded snapshots of both source Targets, transfers only exact identities/evidence to the chosen stable Target ID and never edits source Observations; split restores the previous Target-ID→identity/evidence ownership and catalog order with monotonic revisions, while any later edit of the merged Target blocks undo rather than overwriting it. The eight-record history is included with catalog and correlation decisions in deterministic schema-v3 state; schema v2 remains readable as an empty merge history. Fault injection at all six durable boundaries proves old-or-new atomic recovery of graph, decisions and merge history. Native enumerated round trips, ASan/UBSan and production builds pass. Runtime wiring/migration and product UI remain owned by S6.4. Exact `0.145.0-interface-settings` remains the latest physically accepted board-01 baseline; the deferred S5 RF-positive gate remains open.
- **Next gate:** define the bounded S6.3 comparison model, then prove a deterministic two-Session baseline/diff classifies new, disappeared and changed Targets and every conclusion opens exact retained evidence. The physical S5 gate is postponed, not waived: when the ordered replacement DIV arrives, its read-only profile must pass before the retained one-command IR→nRF24→OOK→FSK matrix can close S5.3/S5.4/S5.6.
...
| S6.3 | Baseline/diff compares two Sessions and classifies new, disappeared and changed Targets with every conclusion opening its evidence | 🟡 in progress |
