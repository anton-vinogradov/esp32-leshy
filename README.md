...
- **Verified checkpoint:** `E-BUILD-237`/`E-AUTO-215`/`E-HIL-233`/`E-UX-088`/`RB-M249` accept exact physical `1.0.0-dev.365`: the Airspace Guard first asks about the surroundings and offers **Everyday**, **Quiet place**, and **Busy place** with plain-language consequences. The affected delta proves all three exact policies, three passive starts/cancels, selector-only repaint with zero static-pixel changes and final Home/none/lease 0. The existing full Airspace Guard release gate remains unchanged; its same-image ambient precursor stayed honestly fail-closed on malformed ingress. Exact dev.357 remains the periodic full anchor.
- **Next gate:** progress remains **23/62**, while the fixed Wi-Fi track is now **12/21 done, 9/21 remaining**. `WF-13` is physically accepted with privacy-minimal retained evidence; no ambient SSID/BSSID/vendor, raw run or screenshot is retained. The next focused delta is `WF-14`: complete the already bounded offline AP/station Field Survey journey, revisit comparison and WiGLE export. The companion verification chain `WF-11` remains active in parallel; second-board and dedicated-client physical gates stay parked, and RF TX remains forbidden.
