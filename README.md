...
- **Verified checkpoint:** exact `0.154.0-targets-notes-edit` at source `16ccf285713cbb2cd33382fd95992f4e6e77199f` is physically accepted in `E-AUTO-112`/`E-HIL-172`/`E-UX-049`. Target Detail→Actions now fits Favorite, Name, Tags and Notes in four rows. The bounded 160-byte note editor sets ASCII `A`, atomically publishes catalog-only schema-v3 generation 5→6 and cold-reopens bytes `41`, then clears the note, publishes generation 6→7 and cold-reopens length 0. Both mutations re-identify the exact CID and perform 3 writes/3 file syncs/3 directory syncs. UI acknowledgement is 153/153 µs, supervised workers complete in 2.943/2.965 s, the 16 KiB pre-mount workspace fits a 34,804 B largest block, heap releases from 84,736 B to 96,516 B and final state is Home/none/lease 0. The deferred S5 RF-positive gate remains open.
- **Next gate:** expose correlation proposals with evidence/confidence and bounded accept/reject. Merge/split remains behind an explicit confirmation that previews ownership change and preserves reversal. The physical S5 gate remains postponed, not waived, until the replacement DIV arrives and passes its read-only profile.
