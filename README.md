...
- **Verified checkpoint:** `E-BUILD-193`/`E-AUTO-168`/`E-HIL-206`/`E-STORAGE-034`/`E-SURVEY-018`/`E-UX-063`/`RB-M204` close the inherited Product Survey terminal gate on the exact already-flashed `1.0.0-dev.276` image. One automatic Wi-Fi+BLE pass accounts 20+31=51 observations with 51 forwarded and zero drops, commits SD generation 175→176, then a cold reset admits exact 176/51 read-only on attempt 1 with the exact CID, zero physical writes and invariant heap 148,124/74,828 B. Library reopens `field-visit-live`, and all cleanup boundaries finish Home/none/lease 0. The raw runner remained fail-closed only because its final host oracle still expected the superseded `product-passive-live` name; the retained checker permits exactly that one failure and exact old-runner SHA, revalidates every terminal condition with the corrected oracle, and rejects any other mismatch.
- **Next gate:** start the next unblocked accepted capability, CAP-052 Device Lock, with a host/build foundation for local PIN setup, bounded retry/recovery and non-bypassable Stop/panic/update/factory-reset cleanup. CAP-050 located/timed export still needs a separately owned non-conflicting GPS profile; physical HTTP parity still needs a dedicated client that cannot touch active Mac Wi-Fi or the Cardputer; the deferred S5 RF carrier gate remains blocked on replacement hardware.
