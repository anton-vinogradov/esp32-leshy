...
- **Next gate:** `FF-2` remains active. Exact physical `1.0.0-dev.343` (`E-BUILD-222`/`E-AUTO-198`/`E-HIL-226`/`E-UX-077`/`RB-M234`) is owner-accepted: the Bluetooth list and open card no longer flicker. Host/build `1.0.0-dev.345` (`E-BUILD-223`/`E-AUTO-199`/`E-UX-078`/`RB-M235`) now fixes the product at 62 capabilities without creating 62 menu rows: nine Home domains lead through entity cards to contextual tasks and one shared guided flow. The first Wi-Fi slice opens a plain-language password-check explanation before recording anything; normal user choices no longer require implementation jargon. Live Wi-Fi sorting continues after user input while focus remains anchored to the selected identity. Protocol Workbench, truthful Lab empty state and the byte-identical screenshot → protected Library → USB export → same-generation cold reopen remain next. RF TX stays forbidden.
