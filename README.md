...
- **Verified checkpoint:** `E-BUILD-191`/`E-AUTO-166`/`E-HIL-204`/`E-UX-061`/`RB-M202` physically accept the first connected, enumeration-only CAP-051 BLE Inspector slice in exact `1.0.0-dev.272` at source `fcd0683cb169ac41e42ff302d074a146457c1b1d`. A private exact connectable fixture is selected by hash without identifier disclosure; the user sees permission review and a second confirmation before passive scan ownership is released. The live NimBLE adapter then connects to that peer, lists 5 services and 7 characteristics within fixed 16/48 capacities, performs zero characteristic reads/writes/subscriptions/pairings, changes 0/76,800 pixels on a stable result card, and Back proves disconnect plus Home/none/lease 0. Rejected dev.271 reset under an interrupt watchdog because a callback held an interrupt-disabling spinlock while starting the next NimBLE operation; dev.272 serializes callbacks with a bounded static recursive task mutex.
- **Next gate:** physically exercise the connected GATT negative lifecycle — wrong peer, connect/discovery timeout, resource conflict and failed cleanup — against the exact dev.272 boundary, without enabling pairing, characteristic read/write or subscription. Success is accepted; CAP-051 remains active until those fail-closed paths are retained. CAP-050 located/timed export still needs a separately owned non-conflicting GPS profile, and physical HTTP parity still needs a client that cannot touch active Mac Wi-Fi.
