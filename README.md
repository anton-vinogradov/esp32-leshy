...
- **Verified checkpoint:** `E-BUILD-198`/`E-AUTO-173`/`E-HIL-210`/`E-STORAGE-037`/`RB-M209` accept exact physical `1.0.0-dev.283` and close the CAP-052 Device Lock implementation boundary. One normal product commit on original board-01 publishes encrypted generation 3 with a valid chunked `LENC` AES-256-GCM envelope: 615 plaintext bytes become 695 physical bytes, the header and exact size validate, ciphertext differs from known plaintext, and the catalog authenticates. The preceding generation 2 cold-reopens read-only with exact CID and zero physical writes. Queues/drops, radio connect/raw-TX calls and leaked leases are zero; final state is Home/none/lease 0, safety armed. The retained dev.282 watchdog failure proves fail-closed reset/zero outputs, and the dev.283 precursor is rejected only for the corrected `ok` versus `FR_OK` oracle token.
- **Next gate:** start the next unblocked S7 implementation slice, CAP-053 bounded Serial Console/shared Actions CLI, while CAP-052 release hardening moves to the later power-cut/signed-update/privacy matrix. CAP-050 located/timed export still needs a separately owned non-conflicting GPS profile; physical HTTP parity still needs a dedicated client that cannot touch active Mac Wi-Fi or the Cardputer; the deferred S5 RF carrier gate remains blocked on replacement hardware.
