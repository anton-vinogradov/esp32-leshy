...
- **Verified checkpoint:** `E-BUILD-187`/`E-AUTO-162`/`E-HIL-201`/`RB-M198` accept live passive Wi-Fi station capture and its truthful export boundary on original board-01 with exact `1.0.0-dev.266` at source `ffd8f8f23cd3153a5de415049fc2665544af66c1`. First/revisit generations 174/175 each retain 2 stations with zero source/pipeline drops; the revisit contains 15 AP, 2 stations and 34 BLE records. Read-only native export preserves all 51 records including both stations, while WiGLE emits exactly the 49 representable AP/BLE rows and reports 2 skipped stations. It remains `untimed_unlocated`, `upload_ready=false`; no ambient identifiers or raw exports are retained and final cleanup is Home/none/lease 0.
- **Next gate:** admit optional trusted GPS and UTC into Field Survey and prove located/timed export without weakening the already accepted untimed/unlocated path. Physical HTTP parity remains deferred to a dedicated client that cannot touch active Mac Wi-Fi.
