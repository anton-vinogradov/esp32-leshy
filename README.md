...
- **Verified checkpoint:** `E-BUILD-179`/`E-AUTO-153`/`E-HIL-194`/`E-UX-058`/`RB-M190` physically accept exact `1.0.0-dev.251` on original board-01 at source `d84f8259c6781dcbe90ae00fba00f0c6f4379c32`. The Bluetooth device card now composes each changing quality/dBm, range and trend row in one reusable 648 B monochrome sprite before a single display push. A live update performs three atomic row pushes, one radar delta repaint, zero full/content clears, zero sprite-allocation failures and zero direct fallbacks; 805 radar pixels change while static chrome stays unchanged. Passive BLE, zero driver drops, exact CID, two complete lifecycles, invariant post-cleanup heap and final Home/none/lease 0 all pass.
- **Next gate:** reuse the exact `1.0.0-dev.251` image already on the original DIV and prove explicit Save, atomic SD commit, exact-generation cold reopen and PCAP export; `.hc22000` readiness must additionally be demonstrated by useful bounded PMKID or replay-consistent M1→M2 evidence, while an ambient no-evidence run must remain honest and non-exportable. Product Survey Stop/commit/cold-recovery remains separately pending after its earlier `running_degraded` attempt failed closed before commit. Physical HTTP parity remains deferred to a dedicated client that cannot touch active Mac Wi-Fi.
