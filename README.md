...
- **Verified checkpoint:** `E-BUILD-194`/`E-AUTO-169`/`RB-M205` accept the exact host/build `1.0.0-dev.277` CAP-052 Device Lock foundation. A pure state machine provides 6–12 digit PIN policy, PBKDF2-HMAC-SHA-256 verification, persistent reset-resistant retry through recovery-only, volatile unlock expiry, a fail-closed access matrix and destructive-only recovery ordering. A fixed 68-byte `LDLK` record and independent NVS provisioned latch distinguish virgin, torn, missing and corrupt state. Production ESP32-S3 crypto/NVS adapters are linked and read-only restore runs at boot; strict native, contract, complete tracked host and production-build gates pass. Existing product UI/actions are not gated yet, so this is not presented as complete Device Lock or encrypted data at rest.
- **Next gate:** add on-device PIN setup/lock/unlock and status UI, measure the real PBKDF2/watchdog budget, prove cold persistence and reset-resistant delay on original board-01, and route safe Stop/panic/update/factory reset around the lock without exposing protected content. CAP-050 located/timed export still needs a separately owned non-conflicting GPS profile; physical HTTP parity still needs a dedicated client that cannot touch active Mac Wi-Fi or the Cardputer; the deferred S5 RF carrier gate remains blocked on replacement hardware.
...
| **Active-confirmed** | **Device Lock (CF-006):** local PIN, bounded retry/recovery, and protection for secrets/evidence without blocking Stop, panic, or recovery | 🟡 next implementation gate, S7 |
