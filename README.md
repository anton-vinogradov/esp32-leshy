...
- **Verified checkpoint:** `E-BUILD-196`/`E-AUTO-171`/`E-HIL-208`/`E-STORAGE-035`/`RB-M207` accept the exact physical `1.0.0-dev.280` CAP-052 Device Lock persistence/retry slice. On original board-01, an isolated disposable NVS namespace enrolls an ephemeral PIN, cold-restores locked generation 1, preserves full post-KDF 5 s and 15 s retry bounds at generations 2/3, cold-restores retry delay, unlocks at generation 4 and explicitly cleans the fixture. Two cold starts prove pending cleanup cannot be forgotten; the product namespace remains virgin before, during and after the test. No raw PIN, digest, whole-NVS copy, product-state write, radio traffic, clone, Cardputer or Mac Wi-Fi is involved; final input errors/drops are zero and cleanup returns Home/none/lease 0.
- **Next gate:** physically prove the fifth-wrong-PIN `recovery_only` transition and destructive recovery ordering, then route safe Stop/panic/update/factory reset and protected actions through the access matrix without exposing protected content. Authenticated encryption at rest remains a separate gate. CAP-050 located/timed export still needs a separately owned non-conflicting GPS profile; physical HTTP parity still needs a dedicated client that cannot touch active Mac Wi-Fi or the Cardputer; the deferred S5 RF carrier gate remains blocked on replacement hardware.
...
| Device Lock protects secrets/evidence with local PIN, bounded retry and tested recovery without blocking Stop/panic/recovery | S7 | 🟡 in progress |
...
| **Active-confirmed** | **Device Lock (CF-006):** local PIN, bounded retry/recovery, and protection for secrets/evidence without blocking Stop, panic, or recovery | 🟡 exact dev.280 physically accepts enrollment, cold locked/retry restore, full 5/15-second post-KDF delays and isolated cleanup; recovery-only, action admission and encryption remain, S7 |
