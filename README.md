...
- **Verified checkpoint:** `E-BUILD-212`/`E-AUTO-187`/`E-HIL-220`/`E-UX-071`/`RB-M223` accept the first physical `FF-1` slice on original board-01. Exact `1.0.0-dev.312` at firmware source `a2964985eff8bd33d8c429d4ec37350446d56834` keeps a selected passive Wi-Fi client identity and channel stable while signal samples advance 29→32. The detail screen performs one initial full render, then only atomic live-row/radar deltas: identity and chrome change by 0 pixels, live data by 49 pixels, with no further content clear/full repaint. Two complete monitor lifecycles keep heap byte-invariant, Device Lock uses and removes an isolated HIL namespace, storage stays 8/54 with zero writes, and cleanup ends Home/none/lease 0. Exact dev.302 remains the periodic full anchor.
- **Next gate:** finish the remaining `FF-1` Targets Radar/localize delta and perform the coherent Wi-Fi/BLE/Targets interaction review. Previously accepted Wi-Fi-network and BLE cards remain inputs to that review; calibrated distance is not claimed. Automation/HID stays parked at the accepted safe dev.308 public-trust checkpoint with execution disconnected; positive Serial Console traffic still waits for an explicitly reviewed no-RF `mux56-3v3` fixture, while optional GPS, physical HTTP parity and the deferred S5 RF carrier gate remain externally blocked.
...
| FF-1 | Wi-Fi/BLE Radar accepted; finish Targets/localize and the coherent cross-radio interaction review for `FUNC-17` | 🟡 active |
