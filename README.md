...
- **Verified checkpoint:** exact `0.144.0-full-guided-s5-rx` turns the public Device → Self-Test → Full path into an autonomous passive hardware/artifact delta on board-01. Quick passes 9/9; Full reports 28 pass, zero fail, one truthful capability-coverage blocker and three N/A. The run exercises all three nRF24 receive paths, a 64-bin CC1101 sweep, bounded OOK/FSK windows and the IR input; FSK records 72 edges in 32 windows with zero overflow. Recovery/Library/disposable audits preserve exact CID, generation 110 and zero product observations; PCAP is correctly N/A for that empty admitted session, while the isolated scratch performs 3 writes/505 B and is removed. Heap finishes at 96,880 B free with a 63,848 B boot-lifetime minimum, 14,960/14,696 B above the independent 80/48 KiB gates. Evidence `E-HIL-162` retains 13 automatic TFT frames, zero radio TX/input drops and final Home/none/lease 0. This accepts the integrated Full/Guided passive self-test delta, not the remaining physical RF-positive or S5 exit gate.
