...
- **Verified checkpoint:** `E-BUILD-245`/`E-AUTO-224`/`E-HIL-241`/`E-SAFETY-089`/`RB-M258` accept exact physical `1.0.0-dev.376`: every real runtime WDT now writes one exact-app cause record to internal NVS on the first Safe-Mode boot, preserves it without duplication across later resets, and opportunistically mirrors it to the enrolled exact-CID SD only after explicit Safe-Mode clear. The retained rejected dev.375 run caught a loopTask stack canary in the new SD path; dev.376 moves the FatFS sector workspace off that stack (4,496 → 384 B), reconnects native USB after every HIL restart, and passes the complete WDT → NVS → restart → SD → Home/none/lease 0 chain. Dev.374 remains the accepted sustained Wi-Fi runtime baseline and dev.357 the periodic full anchor.
