...
- **Next gate:** Progress **24/62**, Wi-Fi **17/21 done, 4/21 remaining**. Power-cycle the original DIV, verify exact dev.391 runtime without reflashing, then deploy the same ordinary image once to the receiver. Run the credential/receiver delta, then use an owned phone for the real-client branch. An AP probe response may precede association: preserve first provenance and record the client frame independently. USB silence remains unexplained; laptop Wi-Fi is untouched.
