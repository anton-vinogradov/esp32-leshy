...
- **Next gate:** connect board-02 as the already qualified bounded IR fixture and run the one-command exact `0.137.0-pulse-store-deadline` flow: two fixed NEC emissions, normal IR save, injected 10 s pre-storage stall, 8 s trip, retained restart, recovery block, two-action clear and final zero lease. The Sub-GHz worker is source/native-covered only; its positive physical gate and paused S5.3 nRF gate still require a repaired or replacement RF carrier/device with plausible read-only identities.
