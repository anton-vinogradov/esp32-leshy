...
- **Verified checkpoint:** Dev.388: 8 TFT captures, fresh same-AP observations at 12/14/15/17 samples, host-observed deadline Stop at 60.395 s; countdown changed 29 dynamic and zero static pixels. Both boards finish Home/none/lease 0, safety armed, zero drops/SD operations. Dev.385 menu and dev.384 card/13-channel graph remain separate exact-image baselines; no full-matrix or optical flicker guarantee.
- **Next gate:** Progress **24/62**, Wi-Fi **17/21 done, 4/21 remaining**. Next: integrate the bounded name decoder with selected-channel reception and card provenance/conflicts, then prove client-association enrichment separately. Reconnection restored the original; dev.387/388 boot and dev.388 AP delta passed. The dev.386 startup failure remains retained; root cause is not conclusively established. Laptop Wi-Fi is untouched.
