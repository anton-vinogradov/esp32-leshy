...
- **Verified checkpoint:** `E-BUILD-200`/`E-AUTO-175`/`E-HIL-211`/`E-UX-066`/`RB-M211` accept the CAP-053 platform/product path and physical stock-profile negative in exact `1.0.0-dev.285` at source `8a5799aca03f96ae518ae0c7c7391b43828d6f4f`. The allocation-free `Serial1` endpoint has a 256-byte volatile ring, 64-byte service bound, monitor-only TX disable and scrub/close cleanup. Device → Serial Console and the strict CLI share the typed Action boundary. On original stock RF-shield hardware preview and confirmed run both stop at `mux_conflict` before configure/start; `Console|Mux56=0`, `pins_touched=false`, heap is 72,576→72,576 B, safe outputs are unchanged and cleanup ends Home/none/lease 0.
- **Next gate:** use an explicitly reviewed no-RF `mux56-3v3` fixture to accept positive receive-only traffic and timeout/fault/panic cleanup, then accept Bridge only with explicit `serial.write` permission. Encrypted transcript Save remains a separate later action. CAP-050 located/timed export still needs a separately owned non-conflicting GPS profile; physical HTTP parity still needs a dedicated client that cannot touch active Mac Wi-Fi or the Cardputer; the deferred S5 RF carrier gate remains blocked on replacement hardware.
