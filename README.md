...
- **Verified checkpoint:** exact `0.156.0-targets-reject-rebuild` at firmware source `0559fc632f8a1dc856000d57113d24b61ab801aa` is physically accepted in `E-AUTO-114`/`E-HIL-174`/`E-UX-051`. Explicit Reject atomically advances Target state 10→11 and decision log 2→3 while keeping Target revision 5→5, visible ownership 4→4 and the full catalog 16→16. The immediate rebuilt view remains usable even at the hard catalog bound, reports the seven skipped admissions truthfully, and a hardware reset cold-reopens generation 11 with 84 immutable source identities. The successful worker state is adopted instead of allocating a duplicate 22.5 KiB graph while the just-exited task stack awaits cleanup; persistence still performs 3 writes/3 file syncs/3 directory syncs, heap fully releases and terminal state is Home/none/lease 0. The deferred S5 RF-positive gate remains open.
