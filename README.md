...
- **Verified checkpoint:** `E-BUILD-241`/`E-AUTO-219`/`E-HIL-237`/`RB-M253` accept exact physical `1.0.0-dev.369`: Device → Connection → Identity Privacy defaults to a fresh private identity for each Leshy Wi-Fi session and offers an explicit hardware-identity opt-out. Two separate passive nearby-network sessions advance generation/application counters 1→2, prove locally administered unicast identities different from hardware, retain no raw address and never modify nearby identities. SoftAP, host-network tools, Mac Wi-Fi changes and raw radio TX remain zero; exact-CID storage is read-only and cleanup reaches Home/none/lease 0. The sandbox-denied precursor is retained by hash and never accessed or flashed the board. Exact dev.357 remains the periodic full anchor.
- **Next gate:** progress remains **24/62**, while the fixed Wi-Fi track is now **16/21 done, 5/21 remaining**. `WF-17` closes the own-STA/AP portion of `FUNC-59`; its broader short-lived synthetic owned-Capture identity work remains active, so the product denominator does not advance. `WF-11` is the next executable Wi-Fi chain: finish the privacy-safe owned-evidence verification journey. `WF-18…WF-21` stay queued behind explicit isolated-fixture/second-board admission, and RF TX remains forbidden.
