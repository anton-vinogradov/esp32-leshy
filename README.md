...
- **Verified checkpoint:** `E-BUILD-189`/`E-AUTO-164`/`RB-M200` accept the host/build foundation of CAP-051 BLE Inspector in exact `1.0.0-dev.268` at source `52b2d1655b486ae029ce2402317c2f270bf88c0c`. The passive boundary now forwards and copies exact compatible legacy advertisement bytes only for one selected identity. Connected GATT has no pairing/read/write/subscribe surface: it requires a fresh exact target, an explicit enumeration-only permission and confirmation token, then owns a separate ESP-RF lease until deterministic disconnect. Wrong identity, malformed/overflowed facts, timeout and failed cleanup remain fail closed.
- **Next gate:** wire the bounded raw capture into the selected Bluetooth device workflow and a versioned export, then implement the platform GATT enumeration adapter and visible permission/cleanup UI before focused physical HIL. Dev.268 is foundation only: it does not yet expose a product screen or make a live connection. CAP-050 physical located/timed export remains deferred until a separately owned non-conflicting GPS profile exists; physical HTTP parity still requires a client that cannot touch active Mac Wi-Fi.
...
| **Passive audit** | **BLE Inspector, receive path (CF-004):** preserves compatible raw advertising records with provenance; this is not a promise of arbitrary BLE link-layer sniffing | 🟡 host/build foundation in exact `1.0.0-dev.268`: bounded exact selected-target payload retention is accepted; product capture/export and physical HIL remain open |
...
| **Active-confirmed** | **BLE Inspector, GATT path (CF-004):** connects only to an explicitly selected device and enumerates services/characteristics under separate permission and lease | 🟡 host/build foundation in exact `1.0.0-dev.268`: the enumeration-only permission/lease/cleanup state machine is accepted; live transport, UI and physical HIL remain open |
