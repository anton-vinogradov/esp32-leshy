...
- **Verified checkpoint:** `E-BUILD-208`/`E-AUTO-183`/`E-HIL-216`/`E-UX-069`/`RB-M219` accept exact physical `1.0.0-dev.306` at firmware source `41bf103008202e742598dfd3e092b28aeda45d95`. The last Device item now exposes the protected four-key P-256 trust list/import/revoke UI. It reads only `/leshy/automation/v1/automation-owner.lhak`, validates public material before a separate review and keeps execution disconnected. Focused button/touch HIL retains stable EN/RU frames and the missing-bundle result with unchanged count/generation, read-only SD, zero private-key/Action/HID/RF output, restored Device Lock/language and final Home/none/lease 0. Exact dev.302 remains the periodic full anchor.
- **Next gate:** use the GitHub-generated public `LHAK` bundle to physically prove authenticated positive enrollment, cold restore, trusted/unknown/invalid inspection and revocation while keeping execution disconnected. Positive Serial Console traffic still waits for an explicitly reviewed no-RF `mux56-3v3` fixture; optional GPS, physical HTTP parity and the deferred S5 RF carrier gate remain externally blocked.
