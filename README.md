...
- **Verified checkpoint:** `E-BUILD-244`/`E-AUTO-223`/`E-HIL-240`/`E-SAFETY-088`/`RB-M257` accept the exact `1.0.0-dev.374` correction for the overnight Wi-Fi-cycle hang. The retained dev.372 incident proves a real Task-WDT reset into latched Safe Mode rather than heap exhaustion or an input/storage failure. Dev.374 bounds every foreground Product/Wi-Fi queue drain, rate-limits passive data-frame inspection and retains the first exact-app watchdog page/stage/CPU trace after quiescing outputs. Exact delta HIL then sustains Wi-Fi Devices for 180.122 s/1,477 hops and Nearby Networks for 180.008 s/66 cycles with continuous progress, zero radio drops, zero watchdog recurrence, zero post-warm heap drift, read-only storage and final Home/none/lease 0. The dev.373 IR replay foundation remains accepted host work with its two-board physical gate deferred; dev.357 remains the periodic full anchor.
