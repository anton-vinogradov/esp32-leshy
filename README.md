...
- **Verified checkpoint:** `E-BUILD-177`/`E-AUTO-151`/`RB-M188` accept exact host/build `1.0.0-dev.249` at product source `d47b2ee5e1636981474398246c5e0c49d88db2ea`: CAP-049 now has a bounded canonical Hashcat mode-22000 serializer, explicit Save confirmation, schema-8 atomic persistence with exact-generation reopen/re-analysis, honest PCAP-only fallback, useful-evidence-gated `.hc22000` export and a synthetic-persistence prohibition. The full tracked host matrix, focused ASan/UBSan cases, contract/mutation checks and clean production build pass. No device, serial port, Mac Wi-Fi, clone or Cardputer was touched, so this is not physical acceptance; exact `1.0.0-dev.248` remains the latest accepted physical baseline.
- **Next gate:** flash exact `1.0.0-dev.249` only to the original DIV and prove explicit Save, atomic SD commit, exact-generation cold reopen and PCAP export; `.hc22000` readiness must additionally be demonstrated by useful bounded PMKID or replay-consistent M1→M2 evidence, while an ambient no-evidence run must remain honest and non-exportable. Product Survey Stop/commit/cold-recovery remains separately pending after its earlier `running_degraded` attempt failed closed before commit. Physical HTTP parity remains deferred to a dedicated client that cannot touch active Mac Wi-Fi.
