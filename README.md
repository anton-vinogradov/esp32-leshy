...
- **Verified checkpoint:** exact `0.155.7-targets-shared-codec` at firmware source `7328220c641e25ca8ae34ea311500990108a1a18` and verification source `9ae9192803c96523ce0b76558494a8899fa88e4c` is physically accepted in `E-AUTO-113`/`E-HIL-173`/`E-UX-050`. The on-device review explains one real Wi-Fi→BLE proposal with both exact observations, confidence `medium`, score 463 and the two awarded features; explicit Accept atomically advances Target state 8→9, decision log 0→1 and the selected Target revision 3→4. A hardware reset cold-reopens that same ID/revision with the proposal gone while the immutable source population remains 69. The shared Session/Target codec workspace lets the full decision graph mount and persist at a measured 29,684 B largest heap block; the adjacent mutation regression performs 3 writes/3 file syncs/3 directory syncs and terminal state is Home/none/lease 0. The deferred S5 RF-positive gate remains open.
- **Next gate:** complete the explicit correlation Reject path without changing ownership, then present reversible merge/split on-device with an ownership preview, confirmation, immutable source evidence and a cold-reopened exact reversal. The physical S5 gate remains postponed, not waived, until the replacement DIV arrives and passes its read-only profile.
