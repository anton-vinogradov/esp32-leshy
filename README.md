...
- **Verified checkpoint:** exact `0.151.2-targets-favorite-compact` at source `02c55b61b0ec5e1efc82efce5423b5c34bf82bcb` is physically accepted in `E-AUTO-109`/`E-HIL-169`/`E-UX-046`. Target Detail→Actions toggles Favorite through the typed Action boundary, re-identifies the exact CID, atomically publishes catalog-only schema-v3 generation 1→2 with 3 writes/3 file syncs/3 directory syncs, then cold-reopens the same 128-bit Target with the changed value. UI acknowledgement is 148 µs, the supervised worker completes in 2.690 s, the 16 KiB pre-mount workspace fits a 34,804 B largest block, heap returns to 97,012 B and final state is Home/none/lease 0. Two real 32 KiB allocation failures remain retained with zero writes; a third rejected run records the corrected mutation but an early-observation defect in the HIL runner. The deferred S5 RF-positive gate remains open.
- **Next gate:** add the next complete bounded metadata interaction (name, then tags/notes) using the same exact-CID atomic path and stable Target selection. Correlation accept/reject follows; merge/split remains behind an explicit confirmation that previews ownership change and preserves reversal. The physical S5 gate remains postponed, not waived, until the replacement DIV arrives and passes its read-only profile.
