...
- **Verified checkpoint:** exact `0.170.0-companion-usb-rx` at source `b58fbc054522cecfca5dd4afcd6ea61098cb05c0` is physically accepted in `E-BUILD-147`/`E-AUTO-118`/`E-HIL-177`/`E-COMPANION-002`. On the explicitly selected original DIV port, native USB grants read-only Session/Target/Compare scopes only while ready Targets owns the live snapshot; it returns two exact Sessions, all 16 Targets, all five detail sections and seven comparison rows (2 added, 1 removed, 0 changed, 4 unchanged). Exact 512-byte input succeeds, 513-byte/malformed/unknown/out-of-range/mutation/post-exit requests fail closed, and leaving Targets revokes the grant. The run performs zero radio TX, storage writes, input drops, port discovery or Cardputer opens and finishes Home/none/lease 0 with invariant 92,972 B free heap. This accepts the read-only native-USB boundary of S6.5; local Web and mutations remain open, as does the deferred S5 RF-positive gate.
