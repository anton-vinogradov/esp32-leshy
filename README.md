...
- **Next gate:** prove the exact dev.257 Field Survey setup/first-visit/revisit/result path in one focused receive-only board-01 delta, including a truthful incomplete-input negative and clean Home/none/lease 0. Native/WiGLE artifact routing, live station capture and optional trusted GPS/time then remain as separate CAP-050 slices. Product Survey Stop/commit/cold-recovery remains separately pending after its earlier `running_degraded` attempt failed closed before commit. Physical HTTP parity remains deferred to a dedicated client that cannot touch active Mac Wi-Fi.
