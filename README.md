...
- **Verified checkpoint:** `E-BUILD-175`/`E-AUTO-149`/`RB-M186` accept exact host/build `1.0.0-dev.247` at source `ccb4a3a5351d065e312ca29cf689d3acd9e6d93b`. It adds the bounded CAP-049 result controller and EN/RU path `Result → Actions → Peer → Evidence → Detail`, truthful `Full`/`PMKID`/`Partial` priority, `Repeat`, filtered zero-mask peers, content-only repaint and fail-closed rejection of M2/M3/M4 with an all-zero Key MIC. The storage foundation adds authentication provenance as Session/segment schema 8 and capture wire 5 while preserving generic raw-frame opening and schema-4 read compatibility. The full `tools/test.sh` suite passed before independent review found that zero-MIC P1; focused capture/policy ASan+UBSan, focused post-fix review, and production build/budget checks pass on this exact source. Exact app/factory hashes are `b3bb3e3a787ef36aa306d0711a40c0b5b730a02f69fc1d72e4a59315b0435ed1`/`186c24fbd831db40dc5b936c6bbd91bee7908cefc38c9aa651fbfd8676c09ff1`; embedded ELF/app identity is `954a5a80064a951bac6a3f278cdd5e06292d7589871ab132f5cd680e36e9dba4`, map is `2fd0300e65fb4b4d4c4b2bc0a1f01bb97627e19a65eb7d1f51503b9b82f79ccf`. This is host/build evidence only: the accepted physical baseline remains `E-BUILD-174`/`E-AUTO-148`/`E-HIL-191`/`RB-M185` on exact dev.246, whose ambient run saw no EAPOL/PMKID and correctly stayed `inconclusive`.
- **Next gate:** after USB power-cycling the original DIV, flash exact dev.247 once and run the focused TFT/navigation regression for `Result → Actions → Peer → Evidence → Detail`, `Repeat`, Up/Down, Right/OK inward, Left/Back and content-only repaint. Only after that physical delta passes should the standard artifact serializer be implemented, product persistence/export be wired to the existing atomic store, and the path be exercised with useful EAPOL/PMKID evidence. Product Survey Stop/commit/cold-recovery is still separately pending: the current mixed-source attempt reached `running_degraded`, failed closed before commit and is not gate evidence. Physical HTTP parity remains deferred to a dedicated client that cannot touch active Mac Wi-Fi.
...
| Focused Wi-Fi authentication Capture reports EAPOL/PMKID and complete/incomplete handshakes, then exports PCAP and `hc22000` | S7 | 🟡 in progress |
...
| **Passive audit** | **Wi-Fi authentication Capture (CF-002):** distinguishes EAPOL/PMKID and complete/incomplete handshakes, preserves a focused Capture, and exports PCAP/`hc22000` | 🟡 active: bounded fail-closed host parser accepted in dev.242; live capture, UI, storage/export and physical HIL remain open |
