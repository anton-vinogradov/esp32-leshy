...
- **Verified checkpoint:** `E-BUILD-199`/`E-AUTO-174`/`RB-M210` accept the host/build foundation of CAP-053 in exact `1.0.0-dev.284` at source `82453d60c17647d894c8f988c35d9cb412883669`. A typed dispatcher now validates schema, authorization, permission, hardware capability, fresh confirmation and atomic `Console`+`Mux56` ownership before starting any action. The shared CLI exposes only preview/run/status/cancel over named profiles; duplicate/unknown/missing fields, unsupported baud, raw GPIO and unconfirmed execution fail closed. This checkpoint does not claim a platform UART adapter, product UI or physical serial traffic.
- **Next gate:** add the bounded Arduino UART adapter, RX ring and Device → Serial Console UI over the accepted Action boundary, then physically accept the receive-only negative/cleanup path on stock RF-shield hardware and the positive path only on a separately declared no-RF `mux56-3v3` fixture. CAP-050 located/timed export still needs a separately owned non-conflicting GPS profile; physical HTTP parity still needs a dedicated client that cannot touch active Mac Wi-Fi or the Cardputer; the deferred S5 RF carrier gate remains blocked on replacement hardware.
