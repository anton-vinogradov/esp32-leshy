...
- **Next gate:** rerun the one-command exact `0.137.0-pulse-store-deadline` flow with fixture `0.2.5-shared-pin-safe`: two fixed NEC emissions, normal IR save, injected 10 s pre-storage stall, 8 s trip, retained restart, recovery block, two-action clear and final zero lease. The first exact attempt and an independent accepted-scenario regression were rejected because board-01 observed zero IR transitions even though fixture `0.2.4` reported a completed 68 ms emission. Review found GPIO14 modelled both as LEDC IR TX and as an independently driven third nRF CE; `0.2.5` gives that physical shared pad a single LEDC-safe owner and adds a source guard. This is a candidate cause, not physical acceptance, until the fresh-flash regression passes. The Sub-GHz worker is source/native-covered only; its positive physical gate and paused S5.3 nRF gate still require a repaired or replacement RF carrier/device with plausible read-only identities.
...
| S5.5 | Runtime completeness: exact 0.136 accepts Product Survey preparation/admission, calibrated Wi-Fi+BLE workers and Wi-Fi Capture Store; 0.137 IR/Sub-GHz Store source/native/automation is ready; the first two-board IR attempt was rejected and fixture 0.2.5 now fixes the shared GPIO14 IR/CE3 ownership before a fresh physical rerun; low-voltage safe-write, sleep/resume and applicable explicit GPS/PN532 assembly profiles remain | 🟡 in progress |
