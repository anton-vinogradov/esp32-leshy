...
- **Verified checkpoint:** `E-BUILD-216`/`E-AUTO-192`/`E-HIL-224`/`E-UX-076`/`E-STORAGE-070`/`RB-M228` accept exact physical `1.0.0-dev.331`. The owner-only, separately confirmed **Disable PIN** action preserves protected access through a cold boot: state remains `disabled`, exact CID `FE343253440000002000000055019CB7` reopens generation 8/observation 54 read-only with its existing Library entry, physical writes and RF TX remain zero, and cleanup ends Home/none/lease 0. The PIN/digest was never observed or retained. Exact dev.302 remains the periodic full anchor.
- **Next gate:** `FF-2` remains active. Exact dev.331 combines the protected screenshot slice from dev.329, streamed no-PSRAM screenshot transfer and idle-repaint suppression from dev.330, and accepted passwordless protected access from dev.331. Host/build dev.332 adds the cross-cutting repaint rule required by the product review: every full-scene call now declares itself explicitly, unchanged live ticks are no-ops, and Bluetooth-list RSSI updates atomically replace only the bounded signal line and quantized bars. Host/build dev.333 applies the same rule to the first receive-only IR Protocol Workbench slice. Dev.334 adds deadline-driven scanner-rate coalescing; dev.335 then restores the useful 0.x snapshot principle by publishing the BLE list once per current two-second passive scan window while retaining immediate navigation, identity-stable selection and changed-row-only rendering. The detail card/radar remains at 4 Hz. Physical TFT acceptance of these corrections is still unclaimed. The Workbench preserves Export as a separate Library action, uses a bounded 1 KiB workspace and has no TX/replay/output API. When board-01 is connected again, the affected delta gate must first visually verify the Bluetooth list, Protocol Workbench cursor and truthful Lab empty state, then perform one byte-identical on-device screenshot → protected Library → USB export workflow and reopen the same generation after a cold boot. Calibrated distance and physical Workbench acceptance are not claimed. Automation/HID stays parked at the accepted safe dev.308 public-trust checkpoint with execution disconnected; positive Serial Console traffic still waits for an explicitly reviewed no-RF `mux56-3v3` fixture, while optional GPS, physical HTTP parity and the deferred S5 RF carrier gate remain externally blocked.
