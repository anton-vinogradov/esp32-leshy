...
- **Verified checkpoint:** exact `0.153.0-targets-tags-edit` at source `042b120e85766d6d3762a7d07f8bf3c183f83cc5` is physically accepted in `E-AUTO-111`/`E-HIL-171`/`E-UX-048`. Target Detail→Actions→Tags exposes a bounded add/remove workflow, adds ASCII tag `A`, atomically publishes catalog-only schema-v3 generation 3→4 and cold-reopens it, then removes it, publishes generation 4→5 and cold-reopens the empty list. Both mutations re-identify the exact CID and perform 3 writes/3 file syncs/3 directory syncs. UI acknowledgement is 158/141 µs, supervised workers complete in 2.915/2.919 s, the 16 KiB pre-mount workspace fits a 34,804 B largest block, heap releases from 85,072 B to 96,852 B and final state is Home/none/lease 0. The deferred S5 RF-positive gate remains open.
- **Next gate:** add bounded notes using the same exact-CID atomic path and stable Target selection. Correlation accept/reject follows; merge/split remains behind an explicit confirmation that previews ownership change and preserves reversal. The physical S5 gate remains postponed, not waived, until the replacement DIV arrives and passes its read-only profile.
