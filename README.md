...
- **Verified checkpoint:** exact `0.152.0-targets-name-edit` at source `7c9e358493afcf0d4fbaf8b27d567745a7138e0e` is physically accepted in `E-AUTO-110`/`E-HIL-170`/`E-UX-047`. Target Detail→Actions→Name exposes a bounded four-row keypad/touch editor, appends `A`, re-identifies the exact CID and atomically publishes catalog-only schema-v3 generation 2→3 with 3 writes/3 file syncs/3 directory syncs. A physical cold reset reopens the same 128-bit Target with UTF-8 bytes `41`; UI acknowledgement is 155 µs, the supervised worker completes in 2.825 s, the 16 KiB pre-mount workspace fits a 34,804 B largest block, heap releases from 85,072 B to 96,852 B and final state is Home/none/lease 0. The deferred S5 RF-positive gate remains open.
- **Next gate:** add bounded tags, then notes, using the same exact-CID atomic path and stable Target selection. Correlation accept/reject follows; merge/split remains behind an explicit confirmation that previews ownership change and preserves reversal. The physical S5 gate remains postponed, not waived, until the replacement DIV arrives and passes its read-only profile.
