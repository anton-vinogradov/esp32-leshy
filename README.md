...
- **Verified checkpoint:** exact `0.150.0-targets-evidence` at source `6ef57155b48be47fecfb6c8bb543886553009832` is physically accepted in `E-AUTO-108`/`E-HIL-168`/`E-UX-045`. One exact flash reuses exact-CID generations 113/114 read-only and exposes all 16 comparison rows as a stable class-first list (2 added, then 14 unchanged), with descending representative signal inside each class. Every row carries an exact baseline/current Observation coordinate; Right/OK opens the selected evidence and Left returns to the same row, then to the Target list. Four TFT frames, stack frames 256/416/272/480/32/80/1,104 B, unchanged 97,488 B heap, zero writes/TX/drops and final Home/none/lease 0 are retained machine-checked. The first 0.150 candidate's stale edge glyph is retained as a rejected visual precursor; the fixed frame is clean. The deferred S5 RF-positive gate remains open.
- **Next gate:** finish the remaining S6.4 mutation/decision depth without hiding immutable source evidence: expose bounded favorite/name/tags/notes editing and correlation accept/reject, then merge/split only behind an explicit confirmation that previews the ownership change and preserves reversal. Implement and host-review one complete interaction slice before spending the next delta HIL. The physical S5 gate remains postponed, not waived, until the replacement DIV arrives and passes its read-only profile.
