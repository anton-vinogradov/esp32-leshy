...
- **Next gate:** implement offline companion search and a versioned, deterministic export over the already accepted native-USB contract, with bounded pagination and no network or direct storage path. Physical HTTP parity is deferred until a dedicated idle network adapter or external client is available; the active Wi-Fi of this Mac is excluded. The physical S5 gate remains postponed, not waived, until the replacement DIV arrives and passes its read-only profile.
