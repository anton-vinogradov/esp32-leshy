...
- **Verified checkpoint:** `E-BUILD-192`/`E-AUTO-167`/`E-HIL-205`/`E-UX-062`/`RB-M203` physically complete CAP-051 BLE Inspector in exact `1.0.0-dev.276` at source `6b5d27b2253bd7b335bab8754379a5ce51a0a5d2`. One fresh flash proves timeout cleanup plus positive recovery; one exact-image reuse then proves wrong peer, timeout, resource conflict and visible failed disconnect all fail closed and return Home/none/lease 0. A final connection still enumerates 5 services/7 characteristics with zero pair/read/write/subscribe operations. The 32-report passive callback queue reaches high-water 3 with zero drops and restores 1,304 B static RAM so post-failure GATT initializes with 1,540 B free and a 1,480 B minimum.
- **Next gate:** resume the active S6.5/S6.6 path by proving Product Survey terminal commit and cold exact-CID reopen without weakening the accepted companion or CAP-051 boundaries. CAP-050 located/timed export still needs a separately owned non-conflicting GPS profile; physical HTTP parity still needs a dedicated client that cannot touch active Mac Wi-Fi or the Cardputer.
