...
- **Verified checkpoint:** Dev.390: 13 TFT captures; no-name window 20.001 s, real AP-frame name without changing signal observations, touch Stop at 1.056 s, fresh scan samples 3→4 and 4→5 after restoration. Countdown: 26 dynamic / zero static pixels. Source AP deadline: 60.314 s. Both Home/none/lease 0, safety armed, no scan-queue drops or SD operations. Before cleanup, the separate 64-record history retained 64/747 observations (683 capacity drops); this is not lossless recording or a full-matrix/optical acceptance.
- **Next gate:** Progress **24/62**, Wi-Fi **17/21 done, 4/21 remaining**. The selected-channel decoder/card integration is installed; next prove a name from a real client's association while the AP remains hidden. A third owned client is needed with the current two-DIV AP/receiver arrangement. USB diagnostic loss remains an open reliability issue: fragment handling and misleading timed-capture retries were fixed, but a later pass does not establish the earlier timeout's root cause. Laptop Wi-Fi is untouched.
...
| FF-5 | Deliver `FUNC-34` IR replay from one selected immutable Capture with preview, confirmation and proven Stop/timeout; host/build foundation is accepted on dev.373; replacement board-03 has arrived, optical IR qualification and exact two-board admission remain open | 🟡 active |
