...
- **Next gate:** bind the host/build-accepted local Web presentation to an explicit user-authorized local connectivity lifecycle without ambient access, persistent cleartext secrets or broader grants, then physically prove USB/Web parity on the original DIV. The physical S5 gate remains postponed, not waived, until the replacement DIV arrives and passes its read-only profile.
