...
- **Verified checkpoint:** `E-BUILD-188`/`E-AUTO-163`/`E-HIL-202`/`RB-M199` accept the optional trusted-context software slice of Field Survey on original board-01 with exact `1.0.0-dev.267` at source `eb23d614785420a10588a4ae5a8d3e351021702b`. Schema 9 persists one immutable, checksummed GPS-NMEA UTC/location record, keeps schemas 1…8 readable and rejects stale, malformed or impossible fixes. The stock no-GPS path cold-recovers generation 175/51 read-only, exports 51 native and 49 representable WiGLE rows, remains truthfully `trusted_source=none`, `untimed_unlocated`, `upload_ready=false`, and exits Home/none/lease 0 with zero scans or writes.
- **Next gate:** start CAP-051 BLE Inspector with bounded compatible raw-advertisement retention and an explicit permissioned connected-GATT state machine. CAP-050 implementation is software-complete for trusted context, but physical located/timed export remains explicitly deferred until a separately owned non-conflicting GPS profile exists. Physical HTTP parity remains deferred to a dedicated client that cannot touch active Mac Wi-Fi.
