...
- **Verified checkpoint:** physical `0.146.0-targets` remains rejected by the loop-task stack canary (`E-HIL-164`). Physical `0.147.0-targets-stack-safe` proves that correction—two real passive Wi-Fi visits committed generations 111/112 with 32 observations, zero drops and no reset/canary—but then failed closed while mounting Sessions read-only after the 22,544 B Targets workspace had already consumed heap. `E-HIL-165` preserves that second failure, its complete safe cleanup and the runner's declared/actual source mismatch instead of treating it as acceptance. Candidate `0.148.0-targets-storage-order` now identifies and recovers the exact-CID Session pair, closes FAT/SPI, and only then allocates the foreground workspace; it also exposes the exact mount error and requires a clean exact HEAD before physical work. Full host/contracts and production build pass at 211,296 B static RAM and 3,107,844 B linked flash. Exact `0.145.0-interface-settings` remains the latest physically accepted board-01 baseline; the deferred S5 RF-positive gate remains open.
- **Next gate:** commit-bind exact `0.148.0-targets-storage-order` and run one short board-01 regression against existing generations 111/112: one flash, exact CID, read-only mount error 0, List→Compare, zero writes/TX/drops, released heap and final Home/lease 0. Only if it passes, reuse those exact flashed bytes without reflashing for the full delta: create two new passive Wi-Fi visits and capture Targets List→Compare→Detail with every generation, cleanup and safety invariant. The physical S5 gate is postponed, not waived: when the replacement DIV arrives, its read-only profile must pass before the retained IR→nRF24→OOK→FSK matrix can close S5.3/S5.4/S5.6.
