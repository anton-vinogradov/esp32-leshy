...
- **Verified checkpoint:** `E-BUILD-176`/`E-AUTO-150`/`E-HIL-192`/`E-UX-056`/`RB-M187` accept exact physical `1.0.0-dev.248` at source `e62599c1827e249845105405797cb75aedaa5226` on the original DIV and exact CID `FE343253440000002000000055019CB7`. A fresh-flash top-level smoke opens all eight Home entries, holds Bluetooth for 15 seconds and returns every path to Home/none/lease 0. The direct passive BLE regression completes two lifecycles with 32 unique devices, strongest-first rows, offline company facts, identity-stable live radar, zero driver drops, exact bounded retention accounting and invariant heap. Fresh-flash CAP-049 then completes real ambient RX, automatic `Result → Actions → Peer → Evidence → Detail` navigation, Repeat, Up/Down, Right/OK, Left/Back, synthetic golden evidence and replay rejection with zero Mac Wi-Fi control, connect, raw TX, manual input or leaked lease. Ambient traffic contained no EAPOL/PMKID, so the truthful result remains `inconclusive`; this does not claim useful authentication evidence or persistence/export. Retained CAP bundle, acceptance pins and entry/BLE pins under `tests/hil/evidence/` bind the exact image and runner hashes.
- **Next gate:** implement the standard authentication artifact serializer, wire schema-8 provenance into the existing atomic product persistence/export path, and exercise it with useful bounded EAPOL/PMKID evidence. Product Survey Stop/commit/cold-recovery remains separately pending after its earlier `running_degraded` attempt failed closed before commit. Physical HTTP parity remains deferred to a dedicated client that cannot touch active Mac Wi-Fi.
