...
- **Verified checkpoint:** `E-BUILD-236`/`E-AUTO-214`/`E-HIL-232`/`E-UX-087`/`RB-M248` accept the exact physical `1.0.0-dev.364` plain-language Wi-Fi Security Advisor. One fresh flash proves strongest-first live sorting, user-owned identity-stable focus, bounded signal-only detail repaint, an idle-pixel-stable advisor, two heap-stable Wi-Fi lifecycles and final Home/none/lease 0. The reviewed Russian screen truthfully says that PMF was not measured and fits every action on one line. Exact dev.357 remains the periodic full anchor.
- **Next gate:** progress remains **23/62**, while the fixed Wi-Fi track is now **11/21 done, 10/21 remaining**. `WF-12` is physically accepted with privacy-minimal retained evidence; ambient SSID/BSSID/vendor and raw screenshots are not retained. The next focused delta is `WF-13`: named Airspace Guard sensitivity profiles with plain-language consequences and no weakening of the existing receive-only, evidence-first, fail-closed policy. The companion verification chain `WF-11` remains active in parallel; second-board and dedicated-client physical gates stay parked, and RF TX remains forbidden.
