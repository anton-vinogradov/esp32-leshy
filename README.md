...
- **Next gate:** Progress **24/62**, Wi-Fi **17/21 done, 4/21 remaining**. Physical reconnection of original board-01 is needed: its dev.386 console and bootloader stopped responding after SD retry output; USB reset did not restore them. Dev.387 was not written (connection failed before flash). Board-03 remains unchanged on dev.385. Recover and verify ordinary boot first, then use the same product image on both DIVs for the user-menu AP-name test. Root cause is not yet confirmed; neither two-DIV acceptance nor live client-frame name enrichment is claimed. Laptop Wi-Fi is not a fixture.
