...
- **Verified checkpoint:** `E-BUILD-180`/`E-AUTO-154`/`E-HIL-195`/`E-UX-059`/`RB-M191` physically accept exact `1.0.0-dev.252` on original board-01 at source `30530812efe045aadd112d8b1b0961a48a48b89b`. The card treats an unchanged BLE catalog revision as a completed incremental no-op instead of falling through to a full-screen render, atomically replaces enriched metadata, and coalesces detail updates at 250 ms/4 Hz. Across two complete scan cycles and 2,633 ms it performs 6 bounded refreshes, defers 19 redundant ones, finishes with no pending refresh, zero content clears/full radar repaints and zero allocation/direct-fallback failures; 904 radar pixels change while static/chrome pixels stay zero. The immediately required `E-AUTO-155`/`E-HIL-196`/`E-CADENCE-002` full checkpoint reuses that exact image without another flash and passes all eight Home sections, 20 TFT states, both 224-row RX waterfalls, read-only Targets 165/166 and USB companion with unchanged generation/heap, zero writes/TX/Mac-Wi-Fi access and final Home/none/lease 0.
- **Next gate:** reuse the exact `1.0.0-dev.252` image already on the original DIV and prove explicit Save, atomic SD commit, exact-generation cold reopen and PCAP export; `.hc22000` readiness must additionally be demonstrated by useful bounded PMKID or replay-consistent M1→M2 evidence, while an ambient no-evidence run must remain honest and non-exportable. Product Survey Stop/commit/cold-recovery remains separately pending after its earlier `running_degraded` attempt failed closed before commit. Physical HTTP parity remains deferred to a dedicated client that cannot touch active Mac Wi-Fi.
...
| **Passive audit** | **Wi-Fi authentication Capture (CF-002):** distinguishes EAPOL/PMKID and complete/incomplete handshakes, preserves a focused Capture, and exports PCAP/`hc22000` | 🟡 active: physical dev.248 accepts live receive/navigation; host/build dev.249 implements explicit atomic Save, exact-generation reopen, PCAP and useful-evidence-gated `hc22000`; physical persistence/export and useful authentication evidence remain open |
