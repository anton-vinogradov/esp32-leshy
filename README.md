...
- **Verified checkpoint:** `E-BUILD-202`/`E-AUTO-177`/`E-HIL-212`/`E-UX-067`/`RB-M213` accept exact physical `1.0.0-dev.288` at source `3d232d2e5aff8e914ccb7e7414194c1ce7e89bbf`. The compact read-only Lab → Automation Inspector product route is connected without an Action/HID/resource path, and all nine top-level routes pass automatically. The first-process BLE regression is fixed by compacting only its idle observation queue 64→32 before NimBLE and restoring 32→64 on exit: 59 ready samples over 15 s retain at least 76,768 B free/31,732 B largest before init and 4,992/2,932 B after; 122 advertisements have zero driver drops and queue high-water 3/32. The rejected dev.287 predecessor crosses UI revision 85→0 before `ble_begin` runs. Privacy-minimal `E-HIL-212` retains both boundaries and final Home/none/lease 0.
