...
- **Verified checkpoint:** `E-BUILD-186`/`E-AUTO-161`/`E-HIL-200`/`RB-M197` accept native CSV and truthful local WiGLE 1.6 export from exact retained Field Survey generation 172/52 on original board-01 with exact `1.0.0-dev.263` at source `0f46b4db840bf38e3beac6424623c33fca8e749e`. Native export emits 52 deduplicated records (16 Wi-Fi AP, 0 stations, 36 BLE) with evidence/provenance fields; WiGLE emits the same 52 representable records and explicitly remains `untimed_unlocated`, `upload_ready=false` rather than inventing UTC or coordinates. Recovery is read-only on attempt 1 with zero scans, commits, physical or blocked writes; no ambient identifiers or raw exports are retained, the Export Ready pixels are machine-pinned, and final cleanup is Home/none/lease 0.
- **Next gate:** capture live passive Wi-Fi station observations in Field Survey and prove their native export while keeping WiGLE's unsupported station boundary explicit. Optional trusted GPS/UTC remains the final CAP-050 slice. Physical HTTP parity remains deferred to a dedicated client that cannot touch active Mac Wi-Fi.
