...
- **Verified checkpoint:** exact `0.195.0-companion-web-gzip-index` at firmware source `52570c587b34f73a9d35628b554cbac5b76063f8` is physically accepted for the **offline USB-only** companion boundary in `E-BUILD-152`/`E-AUTO-124`/`E-HIL-182`/`E-COMPANION-006`. Without flashing or invoking network tools, the explicit original-DIV port exports 2 Sessions, 16 Targets with all 16 complete detail projections and 7 comparison items into an 11,521-byte canonical `leshy.companion.offline.v1` snapshot. Two runs produce the same snapshot ID and file SHA-256; local case-folded search covers name, notes, tags and normalized identities. Exact CID/generation 161/59 survive read-only with zero physical writes, radio TX, Cardputer opens, input drops or leaked leases. The retained evidence contains hashes/counts only, not private payload or queries, and records that active Mac Wi-Fi was untouched.
- **Next gate:** fix and USB-only regress the post-Web device-memory lifecycle defect exposed by this gate: after an earlier Local Web lifecycle, reopening Targets can fail its read-only mount with `ESP_ERR_NO_MEM` until the DIV is reset. Reproduction and repair may start/stop the DIV's own AP through serial diagnostics, but will not associate, reconfigure or disable any Mac network interface. Physical HTTP parity remains deferred to a dedicated idle adapter or external client. The physical S5 gate remains postponed, not waived, until the replacement DIV arrives and passes its read-only profile.
