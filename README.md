...
- **Verified checkpoint:** exact `0.149.0-targets-inplace-reset` at source `8d4a2e86d88807f0de21d5afd9680912d8b7797f` is physically accepted in `E-AUTO-107`/`E-HIL-167`/`E-UX-044`. One exact flash first reopens existing generations 111/112 read-only and proves List→Compare with 16 Targets, mount error 0, zero writes/TX/drops and full heap release. The same flashed bytes then create two real passive Wi-Fi visits as generations 113/114 with 21/18 observations and open List→Compare→Detail; all 16 rows classify as 2 added + 14 unchanged. Five TFT frames, exact CID, ELF stack frames 256/416/32/80/1,104 B, safe outputs and final Home/none/lease 0 are retained by the machine-checked bundle. The three precursor failures 0.146/0.147/0.148 remain visible as `E-HIL-164…166`; the deferred S5 RF-positive gate remains open.
- **Next gate:** complete the remaining S6.4 product depth: Compare becomes a stable class-sorted row list, every row opens the exact baseline/current Observation and explains its change mask, then bounded metadata/correlation decisions use the existing typed Actions without hiding source evidence. First implement and host-review the row/evidence interaction; spend one delta HIL only after the complete 0.150 slice is ready. The physical S5 gate remains postponed, not waived, until the replacement DIV arrives and passes its read-only profile.
