...
void paintLayeredBarDelta(const BarLayers& old, const BarLayers& next,
    ...
    // The validated widths keep these differences within int16_t.
    const std::int16_t inset =
        static_cast<std::int16_t>((width - currentWidth) / 2);
    const std::int16_t edges[] = {0, inset,
        static_cast<std::int16_t>(inset + currentWidth), width};
    for (unsigned strip = 0; strip < 3; ++strip) {
        const std::int16_t span =
            static_cast<std::int16_t>(edges[strip + 1U] - edges[strip]);
        if (span == 0) continue;
        std::int16_t run = -1;
        std::uint16_t runColor = 0;
        // One terminal sentinel row; a 16-bit iterator would wrap at INT16_MAX.
        for (std::int32_t y = 0; y <= height; ++y) {
            ...
            if (y < height) {
                const auto pixel = [&](const BarLayers& bar) {
                    ...
                    return static_cast<std::uint16_t>(
                        background(static_cast<std::int16_t>(y)));
                };
                color = pixel(next);
            ...
            if (changed && run < 0) {
                run = static_cast<std::int16_t>(y); runColor = color;
            }
        }
    }
}
