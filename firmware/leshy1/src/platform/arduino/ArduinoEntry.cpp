...
extern const std::uint8_t bleCompanyAssetStart[]
    asm("_binary_assets_bluetooth_companies_bin_start");
extern const std::uint8_t bleCompanyAssetEnd[]
    asm("_binary_assets_bluetooth_companies_bin_end");
...
using leshy1::apps::spectrum::Cc1101SignalFinder;
using leshy1::apps::spectrum::Cc1101SignalFinderState;
...
using leshy1::apps::ble::BleDeviceNavigationOrder;
using leshy1::apps::ble::BleDeviceSignalStats;
...
Cc1101SignalFinder cc1101SignalFinder;
...
const WifiNetworkSignalStats* liveWifiNetworkSignal() {
    const std::size_t index =
        wifiNetworkCatalog.indexOfIdentity(wifiNetworkDetail);
    const WifiNetworkSignalStats* live = wifiNetworkCatalog.signalAt(index);
    return live == nullptr ? &wifiNetworkDetailSignal : live;
}
...
const Observation* bleDeviceAt(std::size_t index) {
    return bleDeviceNavigationOrder.at(bleDeviceCatalog, index);
}

const Observation* liveBleDeviceDetail() {
    const std::size_t index =
        bleDeviceCatalog.indexOfIdentity(bleDeviceDetail);
    const Observation* live = bleDeviceCatalog.at(index);
    return live == nullptr ? &bleDeviceDetail : live;
}

const BleDeviceSignalStats* liveBleDeviceSignal() {
    const std::size_t index =
        bleDeviceCatalog.indexOfIdentity(bleDeviceDetail);
    const BleDeviceSignalStats* live = bleDeviceCatalog.signalAt(index);
    return live == nullptr ? &bleDeviceDetailSignal : live;
}
...
std::array<std::int16_t, Layout::ScreenWidth> ccFinderRenderedHeights{};
std::uint32_t ccFinderRenderedRevision = UINT32_MAX;
Cc1101SignalFinderState ccFinderRenderedState =
    Cc1101SignalFinderState::Idle;
...
void setProductSurveyWorkerDeadlineInjection(bool armed) {
    portENTER_CRITICAL(&productSurveyWorkerMux);
    productSurveyWorkerDeadlineInjectionOnce = armed;
    portEXIT_CRITICAL(&productSurveyWorkerMux);
}

bool productSurveyWorkerDeadlineInjectionArmed() {
    portENTER_CRITICAL(&productSurveyWorkerMux);
    const bool armed = productSurveyWorkerDeadlineInjectionOnce;
    portEXIT_CRITICAL(&productSurveyWorkerMux);
    return armed;
}

bool consumeProductSurveyWorkerDeadlineInjection() {
    portENTER_CRITICAL(&productSurveyWorkerMux);
    const bool injected = productSurveyWorkerDeadlineInjectionOnce;
    productSurveyWorkerDeadlineInjectionOnce = false;
    portEXIT_CRITICAL(&productSurveyWorkerMux);
    return injected;
}
...
bool armProductSurveyWorkerDeadline(std::uint64_t nowUs) {
    portENTER_CRITICAL(&productSurveyWorkerMux);
    const bool armed = workerDeadlineSupervisor.arm(
        SupervisedWorker::ProductSurvey, nowUs,
        kProductSurveyWorkerDeadlineUs);
    portEXIT_CRITICAL(&productSurveyWorkerMux);
    return armed;
}

bool heartbeatProductSurveyWorker(std::uint64_t nowUs = 0) {
    if (nowUs == 0) {
        nowUs = static_cast<std::uint64_t>(esp_timer_get_time());
        if (nowUs == 0) nowUs = 1;
    }
...
bool disarmProductSurveyWorkerDeadline() {
    portENTER_CRITICAL(&productSurveyWorkerMux);
    const bool disarmed = workerDeadlineSupervisor.disarm(
...
WorkerDeadlineSnapshot workerDeadlineSnapshot() {
    portENTER_CRITICAL(&productSurveyWorkerMux);
    const WorkerDeadlineSnapshot snapshot = workerDeadlineSupervisor.snapshot();
    portEXIT_CRITICAL(&productSurveyWorkerMux);
    return snapshot;
}
...
void runProductSurveyWorker(void*) {
    for (;;) {
        ...
        if (consumeProductSurveyWorkerDeadlineInjection()) {
            vTaskDelay(pdMS_TO_TICKS(
                kProductSurveyWorkerDeadlineInjectionMs));
        }
...
void serviceProductSurveyWorker() {
    ...
    // Detail views keep receiving passive samples. renderSelectionDelta()
    // confines routine changes to their radar cards and repaints static facts
    // only when a later advertisement actually enriches them.
    if (render) {
...
void serviceWorkerDeadlineSupervisor() {
    if (safetySupervisor.latched()) return;
...
void emitSafetyState(Stream& reply) {
    ...
    std::snprintf(
        ...
        "\"worker_supervision\":true,\"worker_active\":\"%s\","
        "\"worker_last_expired\":\"%s\",\"worker_armed\":%s,"
        ...
        leshy1::kernel::safety::supervisedWorkerName(worker.activeWorker),
        leshy1::kernel::safety::supervisedWorkerName(
            worker.lastExpiredWorker),
        worker.armed ? "true" : "false",
        worker.expired ? "true" : "false",
        static_cast<unsigned long long>(worker.deadlineUs / 1000ULL),
...
void triggerWorkerDeadlineTest(Stream& reply) {
    ...
    setProductSurveyWorkerDeadlineInjection(true);
    reply.println(
        "{\"schema\":\"leshy.safety.worker_deadline_test.v1\","
...
NavigationFooter navigationFooterForCurrentState() {
    ...
    if (uiController.page() == 2) {
        ...
        if (rfSpectrumView == RfSpectrumView::CcFinder) {
            return {back, {},
                    {NavigationKey::RightAndSelect, UiTextId::NavAgain}};
        }
...
UiTextId safetyReasonTextId() {
    switch (safetySupervisor.reason()) {
        ...
        case SafetyReason::WorkerDeadline:
            return UiTextId::SafetyWorkerReason;
        default:
            return UiTextId::SafetyOutputReason;
    }
}
...
UiTextId wifiNetworkTrendText(std::int16_t trendDb) {
    if (trendDb >= 4) return UiTextId::WifiNetworkTrendStronger;
    if (trendDb <= -4) return UiTextId::WifiNetworkTrendWeaker;
    return UiTextId::WifiNetworkTrendStable;
}
...
void renderWifiNetworkRadar(const Observation& network,
    ...
    constexpr std::int16_t kTrackY = 258;
    constexpr std::int16_t kTrackHeight = 12;
    const std::int16_t trackWidth = kRadar.width - 2 * kInset;
    const std::uint8_t level = wifiSignalLevel(network.rssiDbm);
    const std::uint16_t tone = level >= 3U
        ? Palette::Positive : (level == 2U ? Palette::Warning
                                           : Palette::Danger);
    ...
    display.setTextColor(tone, Palette::Surface);
    const std::int16_t valueX = kRadar.x + kRadar.width - kInset -
    ...
    display.fillRect(kRadar.x + kInset, kTrackY, trackWidth,
                     kTrackHeight, Palette::Canvas);
    display.drawRect(kRadar.x + kInset, kTrackY, trackWidth,
                     kTrackHeight, Palette::Divider);
    const std::int16_t clamped = network.rssiDbm < -100
        ? -100 : (network.rssiDbm > -40 ? -40 : network.rssiDbm);
    const std::int16_t fillWidth = static_cast<std::int16_t>(
    ...
    display.setTextColor(Palette::TextMuted, Palette::Surface);
    setUiCursor(UiTextRole::Meta, kRadar.x + kInset, 274);
    display.print(line);
    const char* trend = tr(wifiNetworkTrendText(signal.rssiTrendDb));
    const std::int16_t trendX = kRadar.x + kRadar.width - kInset -
                                display.textWidth(trend);
    const std::uint16_t trendTone = signal.rssiTrendDb >= 4
        ? Palette::Positive : (signal.rssiTrendDb <= -4
            ? Palette::Danger : Palette::TextMuted);
    display.setTextColor(trendTone, Palette::Surface);
    setUiCursor(UiTextRole::Meta, trendX, 274);
    display.print(trend);
}
...
void renderWifiNetworkDetailData() {
    ...
    std::snprintf(line, sizeof(line),
                  tr(UiTextId::WifiNetworkPhyFeaturesFormat), phy,
                  facts.wps ? "+" : "--", ftm, antenna);
    display.setTextColor(Palette::TextSecondary, Palette::Canvas);
    setUiCursor(UiTextRole::Meta, 14, 171);
    display.print(line);
    ...
    if (facts.bssColorKnown || facts.vhtCenterChannel1 != 0U ||
        ...
        display.setTextColor(Palette::TextMuted, Palette::Canvas);
        const std::int16_t advancedX = Layout::ScreenWidth - Layout::Edge -
                                       display.textWidth(line);
        setUiCursor(UiTextRole::Meta, advancedX, 207);
        display.print(line);
    }
    renderWifiNetworkRadar(network, signal);
...
UiTextId bleDeviceKindText(BleDeviceKind kind) {
    switch (kind) {
        case BleDeviceKind::Phone: return UiTextId::BleKindPhone;
        case BleDeviceKind::Computer: return UiTextId::BleKindComputer;
        case BleDeviceKind::Watch: return UiTextId::BleKindWatch;
        case BleDeviceKind::Audio: return UiTextId::BleKindAudio;
        case BleDeviceKind::Keyboard: return UiTextId::BleKindKeyboard;
        case BleDeviceKind::Mouse: return UiTextId::BleKindMouse;
        case BleDeviceKind::Input: return UiTextId::BleKindInput;
        case BleDeviceKind::Heart: return UiTextId::BleKindHeart;
        case BleDeviceKind::Thermometer: return UiTextId::BleKindThermometer;
        case BleDeviceKind::Tag: return UiTextId::BleKindTag;
        case BleDeviceKind::Display: return UiTextId::BleKindDisplay;
        case BleDeviceKind::Fitness: return UiTextId::BleKindFitness;
        case BleDeviceKind::Unknown:
        default: return UiTextId::BleKindUnknown;
    }
}

UiTextId bleAddressTypeText(std::uint8_t type) {
    switch (type) {
        case 0U: return UiTextId::BleAddressPublic;
        case 2U: return UiTextId::BleAddressPublicIdentity;
        case 3U: return UiTextId::BleAddressRandomIdentity;
        case 1U:
        default: return UiTextId::BleAddressRandom;
    }
}

UiTextId bleAdvertisementModeText(
        const leshy1::domain::observations::BleAdvertisementFacts& facts) {
    if (facts.connectable) return UiTextId::BleAdvertisementConnectable;
    if (facts.scannable) return UiTextId::BleAdvertisementScannable;
    return UiTextId::BleAdvertisementBroadcast;
}

bool bleDeviceVendor(const Observation& device, char* output,
                     std::size_t capacity) {
    if (output == nullptr || capacity == 0U) return false;
    output[0] = '\0';
    const auto& facts = device.bleAdvertisement;
    if (facts.companyKnown &&
        bleCompanyDatabase.lookup(facts.companyId, output, capacity)) {
        return true;
    }
    if ((facts.knownServiceMask &
         leshy1::domain::observations::BleAdvertisementFacts::kServiceXiaomi)
        != 0U) {
        std::snprintf(output, capacity, "Xiaomi");
        return true;
    }
    return false;
}

void compactBlePrimary(const Observation& device, char* output,
                       std::size_t capacity) {
    if (output == nullptr || capacity == 0U) return;
    output[0] = '\0';
    const char* source = nullptr;
    if (device.labelLength != 0U) {
        source = device.label.data();
    } else {
        const char* subtype = leshy1::apps::ble::bleSubtypeName(
            leshy1::apps::ble::classifyBleSubtype(device));
        if (subtype[0] != '\0') source = subtype;
    }
    char vendor[BleCompanyDatabase::kNameSize + 1U] = {};
    if (source == nullptr && bleDeviceVendor(device, vendor, sizeof(vendor))) {
        source = vendor;
    }
    if (source == nullptr) {
        source = tr(bleDeviceKindText(
            leshy1::apps::ble::classifyBleDevice(device)));
    }
    std::snprintf(output, capacity, "%s", source);
    if (std::strlen(source) >= capacity && capacity > 2U) {
        output[capacity - 2U] = '~';
    }
}

const char* bleDeviceDescriptor(const Observation& device, char* vendor,
                                std::size_t vendorCapacity) {
    const BleSubtype subtype =
        leshy1::apps::ble::classifyBleSubtype(device);
    const char* subtypeName = leshy1::apps::ble::bleSubtypeName(subtype);
    if (subtypeName[0] != '\0') return subtypeName;
    const BleDeviceKind kind =
        leshy1::apps::ble::classifyBleDevice(device);
    if (kind != BleDeviceKind::Unknown) return tr(bleDeviceKindText(kind));
    if (bleDeviceVendor(device, vendor, vendorCapacity)) return vendor;
    return tr(UiTextId::BleKindUnknown);
}

const char* compactBleService(const Observation& device, char* output,
                              std::size_t capacity) {
    if (output == nullptr || capacity == 0U) return "";
    output[0] = '\0';
    const auto& facts = device.bleAdvertisement;
    const char* friendly =
        leshy1::apps::ble::bleServiceName(facts.knownServiceMask);
    if (friendly[0] != '\0') {
        std::snprintf(output, capacity, "%s", friendly);
        return output;
    }
    if (facts.firstServiceUuidLength == 0U) return output;
    std::snprintf(output, capacity, "%s", facts.firstServiceUuid.data());
    return output;
}
...
bool bleDeviceDetailStaticFieldsDiffer(const Observation& left,
    ...
    return left.identity != right.identity ||
        left.labelLength != right.labelLength ||
        std::memcmp(left.label.data(), right.label.data(), left.labelLength) != 0 ||
        !leshy1::domain::observations::bleAdvertisementFactsEqual(
...
void renderBleDeviceDetailData() {
    ...
    const char* descriptor =
        bleDeviceDescriptor(device, vendor, sizeof(vendor));
    if (tracker != BleTrackerKind::None) {
        std::snprintf(line, sizeof(line), tr(UiTextId::BleTrackerFormat),
                      descriptor);
    } else {
    ...
    if (vendorKnown) {
        std::snprintf(line, sizeof(line), tr(UiTextId::BleVendorFormat), vendor);
    } else if (facts.companyKnown) {
        std::snprintf(line, sizeof(line), tr(UiTextId::BleCompanyFormat),
                      static_cast<unsigned>(facts.companyId));
    } else {
        std::snprintf(line, sizeof(line), "%s",
    ...
    char serviceText[24] = {};
    const char* service =
        compactBleService(device, serviceText, sizeof(serviceText));
    if (service[0] != '\0') {
        std::snprintf(line, sizeof(line), tr(UiTextId::BleServiceFormat),
                      service);
    } else {
        std::snprintf(line, sizeof(line), "%s",
    ...
    if (facts.txPowerKnown && facts.appearanceKnown) {
        std::snprintf(line, sizeof(line),
                      ...
                      static_cast<int>(facts.txPowerDbm),
                      static_cast<unsigned>(facts.appearance));
    } else if (facts.txPowerKnown) {
        std::snprintf(line, sizeof(line), tr(UiTextId::BleTxFormat),
                      static_cast<int>(facts.txPowerDbm));
    } else if (facts.appearanceKnown) {
        std::snprintf(line, sizeof(line), tr(UiTextId::BleAppearanceFormat),
                      static_cast<unsigned>(facts.appearance));
    } else {
...
void renderSubGhzModeRow(std::uint8_t index, bool selected) {
    if (index >= 3U) return;
    const UiTextId label = index == 0U
        ? UiTextId::SubGhzSpectrum
        : (index == 1U ? UiTextId::CcFinder : UiTextId::SubGhzRaw);
    const UiTextId note = index == 0U
        ? UiTextId::SubGhzSpectrumNote
        : (index == 1U ? UiTextId::CcFinderNote : UiTextId::SubGhzRawNote);
    renderMenuRow(Components::choiceRow(index), tr(label), tr(note),
                  selected, true, Tone::Positive);
}
...
void renderSubGhzModeMenu(bool clearContent) {
    ...
    for (std::uint8_t index = 0; index < 3U; ++index) {
        renderSubGhzModeRow(index, subGhzModeSelection == index);
    }
}
...
void renderCcFinderResult(bool force) {
    const Cc1101SignalFinderState state = cc1101SignalFinder.state();
    const bool found = cc1101SignalFinder.found();
    const std::uint32_t frequency = cc1101SignalFinder.strongestFrequencyKHz();
    if (!force && state == ccFinderRenderedState &&
        found == ccFinderRenderedFound &&
        (!found || frequency == ccFinderRenderedFrequencyKHz)) {
        return;
    }
    display.fillRect(0, kNrfFinderInfoY, Layout::ScreenWidth,
                     kNrfFinderInfoHeight, Palette::Canvas);
    if (state == Cc1101SignalFinderState::Fault) {
        display.setTextColor(Palette::Danger, Palette::Canvas);
        setUiCursor(UiTextRole::Body, 8, kNrfFinderInfoY + 12);
        display.print(tr(UiTextId::CcFinderFault));
    } else if (state == Cc1101SignalFinderState::Calibrating) {
        display.setTextColor(Palette::TextPrimary, Palette::Canvas);
        setUiCursor(UiTextRole::Body, 8, kNrfFinderInfoY + 6);
        display.print(tr(UiTextId::CcFinderCalibrating));
        display.setTextColor(Palette::TextSecondary, Palette::Canvas);
        setUiCursor(UiTextRole::Meta, 8, kNrfFinderInfoY + 32);
        display.print(tr(UiTextId::CcFinderCalibratingNote));
    } else if (!found) {
        display.setTextColor(Palette::TextPrimary, Palette::Canvas);
        setUiCursor(UiTextRole::Body, 8, kNrfFinderInfoY + 6);
        display.print(tr(UiTextId::CcFinderWaiting));
        display.setTextColor(Palette::TextSecondary, Palette::Canvas);
        setUiCursor(UiTextRole::Meta, 8, kNrfFinderInfoY + 32);
        display.print(tr(UiTextId::CcFinderWaitingNote));
    } else {
        display.setTextColor(Palette::Positive, Palette::Canvas);
        setUiCursor(UiTextRole::Meta, 8, kNrfFinderInfoY + 1);
        display.print(tr(UiTextId::CcFinderFound));
        char line[32] = {};
        std::snprintf(line, sizeof(line), tr(UiTextId::CcFinderFrequencyFormat),
                      static_cast<unsigned long>(frequency / 1000U),
                      static_cast<unsigned long>(frequency % 1000U));
        display.setTextColor(Palette::TextPrimary, Palette::Canvas);
        setUiCursor(UiTextRole::Body, 8, kNrfFinderInfoY + 19);
        display.print(line);
        const char* hint = cc1101SignalFinder.bandHint();
        if (hint[0] != '\0') {
            selectUiFont(UiTextRole::Meta);
            display.setTextColor(Palette::TextSecondary, Palette::Canvas);
            setUiCursor(UiTextRole::Meta,
                        Layout::ScreenWidth - 8 - display.textWidth(hint),
                        kNrfFinderInfoY + 23);
            display.print(hint);
        }
    }
    ccFinderRenderedState = state;
    ccFinderRenderedFound = found;
    ccFinderRenderedFrequencyKHz = frequency;
}

void renderCcFinderGraph(bool force) {
    if (force) {
        display.fillRect(0, kNrfFinderGraphY, Layout::ScreenWidth,
                         kNrfFinderGraphHeight, kSpectrumNoSignal);
        ccFinderRenderedHeights.fill(0);
    }
    for (std::int16_t x = 0; x < Layout::ScreenWidth; ++x) {
        std::size_t first = static_cast<std::size_t>(x) *
            Cc1101SignalFinder::kBinCount / Layout::ScreenWidth;
        std::size_t last = static_cast<std::size_t>(x + 1) *
            Cc1101SignalFinder::kBinCount / Layout::ScreenWidth;
        if (last <= first) last = first + 1U;
        std::uint8_t strength = 0;
        for (std::size_t bin = first;
             bin < last && bin < Cc1101SignalFinder::kBinCount; ++bin) {
            const std::uint8_t candidate = cc1101SignalFinder.strength(bin);
            if (candidate > strength) strength = candidate;
        }
        const std::int16_t height = static_cast<std::int16_t>(
            (strength < Cc1101SignalFinder::kGraphFullScaleDb
                 ? strength : Cc1101SignalFinder::kGraphFullScaleDb) *
            kNrfFinderGraphHeight /
            Cc1101SignalFinder::kGraphFullScaleDb);
        const std::int16_t previous =
            ccFinderRenderedHeights[static_cast<std::size_t>(x)];
        if (height < previous) {
            display.drawFastVLine(x, kNrfFinderAxisY - previous,
                                  previous - height, kSpectrumNoSignal);
        } else if (height > previous) {
            display.drawFastVLine(
                x, kNrfFinderAxisY - height, height - previous,
                strength >= Cc1101SignalFinder::kDetectionRiseDb
                    ? Palette::Positive : Palette::TextMuted);
        }
        ccFinderRenderedHeights[static_cast<std::size_t>(x)] = height;
    }
    ccFinderRenderedRevision = cc1101SignalFinder.revision();
}

std::int16_t ccFinderAxisX(std::uint32_t frequencyKHz) {
    std::size_t bestBin = 0;
    std::uint32_t bestDistance = UINT32_MAX;
    for (std::size_t bin = 0; bin < Cc1101SignalFinder::kBinCount; ++bin) {
        const std::uint32_t candidate =
            Cc1101SignalFinder::frequencyKHz(bin);
        const std::uint32_t distance = candidate > frequencyKHz
            ? candidate - frequencyKHz : frequencyKHz - candidate;
        if (distance < bestDistance) {
            bestDistance = distance;
            bestBin = bin;
        }
    }
    return static_cast<std::int16_t>(bestBin * Layout::ScreenWidth /
                                     Cc1101SignalFinder::kBinCount);
}

void renderCcFinderAxis() {
    display.fillRect(0, kNrfFinderLegendY, Layout::ScreenWidth,
                     kNrfFinderLegendHeight, Palette::Surface);
    display.setTextColor(Palette::TextSecondary, Palette::Surface);
    setUiCursor(UiTextRole::Meta, 4, kNrfFinderLegendY + 1);
    display.print(tr(UiTextId::CcFinderResponse));

    display.fillRect(0, kNrfFinderAxisY, Layout::ScreenWidth,
                     kNrfFinderAxisHeight, Palette::Canvas);
    struct Tick final { std::uint32_t frequencyKHz; const char* label; };
    constexpr Tick ticks[] = {
        {315000U, "315"}, {433920U, "433"},
        {868350U, "868"}, {915000U, "915"},
    };
    selectUiFont(UiTextRole::Meta);
    display.setTextColor(Palette::TextMuted, Palette::Canvas);
    for (const Tick& tick : ticks) {
        const std::int16_t x = ccFinderAxisX(tick.frequencyKHz);
        display.drawFastVLine(x, kNrfFinderAxisY, 3, Palette::Divider);
        const std::int16_t width = display.textWidth(tick.label);
        setUiCursor(UiTextRole::Meta, x - width / 2,
                    kNrfFinderAxisY + 2);
        display.print(tick.label);
    }
}

void renderCcFinderPage(bool clearContent) {
    renderHeader(tr(UiTextId::CcFinderTitle), clearContent);
    renderCcFinderAxis();
    renderCcFinderResult(true);
    renderCcFinderGraph(true);
}
...
void renderInventoryPage(bool clearContent) {
    ...
    if (rfSpectrumView == RfSpectrumView::CcFinder) {
        renderCcFinderPage(clearContent);
        return;
    }
    if (rfSpectrumView == RfSpectrumView::CcBandMenu) {
...
bool renderSelectionDelta() {
    ...
    if (uiController.page() == 2 &&
        ...
        const bool dataChanged =
            renderedUi.bleDeviceSize != bleDeviceVisibleSize() ||
...
bool startCc1101Finder() {
    cc1101SignalFinder.reset();
    cc1101SpectrumReport = {};
    std::uint64_t startedUs =
        static_cast<std::uint64_t>(esp_timer_get_time());
    ...
    const bool hardwareReady = boardCc1101Spectrum.begin(
        owned, &cc1101SpectrumReport);
    const bool started = hardwareReady &&
        cc1101SignalFinder.start(startedUs);
    if (!started) {
        boardCc1101Spectrum.end();
        cc1101SignalFinder.fail();
    }
    rfSpectrumKind = RfSpectrumKind::Cc1101;
    rfSpectrumView = RfSpectrumView::CcFinder;
    ccFinderRenderedHeights.fill(0);
    ccFinderRenderedRevision = UINT32_MAX;
    ccFinderRenderedState = Cc1101SignalFinderState::Idle;
    ccFinderRenderedFrequencyKHz = UINT32_MAX;
    ccFinderRenderedFound = false;
    lastRuntimeEvent = started ? "cc1101_finder_calibrating"
                               : "cc1101_finder_start_failed";
    return true;
}

bool stopCc1101Finder() {
    ...
    rfSpectrumView = RfSpectrumView::SubGhzMenu;
    nextSpectrumUiRefreshUs = 0;
    lastRuntimeEvent = cleanup ? "cc1101_finder_stopped"
                               : "cc1101_finder_cleanup_failed";
    return true;
}
...
void serviceCc1101Finder() {
    ...
    const std::uint32_t revision = cc1101SignalFinder.revision();
    const std::uint32_t frequency =
        cc1101SignalFinder.nextFrequencyKHz();
    std::int16_t rssiDbm = -128;
    std::uint64_t startedUs = 0;
    std::uint64_t endedUs = 0;
    const bool valid = boardCc1101Spectrum.sampleFrequency(
        frequency, &rssiDbm, &startedUs, &endedUs) &&
        cc1101SignalFinder.ingest(
            frequency, rssiDbm, startedUs, endedUs);
    if (!valid) {
        boardCc1101Spectrum.end();
        cc1101SignalFinder.fail();
        lastRuntimeEvent = "cc1101_finder_runtime_fault";
        renderInteractiveScreen(true);
        return;
    }
    if (cc1101SignalFinder.revision() != revision &&
        !uiController.isRoot() && uiController.page() == 2) {
        display.startWrite();
        renderCcFinderResult(false);
        renderCcFinderGraph(false);
        display.endWrite();
        lastRuntimeEvent = cc1101SignalFinder.found()
            ? "cc1101_finder_signal_found"
            : (cc1101SignalFinder.calibrated()
                   ? "cc1101_finder_waiting"
                   : "cc1101_finder_calibrating");
    }
}
...
bool applyUiAction(UiAction action, bool render = true) {
    ...
    if (!wasRoot && uiController.page() == 2) {
        ...
        if (bleProductView == BleProductView::DeviceDetail) {
            ...
            if (action == UiAction::Up && bleDeviceSelection > 0U) {
                ...
                changed = true;
                       ...
                       bleDeviceSelection + 1U < bleDeviceVisibleSize()) {
                ++bleDeviceSelection;
                changed = true;
            } else if (action == UiAction::Select ||
                       action == UiAction::Right) {
                const Observation* device = bleDeviceAt(bleDeviceSelection);
                if (device != nullptr) {
                    bleDeviceDetail = *device;
                    const std::size_t catalogIndex =
                        bleDeviceCatalog.indexOfIdentity(*device);
                    const BleDeviceSignalStats* signal =
                        bleDeviceCatalog.signalAt(catalogIndex);
            ...
            handled = true;
            if (action == UiAction::Up && subGhzModeSelection != 0U) {
                --subGhzModeSelection;
                changed = true;
            } else if (action == UiAction::Down &&
                       subGhzModeSelection != 2U) {
                ++subGhzModeSelection;
                changed = true;
            } else if (action == UiAction::Select ||
                       action == UiAction::Right) {
        ...
        } else if (rfSpectrumView == RfSpectrumView::CcFinder) {
            handled = true;
            if (action == UiAction::Back || action == UiAction::Left) {
                changed = stopCc1101Finder();
            } else if (action == UiAction::Select ||
                       action == UiAction::Right) {
                std::uint64_t nowUs =
                    static_cast<std::uint64_t>(esp_timer_get_time());
                if (nowUs == 0U) nowUs = 1U;
                changed = cc1101SignalFinder.restart(nowUs);
                if (changed) {
                    ccFinderRenderedRevision = UINT32_MAX;
                    ccFinderRenderedState = Cc1101SignalFinderState::Idle;
                    ccFinderRenderedFrequencyKHz = UINT32_MAX;
                    ccFinderRenderedFound = false;
                    ccFinderRenderedHeights.fill(0);
                    lastRuntimeEvent = "cc1101_finder_calibrating";
                }
            }
            ...
            if (action == UiAction::Up && wifiNetworkSelection > 0) {
                ...
                if (observation != nullptr) {
                    ...
                    const std::size_t catalogIndex =
                        wifiNetworkCatalog.indexOfIdentity(*observation);
                    const WifiNetworkSignalStats* signal =
                        wifiNetworkCatalog.signalAt(catalogIndex);
                    wifiNetworkDetailSignal = signal == nullptr
                        ? WifiNetworkSignalStats{
                            1U, observation->rssiDbm,
                            observation->rssiDbm, 0}
                        : *signal;
                    wifiProductView = WifiProductView::NetworkDetail;
...
void emitCc1101FinderReport(Stream& reply) {
    const auto& report = cc1101SpectrumReport;
    std::uint16_t nonzeroBins = 0;
    for (std::size_t index = 0;
         index < Cc1101SignalFinder::kBinCount; ++index) {
        if (cc1101SignalFinder.strength(index) != 0U) ++nonzeroBins;
    }
    auto& line = diagnosticJson;
    std::snprintf(
        line, sizeof(line),
        "{\"schema\":\"leshy.cc1101.signal-finder.v1\",\"kind\":\"state\","
        "\"view\":\"%s\",\"state\":\"%s\",\"calibrated\":%s,"
        "\"found\":%s,\"frequency_khz\":%lu,\"band_hint\":\"%s\","
        "\"response_db\":%u,\"response_threshold_db\":%u,"
        "\"response_bins\":%u,\"bins\":%u,\"step_khz\":%lu,"
        ...
        "\"wire\":{\"register_reads\":%lu,\"register_writes\":%lu,"
        "\"command_strobes\":%lu,\"reset_strobes\":%lu,"
        "\"receive_strobes\":%lu,\"idle_strobes\":%lu},"
        ...
        "\"current_lease_mask\":%lu}",
        rfSpectrumViewName(rfSpectrumView),
        leshy1::apps::spectrum::cc1101SignalFinderStateName(
            cc1101SignalFinder.state()),
        cc1101SignalFinder.calibrated() ? "true" : "false",
        cc1101SignalFinder.found() ? "true" : "false",
        static_cast<unsigned long>(
            cc1101SignalFinder.found()
                ? cc1101SignalFinder.strongestFrequencyKHz() : 0U),
        cc1101SignalFinder.bandHint(),
        static_cast<unsigned>(cc1101SignalFinder.strongestRiseDb()),
        static_cast<unsigned>(Cc1101SignalFinder::kDetectionRiseDb),
        static_cast<unsigned>(nonzeroBins),
        static_cast<unsigned>(Cc1101SignalFinder::kBinCount),
        static_cast<unsigned long>(Cc1101SignalFinder::kStepKHz),
        static_cast<unsigned long>(cc1101SignalFinder.sweeps()),
        static_cast<unsigned>(cc1101SignalFinder.calibrationPasses()),
        static_cast<unsigned>(cc1101SignalFinder.nextBin()),
        report.rxOnly ? "true" : "false",
...
void emitWifiNetworkDetailState(Stream& reply) {
    ...
    std::snprintf(
        ...
        static_cast<int>(network.rssiDbm),
        static_cast<unsigned>(signal.samples),
        static_cast<int>(signal.minimumRssiDbm),
        static_cast<int>(signal.maximumRssiDbm),
...
void emitBleDeviceDetailState(Stream& reply) {
    const std::size_t catalogIndex =
        bleDeviceCatalog.indexOfIdentity(bleDeviceDetail);
    const bool live = bleProductView == BleProductView::DeviceDetail &&
        catalogIndex < bleDeviceCatalog.size();
    const Observation& device = *liveBleDeviceDetail();
    const BleDeviceSignalStats& signal = *liveBleDeviceSignal();
    const auto& facts = device.bleAdvertisement;
    char vendor[BleCompanyDatabase::kNameSize + 1U] = {};
    ...
    char service[24] = {};
    compactBleService(device, service, sizeof(service));
    std::uint32_t identityHash = 2166136261U;
    for (std::size_t index = 0; index < device.identityLength; ++index) {
        identityHash ^= device.identity[index];
    ...
    const BleTrackerKind tracker =
        leshy1::apps::ble::classifyBleTracker(device);
    const char* trackerName = tracker == BleTrackerKind::FindMy
        ? "find_my" : (tracker == BleTrackerKind::SmartTag
            ? "smart_tag" : (tracker == BleTrackerKind::Tile
                ? "tile" : "none"));
    auto& line = diagnosticJson;
    std::snprintf(
        line, sizeof(line),
        "{\"schema\":\"leshy.ble.device_detail.v1\",\"kind\":\"state\","
        "\"active\":%s,\"passive\":true,\"active_probe_allowed\":false,"
        "\"identity_hash\":%lu,\"label_known\":%s,"
        "\"company_database_available\":%s,"
        "\"company_database_records\":%u,"
        "\"facts_known\":%s,\"vendor_known\":%s,\"vendor\":\"%s\","
        "\"company_known\":%s,\"company_id\":%u,"
        "\"device_kind\":%u,\"subtype\":\"%s\","
        "\"tracker\":\"%s\",\"address_type\":%u,"
        "\"advertisement_type\":%u,\"legacy\":%s,"
        "\"scannable\":%s,\"connectable\":%s,"
        "\"tx_power_known\":%s,\"tx_power_dbm\":%d,"
        "\"appearance_known\":%s,\"appearance\":%u,"
        "\"service\":\"%s\",\"service_uuid_hash\":%lu,"
        "\"known_service_mask\":%u,"
        "\"service_uuid_count\":%u,\"service_data_count\":%u,"
        ...
        bleCompanyDatabase.available() ? "true" : "false",
        static_cast<unsigned>(bleCompanyDatabase.records()),
        facts.present ? "true" : "false",
        vendorKnown ? "true" : "false", vendor,
        facts.companyKnown ? "true" : "false",
        static_cast<unsigned>(facts.companyId),
        static_cast<unsigned>(leshy1::apps::ble::classifyBleDevice(device)),
        leshy1::apps::ble::bleSubtypeName(subtype), trackerName,
        static_cast<unsigned>(facts.addressType),
        static_cast<unsigned>(facts.advertisementType),
        facts.legacy ? "true" : "false",
        facts.scannable ? "true" : "false",
        facts.connectable ? "true" : "false",
        facts.txPowerKnown ? "true" : "false",
        static_cast<int>(facts.txPowerDbm),
        facts.appearanceKnown ? "true" : "false",
        static_cast<unsigned>(facts.appearance), service,
        static_cast<unsigned long>(facts.firstServiceUuidHash),
        static_cast<unsigned>(facts.knownServiceMask),
        static_cast<unsigned>(facts.serviceUuidCount),
        static_cast<unsigned>(facts.serviceDataCount),
        static_cast<unsigned>(facts.manufacturerDataLength),
        static_cast<unsigned>(facts.payloadLength),
        static_cast<int>(device.rssiDbm),
        static_cast<unsigned>(signal.samples),
        static_cast<int>(signal.minimumRssiDbm),
        static_cast<int>(signal.maximumRssiDbm),
...
void handleCommand(Stream& reply, const char* command) {
    ...
    } else if (std::strcmp(command, "hardware.cc1101.finder") == 0) {
