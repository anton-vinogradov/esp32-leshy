...
def main() -> int:
    ...
    require(failures, "kCapacity = 4" in CATALOG_HEADER.read_text(encoding="utf-8") and
            '"self-test", "SELF-TEST"' in catalog and
            catalog.index('"self-test", "SELF-TEST"') > catalog.index('"library", "LIBRARY"'),
            "Self-Test is not the final catalog item")
    require(failures, 'case 4: return "self_test"' in
