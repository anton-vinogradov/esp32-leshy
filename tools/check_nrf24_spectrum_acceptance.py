#!/usr/bin/env python3
"""Fail closed unless the exact 0.82 nRF24 spectrum proof is intact."""
...
EVIDENCE = ROOT / "tests/hil/evidence/board-01-nrf24-spectrum-0.82.json"
BUNDLE = ROOT / "tests/hil/evidence/board-01-nrf24-spectrum-0.82"
CID = "FE343253440000002000000055019CB7"
SOURCE_COMMIT = "9ce989eac9f1a9bddb220af7433304c9432a04ec"
RUNNER_COMMIT = "d1c1903b278a3e5ff2596567a359f11f6ab82ba8"
CAPTURES = {"plan", "source_menu", "running", "paused", "stopped", "home"}
...
def main() -> int:
    ...
    require(failures,
            evidence.get("schema") == "leshy.nrf24_spectrum_acceptance.v1" and
            evidence.get("status") == "pass_receive_only_user_checkpoint" and
            evidence.get("board") == "board-01" and
            candidate.get("version") == "0.82.0-nrf24-spectrum" and
    ...
    require(failures, retained.get("files") == 24 and
            retained.get("tft_states") == 6,
    ...
    runner_blob = git_blob(RUNNER_COMMIT, "tools/run_1x_nrf24_spectrum_hil.py")
    ...
    if run_path.is_file():
        run = load(run_path)
        require(failures,
                run.get("schema") == "leshy.nrf24_spectrum_hil.run.v1" and
        ...
        require(failures,
                ...
                boot.get("heap_free") == after.get("heap_free") ==
                    verified.get("heap_free_after") and
                boot.get("heap_min_free") == after.get("heap_min_free") ==
                    verified.get("heap_min_after"),
        ...
        for label, expected_state, active, cleanup in (
            ...
            sweeps = report.get("sweeps", -1)
            final_writes = 2 if cleanup else 0
            require(failures,
                    report.get("schema") == "leshy.nrf24.spectrum.v1" and
                    ...
                    report.get("modules") == 2 and
                    report.get("range_mhz") == [2402, 2484] and
                    report.get("channels") == 83 and
                    report.get("dwell_us") == 200 and
                    ...
                    }, f"{label} spectrum contract mismatch")
        require(failures,
                reports.get("paused_before", {}).get("sweeps") ==
                    reports.get("paused_after", {}).get("sweeps") == 13 and
                reports.get("running", {}).get("sweeps") == 12 and
                reports.get("resumed", {}).get("sweeps") == 21 and
                reports.get("stopped", {}).get("total_hits") == 99,
    ...
    require(failures,
            ...
            limits.get("cc1101_user_spectrum") == "next_checkpoint" and
            limits.get("demo_s4_closed") is False and
            limits.get("release_promoted") is False,
            "scope limits were weakened")

    if failures:
        print("\n".join(f"FAIL: {item}" for item in failures))
        return 1
    print("PASS: exact 0.82 receive-only nRF24 spectrum evidence is intact")
    return 0
