...
PROTECTED_READ_FIXTURE_SCHEMA = "leshy.device_lock.protected_read_fixture.v1"
...
def protected_read_fixture_command(
        device: PassiveSerial, operation: str) -> dict[str, Any]:
    if operation not in ("begin", "cleanup"):
        raise ValueError("protected read fixture operation must be begin or cleanup")
    return query(
        device,
        f"device-lock.protected-read-fixture {operation}".encode("ascii"),
        PROTECTED_READ_FIXTURE_SCHEMA,
        "protected_read_fixture",
    )


def protected_read_fixture_failures(
        report: dict[str, Any], label: str, *, operation: str,
        status: str, active: bool) -> list[str]:
    expected: dict[str, Any] = {
        "operation": operation,
...
class TemporaryProtectedReadAdmissionHil:
    """RAM-only protected UI and read-only companion admission."""
    ...
    def start(self) -> dict[str, Any]:
        ...
        self.hil_started = True
        self.fixture_started = True
        self.begin_report = protected_read_fixture_command(
            self.device, "begin")
        failures = protected_read_fixture_failures(
            self.begin_report, "fixture_begin", operation="begin",
            status="begun", active=True)
        if failures:
            raise RuntimeError("; ".join(failures))
    ...
    def close(self) -> None:
        failures: list[str] = []
        if self.fixture_started and not self.cleanup_proven:
            self.cleanup_report = protected_read_fixture_command(
                self.device, "cleanup")
            if self.cleanup_report.get("status") == "hil_session_required":
                self.sessions.append(
                    begin_hil(self.device, self.run_id, self.app_identity))
                ...
                self.cleanup_report = protected_read_fixture_command(
                    self.device, "cleanup")
            failures.extend(protected_read_fixture_failures(
                self.cleanup_report, "fixture_cleanup", operation="cleanup",
                status="cleaned", active=False))
            self.cleanup_proven = not failures
        if (not self.fixture_started or self.cleanup_proven) and \
