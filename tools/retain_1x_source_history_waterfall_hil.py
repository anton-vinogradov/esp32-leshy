#!/usr/bin/env python3
"""Canonicalize the exact source-bin waterfall-history HIL run."""
...
def main() -> int:
    ...
    if run.get("passed") is not True or run.get("gate_eligible") is not True or \
        ...
        parser.error("source is not the exact passing source-history run")
    ...
    row_render_max_us = max(
        reports[key].get("timing", {}).get("row_render_max_us", 0)
        for _, key in WATERFALL_REPORTS)
    wires = {
    ...
    result = {
        "schema": "leshy.source_history_waterfall_acceptance.v1",
        "status": "pass_source_history_waterfall_checkpoint",
        "board": "board-01",
        ...
        "verified": {
            ...
            "history_rows": 224,
            "graph_width_px": 240,
            "retained_row_bytes": 83,
            "retained_history_bytes": 18592,
            "previous_history_bytes": 53760,
            "static_ram_saved_bytes": 35168,
            "nrf_source_bins": 83,
            "cc_source_bins": 64,
            "horizontal_interpolation": False,
            "display_expansion": "nearest_source_bin_at_render",
            "row_render_max_us": row_render_max_us,
