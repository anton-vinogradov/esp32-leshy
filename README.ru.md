...
- **Проверенный checkpoint:** `E-BUILD-245`/`E-AUTO-224`/`E-HIL-241`/`E-SAFETY-089`/`RB-M258` принимают exact physical `1.0.0-dev.376`: каждый настоящий runtime WDT теперь на первом запуске Safe Mode пишет одну exact-app причину во внутреннюю NVS, не дублирует её при следующих reset и по возможности зеркалирует на зарегистрированную exact-CID SD только после явного выхода из Safe Mode. Сохранённый отклонённый run dev.375 поймал stack canary loopTask в новом SD-пути; dev.376 переносит секторный workspace FatFS с этого стека (4 496 → 384 B), переоткрывает native USB после каждого HIL restart и проходит всю цепочку WDT → NVS → restart → SD → Home/none/lease 0. Dev.374 остаётся принятым baseline длительной работы Wi-Fi, dev.357 — periodic full anchor.
