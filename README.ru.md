...
- **Проверенный checkpoint:** exact `0.155.7-targets-shared-codec` на firmware source `7328220c641e25ca8ae34ea311500990108a1a18` и verification source `9ae9192803c96523ce0b76558494a8899fa88e4c` физически принят в `E-AUTO-113`/`E-HIL-173`/`E-UX-050`. On-device review объясняет одно реальное предложение Wi-Fi→BLE двумя exact observations, confidence `medium`, score 463 и двумя начисленными features; явный Accept атомарно продвигает Target state 8→9, decision log 0→1 и revision выбранного Target 3→4. После hardware reset cold reopen-ится тот же ID/revision, proposal исчезает, а immutable source population остаётся 69. Общий workspace codec Session/Target позволяет mount и persistence полного decision graph при измеренном largest heap block 29 684 B; соседняя regression mutation выполняет 3 writes/3 file syncs/3 directory syncs, финал — Home/none/lease 0. Отложенный RF-positive gate S5 открыт.
- **Следующий gate:** завершить явный Reject correlation без изменения ownership, затем показать reversible merge/split на устройстве с preview ownership, confirmation, неизменяемым source evidence и cold-reopen точного reversal. Physical gate S5 остаётся отложенным до приезда replacement DIV и его read-only profile.
