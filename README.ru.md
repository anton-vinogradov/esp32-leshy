...
- **Проверенный checkpoint:** `E-BUILD-200`/`E-AUTO-175`/`E-HIL-211`/`E-UX-066`/`RB-M211` принимают platform/product path CAP-053 и physical stock-profile negative в exact `1.0.0-dev.285` на source `8a5799aca03f96ae518ae0c7c7391b43828d6f4f`. Allocation-free endpoint `Serial1` имеет volatile ring 256 bytes, service bound 64 bytes, monitor-only TX disable и cleanup scrub/close. Устройство → Serial Console и strict CLI используют общий typed Action boundary. На original stock RF-shield и preview, и confirmed run останавливаются на `mux_conflict` до configure/start; `Console|Mux56=0`, `pins_touched=false`, heap 72 576→72 576 B, safe outputs неизменны и cleanup заканчивается Home/none/lease 0.
- **Следующий gate:** на explicitly reviewed no-RF fixture `mux56-3v3` принять positive receive-only traffic и cleanup timeout/fault/panic, затем принять Bridge только с explicit permission `serial.write`. Encrypted Save transcript остаётся отдельной более поздней action. Located/timed export CAP-050 всё ещё требует отдельно принадлежащего non-conflicting GPS profile; physical HTTP parity — dedicated client, который не затрагивает active Wi-Fi Mac или Cardputer; deferred S5 RF carrier gate остаётся заблокирован replacement hardware.
