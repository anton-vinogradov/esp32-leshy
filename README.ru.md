...
- **Проверенный checkpoint:** `E-BUILD-216`/`E-AUTO-192`/`E-HIL-224`/`E-UX-076`/`E-STORAGE-070`/`RB-M228` принимают exact physical `1.0.0-dev.331`. Owner-only и отдельно подтверждённое действие **Отключить PIN** сохраняет protected access после cold boot: состояние остаётся `disabled`, exact CID `FE343253440000002000000055019CB7` read-only открывает generation 8/observation 54 с прежней записью Библиотеки, physical writes и RF TX равны zero, cleanup заканчивается Home/none/lease 0. PIN/digest не наблюдались и не сохранялись. Exact dev.302 остаётся periodic full anchor.
- **Следующий gate:** `FF-2` остаётся активен. Exact dev.331 объединяет protected screenshot slice dev.329, потоковую no-PSRAM передачу screenshot и подавление idle repaint dev.330, а также принятый passwordless protected access dev.331. Host/build dev.332 добавляет обязательное по product review сквозное правило repaint: каждый full-scene вызов явно объявляет scope, неизменившийся live tick ничего не рисует, а изменение RSSI в списке Bluetooth атомарно заменяет только ограниченную строку сигнала и quantized bars. Host/build dev.333 применяет то же правило к первому receive-only IR-срезу Protocol Workbench. Dev.334 добавляет deadline-driven coalescing scanner-rate updates; dev.335 затем возвращает полезный принцип snapshot из 0.x: список BLE публикуется один раз за текущее двухсекундное окно passive scan, сохраняя немедленную navigation, identity-stable selection и отрисовку только изменившихся строк. Карточка/detail radar остаётся на 4 Гц. Physical TFT acceptance этих corrections пока не заявляется. Workbench сохраняет Export отдельным Library action, workspace ограничен 1 KiB, TX/replay/output API отсутствует. Когда board-01 снова будет подключена, affected delta gate сначала визуально проверит список Bluetooth, cursor Protocol Workbench и честное empty state Lab, затем один раз пройдёт byte-identical workflow on-device screenshot → protected Library → USB export и откроет то же generation после cold boot. Calibrated distance и physical acceptance Workbench не заявляются. Automation/HID остаётся заморожен на принятом безопасном public-trust checkpoint dev.308 при отключённом execution; positive Serial Console traffic всё ещё ждёт explicitly reviewed no-RF fixture `mux56-3v3`, а optional GPS, physical HTTP parity и deferred S5 RF carrier gate остаются externally blocked.
