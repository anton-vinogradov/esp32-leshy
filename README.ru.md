...
- **Проверенный checkpoint:** `E-BUILD-241`/`E-AUTO-219`/`E-HIL-237`/`RB-M253` принимают exact physical `1.0.0-dev.369`: Устройство → Подключение → «Приватность подключения» по умолчанию создаёт новую private identity для каждой Wi-Fi-сессии Лешего и даёт явный opt-out «Аппаратный адрес». Две отдельные passive сессии «Сети рядом» продвигают generation/application 1→2, доказывают local-admin unicast identities, отличные от hardware, не сохраняют raw address и не меняют identities окружающих устройств. SoftAP, host-network tools, Wi-Fi Mac и raw radio TX остаются zero; exact-CID storage read-only, cleanup достигает Home/none/lease 0. Sandbox-denied precursor сохранён hash-ом и не получил доступ к плате или flash. Exact dev.357 остаётся periodic full anchor.
- **Следующий gate:** progress остаётся **24/62**, отдельный фиксированный Wi-Fi track теперь **16/21 готово, 5/21 осталось**. `WF-17` закрывает часть `FUNC-59` про собственные STA/AP identities; более широкая short-lived synthetic identity из owned Capture остаётся active, поэтому общий знаменатель не меняется. `WF-11` — следующая исполнимая Wi-Fi-цепочка: завершить privacy-safe journey проверки собственного evidence. `WF-18…WF-21` остаются queued до явного admission изолированного fixture/второй платы, RF TX запрещён.
