...
- **Проверенный checkpoint:** `E-BUILD-195`/`E-AUTO-170`/`E-HIL-207`/`E-UX-064`/`RB-M206` принимают exact physical `1.0.0-dev.278` — UI/KDF slice Device Lock CAP-052. На original board-01 непостоянный status/PIN editor выполняет два exact known-vector PBKDF2-HMAC-SHA-256 по 120 000 rounds; повторный KDF 7,467 s сохраняет byte-invariant warm heap 67 488 B, подтверждает UI input за 19,929 ms и перерисовывает только изменённую цифру PIN. Generation credential остаётся zero, storage — generation 176 с zero physical writes, radio не затронуто, errors/drops input равны нулю, cleanup возвращает Home/none/lease 0. Приняты только responsive UI и production boundary KDF/watchdog, не enrollment, cold retry/recovery, admission protected actions или encrypted data at rest.
- **Следующий gate:** физически enroll-ить PIN, cold-restore exact credential, доказать reset-resistant retry delay и переход recovery-only на original board-01, затем провести safe Stop/panic/update/factory reset и protected actions через access matrix без раскрытия protected content. Located/timed export CAP-050 всё ещё требует отдельно принадлежащего non-conflicting GPS profile; physical HTTP parity — dedicated client, который не затрагивает active Wi-Fi Mac или Cardputer; deferred S5 RF carrier gate остаётся заблокирован replacement hardware.
...
| **Active-confirmed** | **Блокировка устройства (CF-006):** local PIN, bounded retry/recovery и защита secrets/evidence без блокировки Stop, panic и recovery | 🟡 exact dev.278 физически принимает responsive status/PIN UI и production KDF/watchdog; остаются enrollment, cold retry/recovery, action admission и encryption, S7 |
