...
- **Проверенный checkpoint:** Wi-Fi UI delta dev.384: восемь страниц карточки, product-key/synthetic-touch навигация, неподвижная подготовка, 13 измеренных каналов, неизменяемое обрамление графика, ноль scan drops/операций SD и Home/none/lease 0. Это не проверка пальцем/оптическая приёмка мерцания. Тёплый отказ BLE dev.381 остаётся открытым.
- **Следующий gate:** Progress **24/62**, Wi-Fi **17/21 готово, 4/21 осталось**. Дальше: скрытое имя/источник на втором собственном DIV, оставшаяся интеграция дерева задач/мастеров и допущенные Wi-Fi Lab fixtures. Wi-Fi ноутбука не используется как fixture. Тёплый BLE и очередная полная physical matrix остаются отдельными незакрытыми gates.
