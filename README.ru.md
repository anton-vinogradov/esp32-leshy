...
- **Проверенный checkpoint:** `E-BUILD-244`/`E-AUTO-223`/`E-HIL-240`/`E-SAFETY-088`/`RB-M257` принимают exact `1.0.0-dev.374`, исправляющий ночное зависание Wi-Fi-цикла. Сохранённый incident dev.372 доказывает настоящий reset Task-WDT в latched Safe Mode, а не исчерпание heap либо ошибку input/storage. Dev.374 ограничивает каждый foreground drain очередей Product/Wi-Fi, ограничивает частоту анализа passive data frames и сохраняет первый exact-app watchdog trace page/stage/CPU после quiesce outputs. Exact delta HIL затем держит «Устройства Wi-Fi» 180,122 с/1 477 переключений и «Сети рядом» 180,008 с/66 циклов с непрерывным прогрессом, нулевыми radio drops, без повторения watchdog, без post-warm drift heap, с read-only storage и финальным Home/none/lease 0. Host-основа IR replay dev.373 остаётся принятой с отложенным two-board physical gate; dev.357 остаётся periodic full anchor.
