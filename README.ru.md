...
- **Следующий gate:** доказать exact dev.257 Field Survey setup/первый визит/revisit/result одним focused receive-only delta на board-01, включая честный negative incomplete input и clean Home/none/lease 0. Routing native/WiGLE artifacts, live capture stations и optional trusted GPS/time затем остаются отдельными slices CAP-050. Stop/commit/cold-recovery Product Survey остаётся отдельным pending после прежней попытки `running_degraded`, завершившейся fail closed до commit. Physical HTTP parity остаётся отложенным до dedicated client, не затрагивающего активный Wi-Fi Mac.
