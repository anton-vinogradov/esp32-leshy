...
- **Проверенный checkpoint:** `E-BUILD-237`/`E-AUTO-215`/`E-HIL-233`/`E-UX-088`/`RB-M249` принимают exact physical `1.0.0-dev.365`: Защита эфира сначала спрашивает об окружении и предлагает понятные профили **Обычно**, **Тихое место** и **Многолюдное место**. Affected delta доказывает три exact policy, три passive start/cancel, repaint только selector content при zero static-pixel changes и final Home/none/lease 0. Существующий full release gate Защиты эфира не ослаблен; его same-image ambient precursor честно остался fail-closed на malformed ingress. Exact dev.357 остаётся periodic full anchor.
- **Следующий gate:** progress остаётся **23/62**, отдельный фиксированный Wi-Fi track теперь **12/21 готово, 9/21 осталось**. `WF-13` физически принят с privacy-minimal retained evidence; ambient SSID/BSSID/vendor, raw run и screenshot не сохраняются. Следующая focused delta — `WF-14`: завершить уже bounded пользовательский путь offline Field Survey AP/station, сравнение revisit и экспорт WiGLE. Companion verification chain `WF-11` остаётся active параллельно; physical gates второй платы и dedicated client отложены, RF TX запрещён.
