...
- **Проверенный checkpoint:** physical 0.146/0.147/0.148 — три разных сохранённых fail-closed результата. `E-HIL-164` нашёл многокилобайтный result сравнения в stack loop; 0.147 устранил его, записал реальные поколения 111/112 с 32 observations и zero drops, затем выявил post-workspace mount failure в `E-HIL-165`. Exact source-bound 0.148 исправил порядок storage, но его one-flash short gate и decoded reproduction без перепрошивки нашли второй aggregate temporary 7 736 B в `TargetsController::reset()`; `E-HIL-166` сохраняет exact backtrace, panic reset, zero writes/TX и final safe Home/lease 0. Candidate `0.149.0-targets-inplace-reset` byte-clear-ит lifecycle-owned result на месте. Exact ELF preflight теперь отклоняет oversized Targets frames до прошивки; текущие frames: reset 256 B, load 416 B, comparison 80 B, deepest builder 1 104 B. Полный host/contracts и production build проходят при 211 296 B static RAM и 3 107 472 B linked flash. Exact 0.145 остаётся последней физически принятой baseline board-01; отложенный RF-positive gate S5 открыт.
- **Следующий gate:** commit-bind exact `0.149.0-targets-inplace-reset` и выполнить короткую board-01 regression на существующих поколениях 111/112: одна прошивка, exact-CID read-only mount error 0, List→Compare, zero writes/TX/drops, освобождённый heap и final Home/lease 0. Только после pass переиспользовать те же flashed bytes без перепрошивки для full delta с двумя новыми passive Wi-Fi визитами и evidence List→Compare→Detail. Physical gate S5 остаётся отложенным до приезда replacement DIV и его read-only profile.
