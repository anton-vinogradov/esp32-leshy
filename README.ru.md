...
- **Проверенный checkpoint:** `E-BUILD-175`/`E-AUTO-149`/`RB-M186` принимают exact host/build `1.0.0-dev.247` на source `ccb4a3a5351d065e312ca29cf689d3acd9e6d93b`. Он добавляет bounded controller результата CAP-049 и EN/RU path `Результат → Действия → Устройство → Доказательства → Детали`, честный приоритет `Полный`/`PMKID`/`Частичный`, `Повторить`, фильтрацию zero-mask peers, content-only repaint и fail-closed rejection M2/M3/M4 с полностью нулевым Key MIC. Storage foundation добавляет provenance authentication как schema 8 Session/segment и wire 5 capture, сохраняя generic открытие raw frames и совместимость чтения schema 4. Full suite `tools/test.sh` прошёл до того, как независимое review нашло этот P1 zero MIC; на exact source после fix проходят focused capture/policy ASan+UBSan, focused review и production build/budget checks. Exact hashes app/factory: `b3bb3e3a787ef36aa306d0711a40c0b5b730a02f69fc1d72e4a59315b0435ed1`/`186c24fbd831db40dc5b936c6bbd91bee7908cefc38c9aa651fbfd8676c09ff1`; embedded identity ELF/app: `954a5a80064a951bac6a3f278cdd5e06292d7589871ab132f5cd680e36e9dba4`, map: `2fd0300e65fb4b4d4c4b2bc0a1f01bb97627e19a65eb7d1f51503b9b82f79ccf`. Это только host/build evidence: принятым physical baseline остаётся `E-BUILD-174`/`E-AUTO-148`/`E-HIL-191`/`RB-M185` на exact dev.246, чей ambient run не увидел EAPOL/PMKID и корректно остался `inconclusive`.
- **Следующий gate:** после USB power-cycle оригинального DIV один раз прошить exact dev.247 и выполнить focused TFT/navigation regression для path `Результат → Действия → Устройство → Доказательства → Детали`, `Повторить`, Up/Down, Right/OK внутрь, Left/Back и content-only repaint. Только после этого physical delta следует реализовать standard artifact serializer, подключить product persistence/export к существующему atomic store и проверить path полезным EAPOL/PMKID evidence. Stop/commit/cold-recovery Product Survey остаётся отдельным pending: текущая mixed-source попытка дошла до `running_degraded`, fail closed до commit и не является gate evidence. Physical HTTP parity остаётся отложенным до dedicated client, не затрагивающего активный Wi-Fi Mac.
