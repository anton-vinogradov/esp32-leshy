...
- **Следующий gate:** `FF-2` остаётся активен. Exact dev.331 теперь объединяет protected screenshot slice dev.329, потоковую no-PSRAM передачу screenshot и подавление idle repaint dev.330, а также принятый passwordless protected access dev.331. Прежний owner-lock precondition закрыт; когда board-01 снова будет подключена, affected delta gate должен один раз пройти byte-identical workflow on-device screenshot → protected Library → USB export и открыть то же generation после cold boot. Calibrated distance не заявляется. Automation/HID остаётся заморожен на принятом безопасном public-trust checkpoint dev.308 при отключённом execution; positive Serial Console traffic всё ещё ждёт explicitly reviewed no-RF fixture `mux56-3v3`, а optional GPS, physical HTTP parity и deferred S5 RF carrier gate остаются externally blocked.
