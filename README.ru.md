...
- **Следующий gate:** физически прогнать готовый по host/build кандидат `0.182.0-companion-web-http-parity`. Один explicit-port delta обязан подключиться к temporary AP, выполнить exact `GET /` и все paged requests Session/Target/Compare через общий `POST /api/v1/companion`, доказать byte-equivalent projections native USB, выполнить и вернуть одну confirmed mutation Favorite тем же exact-CID atomic worker, уничтожить все credential/grant/listener и восстановить захваченное состояние Wi-Fi Mac. Physical gate S5 остаётся отложенным до приезда replacement DIV и прохождения его read-only profile.
