...
- **Проверенный checkpoint:** exact `0.156.0-targets-reject-rebuild` на firmware source `0559fc632f8a1dc856000d57113d24b61ab801aa` физически принят в `E-AUTO-114`/`E-HIL-174`/`E-UX-051`. Явный Reject атомарно продвигает Target state 10→11 и decision log 2→3, сохраняя revision Target 5→5, видимое ownership 4→4 и полный каталог 16→16. Немедленно перестроенный view остаётся рабочим даже на пределе каталога, честно сообщает семь пропущенных admissions, а hardware reset cold-reopen-ит generation 11 с 84 immutable source identities. Runtime принимает уже проверенный graph worker без второй allocation 22,5 КиБ; persistence выполняет 3 writes/3 file syncs/3 directory syncs, heap полностью освобождается, финал — Home/none/lease 0. Отложенный RF-positive gate S5 открыт.
