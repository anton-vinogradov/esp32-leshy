...
- **Проверенный checkpoint:** `E-BUILD-177`/`E-AUTO-151`/`RB-M188` принимают exact host/build `1.0.0-dev.249` на product source `d47b2ee5e1636981474398246c5e0c49d88db2ea`: CAP-049 теперь имеет bounded canonical serializer Hashcat mode 22000, явное подтверждение Сохранить, atomic persistence schema 8 с exact-generation reopen/re-analysis, честный fallback только PCAP, экспорт `.hc22000` лишь при полезном evidence и запрет persistence synthetic fixture. Проходят полный tracked host matrix, focused ASan/UBSan cases, contract/mutation checks и clean production build. Device, serial port, Wi-Fi Mac, клон и Cardputer не затрагивались, поэтому это не physical acceptance; exact `1.0.0-dev.248` остаётся последним принятым physical baseline.
- **Следующий gate:** прошить exact `1.0.0-dev.249` только на оригинальный DIV и доказать явный Save, atomic commit SD, exact-generation cold reopen и экспорт PCAP; готовность `.hc22000` дополнительно обязана быть доказана полезным bounded PMKID или replay-consistent M1→M2 evidence, а ambient run без evidence должен оставаться честным и неэкспортируемым. Stop/commit/cold-recovery Product Survey остаётся отдельным pending после того, как прежняя попытка `running_degraded` fail closed до commit. Physical HTTP parity остаётся отложенным до dedicated client, не затрагивающего активный Wi-Fi Mac.
