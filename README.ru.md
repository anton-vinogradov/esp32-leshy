...
| S5.6 | Интегральный hardware gate S5: exact 0.144 уже принимает автономную on-device половину Full receivers/artifacts без утечки leases/outputs. One-build/one-flash-per-role runner IR→nRF24→OOK→FSK build-checked на source `4f97b3a`, а его strict cross-child acceptance host-checked на `95079ec`; выполнить physical половину с qualified source после S5.3/S5.4 | ⬜ дальше |
