...
- **Проверенный checkpoint:** `E-BUILD-184`/`E-AUTO-159`/`E-HIL-198`/`RB-M195` сохраняют exact receive-only preflight Field Survey `1.0.0-dev.261` на оригинальном board-01, source `b38464f93cdb7807734a24b1e1a08f03d4bbae24`. Перенос 4 560-байтного raw capture CAP-049, нужного только HIL, из постоянной RAM продукта снижает static use до 231 624 B и восстанавливает реальный lifecycle Wi-Fi→BLE: 12 Wi-Fi и 33 BLE records прочитаны, приняты и переданы с zero drops/writes, exact generation 170 не меняется, cleanup достигает Home/none/lease 0 при неактивном HIL. Сохранённый precursor dev.260 фиксирует прежний timeout NimBLE `host_sync` и отдельную dense-input границу 81→64/17 drops. Это regression checkpoint, а не gate визита CAP-050.
- **Следующий gate:** сделать source input плотнее 64 records bounded и complete без изъятия BLE startup budget, затем выполнить один exact delta первого/revisit визита с commit/cold recovery и deterministic negative incomplete input. Routing native/WiGLE artifacts, live capture stations и optional trusted GPS/time остаются отдельными slices CAP-050. Physical HTTP parity остаётся отложенным до dedicated client, не затрагивающего активный Wi-Fi Mac.
