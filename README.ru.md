...
- **Следующий gate:** представить уже принятый bounded companion contract через local Web без второго domain API и более широких grants. USB NDJSON и Web JSON обязаны иметь одинаковые scopes, lifecycle capabilities, правила preview/confirmation и fail-closed bounds. Physical gate S5 остаётся отложенным до приезда replacement DIV и прохождения его read-only profile.
