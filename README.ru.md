...
- **Проверенный checkpoint:** exact `0.129.0-pre-app-watchdog` завершает физическую цепочку двух плат NEC receive → save → cold Library CSV за 33/33 шага. Exact `0.130.0`/`0.131.0` меняют shared MISO/GPIO13 board-02 из LOW с подключённым RF carrier в HIGH при снятом carrier. Exact `0.132.0-carrier-csn-characterization` снова подключает carrier и наблюдает все четыре receiver select в HIGH на 32/32 samples (nRF GPIO4/48/21 и CC1101 GPIO5), а MISO возвращается в LOW: 0/32 HIGH samples под обоими pull. Run выполняет zero SPI bytes, receiver reads, CE-high events, strobes и TX commands и возвращает Home/none/lease 0. Это отвергает случайно выбранный receiver и локализует fault до carrier module или общей MISO-сети carrier; один модуль не определён, antenna/U.FL fault не доказан. Exact `0.135.0-survey-preparation-deadline` расширяет принятый safety slice S5.5 на board-01 до preparation/admission Product Survey: normal публичный BLE lifecycle последовательно взводит preparation и калиброванный worker, принимает 30/30 observations за одну scan attempt с zero scan drops/retries и без ложного trip. Второй lifecycle внедряет stall 10 s до hardware preparation; preparation deadline 8 000 ms срабатывает один раз через 8 001 ms, cancel/cleanup доводит owner/lease до `none`/`0`, Safe Mode переживает restart, требует явный двухшаговый clear и возвращает Home с неизменными exact CID/catalog 98/0 и zero storage writes.
...
| S5.5 | Полнота runtime: exact 0.135 принимает preparation/admission Product Survey плюс калиброванные heartbeat/deadline slices workers Wi-Fi+BLE; другие workers, low-voltage safe-write, sleep/resume и применимые явные assembly profiles GPS/PN532 остаются | ⬜ дальше |
