...
- **Проверенный checkpoint:** `E-BUILD-180`/`E-AUTO-154`/`E-HIL-195`/`E-UX-059`/`RB-M191` физически принимают exact `1.0.0-dev.252` на оригинальном board-01, source `30530812efe045aadd112d8b1b0961a48a48b89b`. Неизменившаяся revision BLE catalog теперь считается завершённым incremental no-op и не проваливается в full-screen render; enriched metadata заменяется атомарно, а обновления detail объединяются с периодом 250 ms/4 Hz. За два полных scan cycle и 2 633 ms выполняются 6 bounded refreshes, 19 лишних объединяются, pending в конце отсутствует; content clears/full radar repaints и allocation/direct-fallback failures равны zero, меняются 904 radar pixels и zero static/chrome pixels. Обязательный `E-AUTO-155`/`E-HIL-196`/`E-CADENCE-002` full checkpoint повторно использует тот же exact image без новой прошивки и проходит восемь Home sections, 20 TFT states, оба RX-водопада по 224 строки, read-only Targets 165/166 и USB companion с неизменными generation/heap, zero writes/TX/доступа к Wi-Fi Mac и final Home/none/lease 0.
- **Следующий gate:** переиспользовать exact image `1.0.0-dev.252`, уже прошитый в оригинальный DIV, и доказать явный Save, atomic commit SD, exact-generation cold reopen и экспорт PCAP; готовность `.hc22000` дополнительно обязана быть доказана полезным bounded PMKID или replay-consistent M1→M2 evidence, а ambient run без evidence должен оставаться честным и неэкспортируемым. Stop/commit/cold-recovery Product Survey остаётся отдельным pending после того, как прежняя попытка `running_degraded` fail closed до commit. Physical HTTP parity остаётся отложенным до dedicated client, не затрагивающего активный Wi-Fi Mac.
...
| **Passive audit** | **Захват Wi-Fi-аутентификации (CF-002):** различает EAPOL/PMKID и complete/incomplete handshake, сохраняет focused Capture и экспортирует PCAP/`hc22000` | 🟡 active: physical dev.248 принимает live receive/navigation; host/build dev.249 реализует явный atomic Save, exact-generation reopen, PCAP и `hc22000` только с полезным evidence; physical persistence/export и полезный authentication evidence остаются открыты |
