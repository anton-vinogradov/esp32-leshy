...
- **Проверенный checkpoint:** `E-BUILD-208`/`E-AUTO-183`/`E-HIL-216`/`E-UX-069`/`RB-M219` принимают exact physical `1.0.0-dev.306` на firmware source `41bf103008202e742598dfd3e092b28aeda45d95`. Последний пункт «Устройство» теперь показывает защищённый UI list/import/revoke trust store P-256 на четыре ключа. Он читает только `/leshy/automation/v1/automation-owner.lhak`, проверяет public material до отдельного review и сохраняет execution disconnected. Focused button/touch HIL сохраняет stable EN/RU frames и missing-bundle result при неизменных count/generation, read-only SD, zero private-key/Action/HID/RF output, восстановленных Device Lock/language и final Home/none/lease 0. Exact dev.302 остаётся periodic full anchor.
- **Следующий gate:** использовать public bundle `LHAK`, созданный GitHub, и физически доказать authenticated positive enrollment, cold restore, inspection trusted/unknown/invalid и revocation при отключённом execution. Positive Serial Console traffic всё ещё ждёт explicitly reviewed no-RF fixture `mux56-3v3`; optional GPS, physical HTTP parity и deferred S5 RF carrier gate остаются externally blocked.
