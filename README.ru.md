...
- **Проверенный checkpoint:** exact `0.170.0-companion-usb-rx` на source `b58fbc054522cecfca5dd4afcd6ea61098cb05c0` физически принят в `E-BUILD-147`/`E-AUTO-118`/`E-HIL-177`/`E-COMPANION-002`. На явно выбранном порту оригинального DIV native USB выдаёт read-only scopes Session/Target/Compare только пока ready Targets владеет live snapshot; он возвращает две exact Sessions, все 16 Targets, все пять detail sections и семь строк comparison (2 added, 1 removed, 0 changed, 4 unchanged). Exact input 512 bytes проходит, а 513-byte/malformed/unknown/out-of-range/mutation/post-exit requests fail-close-ятся; выход из Targets отзывает grant. Run выполняет zero radio TX, storage writes, input drops, port discovery и открытий Cardputer, заканчивая Home/none/lease 0 при invariant free heap 92 972 B. Это принимает read-only native-USB boundary S6.5; local Web и mutations остаются открыты, как и отложенный RF-positive gate S5.
