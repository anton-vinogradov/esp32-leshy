...
- **Проверенный checkpoint:** host/build checkpoint `E-CORR-004` завершает S6.3 bounded read-only Action `target.compare` schema v1. Она сравнивает две разные доступные остановленные Sessions по exact references source ID/generation/sequence/time, выбирает последнее сохранённое Observation для каждой identity Target и deterministic сообщает added, removed, changed и unchanged Targets. Changed result содержит явную маску identity/radio/frequency/channel/signal/label/Wi-Fi/BLE; шум RSSI меньше 6 dB намеренно считается стабильным. Каждый результат удерживает использованное exact baseline/current evidence. Missing source, unavailable record, malformed Observation или mismatch identity/source очищает весь результат вместо partial diff. Golden tests реальных Sessions, negative tests, ASan/UBSan и production build проходят; bounded result занимает 7 736 B без роста static RAM. Exact `0.145.0-interface-settings` остаётся последней физически принятой baseline board-01; отложенный RF-positive gate S5 остаётся открыт.
- **Следующий gate:** подключить lifecycle-owned Target state и comparison двух Sessions к product без overflow permanent static, затем показать полезные on-device Target List/Detail и Compare summary, где строки открывают exact evidence и сохраняют принятую стабильную навигацию/no-flicker shell. Physical gate S5 отложен, но не отменён: когда приедет заказанный replacement DIV, его read-only profile должен пройти до запуска сохранённой one-command матрицы IR→nRF24→OOK→FSK, закрывающей S5.3/S5.4/S5.6.
