...
- **Проверенный checkpoint:** `E-BUILD-188`/`E-AUTO-163`/`E-HIL-202`/`RB-M199` принимают software slice optional trusted context Field Survey на оригинальном board-01 с exact `1.0.0-dev.267`, source `eb23d614785420a10588a4ae5a8d3e351021702b`. Schema 9 сохраняет одну immutable checksummed запись GPS-NMEA UTC/location, продолжает читать schemas 1…8 и отвергает stale, malformed или невозможные fixes. Stock no-GPS path cold-recover-ит generation 175/51 read-only, экспортирует 51 native и 49 representable WiGLE rows, честно остаётся `trusted_source=none`, `untimed_unlocated`, `upload_ready=false` и выходит Home/none/lease 0 с zero scans/writes.
- **Следующий gate:** начать CAP-051 BLE Inspector с bounded сохранением compatible raw advertisements и явной permissioned state machine connected GATT. Реализация trusted context CAP-050 software-complete, но physical located/timed export явно отложен до появления отдельно принадлежащего non-conflicting GPS profile. Physical HTTP parity остаётся отложенным до dedicated client, не затрагивающего активный Wi-Fi Mac.
