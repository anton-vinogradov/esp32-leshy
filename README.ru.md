...
- **Проверенный checkpoint:** `E-BUILD-186`/`E-AUTO-161`/`E-HIL-200`/`RB-M197` принимают native CSV и честный local WiGLE 1.6 export из exact retained Field Survey generation 172/52 на оригинальном board-01 с exact `1.0.0-dev.263`, source `0f46b4db840bf38e3beac6424623c33fca8e749e`. Native export выдаёт 52 deduplicated records (16 Wi-Fi AP, 0 stations, 36 BLE) с evidence/provenance; WiGLE выдаёт те же 52 representable records и явно остаётся `untimed_unlocated`, `upload_ready=false`, не выдумывая UTC или координаты. Recovery read-only с attempt 1, zero scans, commits, physical/blocked writes; ambient identifiers и raw exports не сохраняются, pixels «Экспорт готов» machine-pinned, final cleanup Home/none/lease 0.
- **Следующий gate:** захватить live passive observations Wi-Fi stations в Field Survey и доказать их native export, сохранив явную границу unsupported station для WiGLE. Optional trusted GPS/UTC остаётся финальным slice CAP-050. Physical HTTP parity остаётся отложенным до dedicated client, не затрагивающего активный Wi-Fi Mac.
