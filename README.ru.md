...
- **Проверенный checkpoint:** `E-BUILD-194`/`E-AUTO-169`/`RB-M205` принимают exact host/build `1.0.0-dev.277` — foundation Device Lock CAP-052. Pure state machine реализует PIN 6–12 цифр, verifier PBKDF2-HMAC-SHA-256, persistent reset-resistant retry до recovery-only, volatile expiry unlock, fail-closed access matrix и destructive-only recovery ordering. Fixed record `LDLK` 68 bytes и независимый provisioned latch NVS различают virgin, torn, missing и corrupt state. Production adapters crypto/NVS ESP32-S3 входят в binary, read-only restore выполняется при boot; проходят strict native, contract, полный tracked host и production-build gates. Existing product UI/actions ещё не gated, поэтому это не выдаётся за завершённый Device Lock или encrypted data at rest.
- **Следующий gate:** добавить on-device UI PIN setup/lock/unlock и status, измерить real PBKDF2/watchdog budget, доказать cold persistence и reset-resistant delay на original board-01, провести safe Stop/panic/update/factory reset вокруг lock без раскрытия protected content. Located/timed export CAP-050 всё ещё требует отдельно принадлежащего non-conflicting GPS profile; physical HTTP parity — dedicated client, который не затрагивает active Wi-Fi Mac или Cardputer; deferred S5 RF carrier gate остаётся заблокирован replacement hardware.
