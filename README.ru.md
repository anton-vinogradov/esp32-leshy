...
- **Проверенный checkpoint:** `E-BUILD-192`/`E-AUTO-167`/`E-HIL-205`/`E-UX-062`/`RB-M203` физически завершают CAP-051 BLE Inspector в exact `1.0.0-dev.276`, source `6b5d27b2253bd7b335bab8754379a5ce51a0a5d2`. Один fresh flash доказывает timeout cleanup и последующее positive recovery; один reuse exact image затем доказывает, что wrong peer, timeout, resource conflict и видимый failed disconnect все fail closed и возвращают Home/none/lease 0. Финальное подключение по-прежнему перечисляет 5 services/7 characteristics с zero pair/read/write/subscribe operations. Bounded passive callback queue на 32 reports достигает high-water 3 с zero drops и возвращает 1 304 B static RAM, поэтому GATT после отказов инициализируется при 1 540 B free и minimum 1 480 B.
- **Следующий gate:** продолжить активный путь S6.5/S6.6 и доказать terminal commit Product Survey с cold reopen по exact CID, не ослабляя принятые companion- и CAP-051-boundaries. Located/timed export CAP-050 всё ещё требует отдельно принадлежащего non-conflicting GPS profile, а physical HTTP parity — dedicated client, который не может затронуть активный Wi-Fi Mac или Cardputer.
