...
- **Проверенный checkpoint:** exact `0.152.0-targets-name-edit` на source `7c9e358493afcf0d4fbaf8b27d567745a7138e0e` физически принят в `E-AUTO-110`/`E-HIL-170`/`E-UX-047`. «Цель»→«Действия»→«Имя» открывает bounded четырёхстрочный keypad/touch editor, добавляет `A`, повторно идентифицирует exact CID и атомарно публикует catalog-only schema-v3 generation 2→3 с 3 writes/3 file syncs/3 directory syncs. Physical cold reset открывает тот же 128-bit Target с UTF-8 bytes `41`; UI acknowledgement занимает 155 µs, supervised worker — 2,825 s; pre-mount workspace 16 КиБ помещается в largest block 34 804 B, heap освобождается с 85 072 до 96 852 B, финал — Home/none/lease 0. Отложенный RF-positive gate S5 открыт.
- **Следующий gate:** добавить bounded tags, затем notes, на том же exact-CID atomic path с сохранением выбранного Target. Далее идут correlation accept/reject; merge/split остаётся за явным confirmation с preview изменения ownership и сохранением reversal. Physical gate S5 остаётся отложенным до приезда replacement DIV и его read-only profile.
