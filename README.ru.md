...
- **Проверенный checkpoint:** `0.146.0-targets` сохранён как fail-closed и не принят: первый физический вход в «Цели» вызвал stack canary задачи loop в `compareTargetSessions`. Декодированный trace, identities raw serial и отдельный результат failed runner сохранены как machine-checked `E-HIL-164`. Candidate `0.147.0-targets-stack-safe` убирает возврат многокилобайтного значения, пишет result 7 736 B прямо в foreground workspace и переносит два снимка сторон сравнения в проверяемый автоматически освобождаемый heap scratch. Native `-fstack-usage` теперь измеряет 112 B у входа сравнения и 1 088 B у самого глубокого builder evidence. Финальное четырёхстрочное меню Wi-Fi также заменяет дублирующий shortcut PCAP публичным persistent path **«Записать визит»**; запись пакетов остаётся в отдельном Home-разделе «Запись». Полный host/contracts и production build проходят при 211 296 B static RAM и 3 107 636 B linked flash. Exact `0.145.0-interface-settings` остаётся последней физически принятой baseline board-01 до corrected delta; отложенный RF-positive gate S5 остаётся открыт.
- **Следующий gate:** commit-bind exact `0.147.0-targets-stack-safe`, один раз прошить board-01, создать две реальные passive Wi-Fi Sessions через Wi-Fi → «Записать визит», затем автоматически снять Targets List→Compare→Detail и доказать отсутствие reset/stack canary, exact generations/CID, bounded strongest-first projection, read-only zero-write behavior, освобождение comparison/workspace heap, inactive outputs, zero input drops и final Home/lease 0. Physical gate S5 отложен, но не отменён: когда приедет заказанный replacement DIV, его read-only profile должен пройти до запуска сохранённой one-command матрицы IR→nRF24→OOK→FSK, закрывающей S5.3/S5.4/S5.6.
