...
- **Проверенный checkpoint:** `E-BUILD-197`/`E-AUTO-172`/`E-HIL-209`/`E-UX-065`/`E-STORAGE-036`/`RB-M208` принимают exact physical `1.0.0-dev.281` — recovery/admission slice Device Lock CAP-052. На original board-01 один isolated run выполняет все пять неверных PIN с измеренными post-KDF retry bounds 4 931/14 931/59 930/299 930 ms, cold-restore-ит generation-6 `recovery_only`, запрещает все семь protected operation classes и допускает все семь safe classes в каждом relevant state. Реальный запуск Library остаётся на opaque Home, реальный export не возвращает protected content. Preview reset non-destructive; confirmed reset доказывает erase protected data при ещё существующем credential, затем очищает его. Explicit cleanup fixture и final cold boot возвращают нетронутый product namespace к generation 0/unconfigured, Home/none/lease 0 с zero errors/drops input.
- **Следующий gate:** добавить authenticated encryption at rest для protected data; это единственный оставшийся implementation gate CAP-052. Located/timed export CAP-050 всё ещё требует отдельно принадлежащего non-conflicting GPS profile; physical HTTP parity — dedicated client, который не затрагивает active Wi-Fi Mac или Cardputer; deferred S5 RF carrier gate остаётся заблокирован replacement hardware.
...
| **Active-confirmed** | **Блокировка устройства (CF-006):** local PIN, bounded retry/recovery и защита secrets/evidence без блокировки Stop, panic и recovery | 🟡 exact dev.281 физически принимает cold recovery-only, все protected/safe action classes и destructive recovery с erase до credential; остаётся только authenticated encryption at rest, S7 |
