...
- **Проверенный checkpoint:** `E-BUILD-187`/`E-AUTO-162`/`E-HIL-201`/`RB-M198` принимают live passive capture Wi-Fi stations и его честную export boundary на оригинальном board-01 с exact `1.0.0-dev.266`, source `ffd8f8f23cd3153a5de415049fc2665544af66c1`. Generations first/revisit 174/175 сохраняют по 2 stations с zero source/pipeline drops; revisit содержит 15 AP, 2 stations и 34 BLE records. Read-only native export сохраняет все 51 record, включая обе stations, а WiGLE выдаёт ровно 49 representable AP/BLE rows и сообщает о 2 skipped stations. Он остаётся `untimed_unlocated`, `upload_ready=false`; ambient identifiers и raw exports не сохраняются, final cleanup — Home/none/lease 0.
- **Следующий gate:** допустить optional trusted GPS и UTC в Field Survey и доказать located/timed export, не ослабляя уже принятую untimed/unlocated ветку. Physical HTTP parity остаётся отложенным до dedicated client, не затрагивающего активный Wi-Fi Mac.
