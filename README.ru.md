...
- **Следующий gate:** `FF-2` остаётся активен. Exact dev.331 объединяет protected screenshot slice dev.329, потоковую no-PSRAM передачу screenshot и подавление idle repaint dev.330, а также принятый passwordless protected access dev.331. Host/build dev.332 добавляет обязательное по product review сквозное правило repaint: каждый full-scene вызов теперь явно объявляет scope, неизменившийся live tick ничего не рисует, а изменение RSSI в списке Bluetooth атомарно заменяет только ограниченную строку сигнала и quantized bars. Он также отличает смонтированную read-only SD без необязательной папки Automation от действительно недоступной карты. Прежний owner-lock precondition закрыт; когда board-01 снова будет подключена, affected delta gate сначала визуально проверит список Bluetooth и честное empty state Lab, затем один раз пройдёт byte-identical workflow on-device screenshot → protected Library → USB export и откроет то же generation после cold boot. Calibrated distance не заявляется. Automation/HID остаётся заморожен на принятом безопасном public-trust checkpoint dev.308 при отключённом execution; positive Serial Console traffic всё ещё ждёт explicitly reviewed no-RF fixture `mux56-3v3`, а optional GPS, physical HTTP parity и deferred S5 RF carrier gate остаются externally blocked.
