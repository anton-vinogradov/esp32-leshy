...
- **Проверенный checkpoint:** `E-BUILD-207`/`E-AUTO-182`/`RB-M218` принимают exact host/build `1.0.0-dev.304` на firmware source `96a55a4d46f3171690cb3778d99acc2a95cbd0c2`. Passive product Inspector теперь использует real verifier ECDSA-P256/SHA-256 mbedTLS с canonical, CRC-protected, generation-counted trust store NVS максимум на четыре public key. Missing state восстанавливается пустым; corruption и persistence failure fail closed. Enrollment/revocation требует Device Lock плюс fresh confirmation, но product UI пока не может вызвать mutation. Защищённый GitHub environment выводит и публикует только public bundle `LHAK` 128 bytes и public metadata; private key остаётся в GitHub secrets. Execution по-прежнему disconnected. Exact physical dev.303 остаётся последним focused physical delta, dev.302 — periodic full anchor.
- **Следующий gate:** подключить видимый владельцу UI import/list/revoke trust store, проверить public point и derived key ID до enrollment, затем физически доказать authenticated enrollment, cold restore, inspection trusted/unknown/invalid и revocation при отключённом execution. Positive Serial Console traffic всё ещё ждёт explicitly reviewed no-RF fixture `mux56-3v3`; optional GPS, physical HTTP parity и deferred S5 RF carrier gate остаются externally blocked.
...
| **Passive audit** | **BadUSB inspection (CF-008):** разбирает подписанный Automation/HID script и показывает target, permissions, действия и пределы без исполнения | ✅ exact physical dev.303 принимает malformed/unsigned packages с real exact-CID SD в stable EN/RU views Inspector при zero output Action/HID/resource/RF и complete cleanup; real trust и execution не подключены |
...
| **Active-confirmed** | **Automation/HID execution (CF-008):** выполняет только подписанный permissioned script после preview цели, действий, ceilings и конечной длительности | ⬜ запланировано, S7; принятый passive dev.303 намеренно всё ещё не имеет HID/Action output path |
