...
- **Проверенный checkpoint:** `E-BUILD-205`/`E-AUTO-180`/`E-HIL-214`/`E-CADENCE-003`/`RB-M216` принимают periodic full current-capability checkpoint на exact physical `1.0.0-dev.302`, firmware source `48a27c74cfb3dca1ff1c2ed612bb4a3019133451`. Одна fresh application flash проходит все 21 экрана Home/RF, все 16 evidence views Targets/Compare и две read-only USB companion sessions над persistent generations 7/8 по 54 observations. Оба waterfall по 224 rows меняют только graph pixels; storage, heap после одного bounded cold lazy initialization 260 B, input, CID, safe outputs и final Home/none/lease 0 invariant. RAM-only fixtures Device Lock допускают только protected UI или read-only companion, снимают mutation scope, не пишут credential/data key/product namespace и доказывают cleanup.
- **Следующий gate:** физически провести один bounded malformed/unsigned `.lhau` через nested Automation Inspector и сохранить стабильные EN/RU summary frames при zero output Action/HID/resource. Реальный P-256 trust store и каждый execution path остаются более поздними отдельными gates. Positive Serial Console traffic всё ещё ждёт explicitly reviewed no-RF fixture `mux56-3v3`; optional GPS, physical HTTP parity и deferred S5 RF carrier gate остаются externally blocked.
