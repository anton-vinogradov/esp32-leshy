...
- **Проверенный checkpoint:** `E-BUILD-191`/`E-AUTO-166`/`E-HIL-204`/`E-UX-061`/`RB-M202` физически принимают первый connected enumeration-only slice CAP-051 BLE Inspector в exact `1.0.0-dev.272`, source `fcd0683cb169ac41e42ff302d074a146457c1b1d`. Private exact connectable fixture выбирается по hash без раскрытия identifier; пользователь видит review разрешения и второе подтверждение до освобождения passive scan ownership. Live NimBLE adapter подключается к exact peer, перечисляет 5 services и 7 characteristics в fixed capacities 16/48, выполняет zero characteristic reads/writes/subscriptions/pairings, меняет 0/76 800 pixels стабильной карточки, а Back доказывает disconnect и Home/none/lease 0. Отклонённый dev.271 перезагружался по interrupt watchdog: callback удерживал отключающий interrupts spinlock во время запуска следующей NimBLE operation; dev.272 сериализует callbacks bounded static recursive task mutex.
- **Следующий gate:** физически пройти negative lifecycle connected GATT — wrong peer, timeout connect/discovery, resource conflict и failed cleanup — на exact boundary dev.272 без включения pairing, characteristic read/write или subscription. Success принят; CAP-051 остаётся active до retained fail-closed paths. Located/timed export CAP-050 всё ещё требует отдельно принадлежащего non-conflicting GPS profile, а physical HTTP parity — client, который не затрагивает активный Wi-Fi Mac.
