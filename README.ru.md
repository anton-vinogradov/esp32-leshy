...
- **Проверенный checkpoint:** host/build checkpoint `E-CORR-005` начинает S6.4 первым исполнимым on-device slice «Цели». Home включает его только при наличии сохранённого источника Session; открытие повторно идентифицирует exact enrolled CID, монтирует product store read-only и восстанавливает два atomic head без write path. Foreground-only workspace 22 544 B и transient admission scratch 11 272 B выводят не более 16 самых сильных текущих identities, а свободные строки заполняют strongest baseline-only identities, не изменяя обе исходные Sessions. Обычный четырёхстрочный список ставит «Сравнить визиты» первым, сортирует Цели по текущему сигналу, открывает полноэкранные детали с exact source generation и показывает deterministic counts новых/исчезнувших/изменённых/неизменных. Dense-air fixture на 28 identities, monotonic-reset, negative, ASan/UBSan и полный host suite проходят. Production `0.146.0-targets` собирается при 211 296 B static RAM и 3 108 996 B linked flash. Exact `0.145.0-interface-settings` остаётся последней физически принятой baseline board-01 до прохождения focused delta ниже; отложенный RF-positive gate S5 остаётся открыт.
- **Следующий gate:** привязать exact `0.146.0-targets` к committed source, один раз прошить board-01, записать две bounded реальные passive Wi-Fi Sessions, затем автоматически снять Targets List→Compare→Detail и доказать exact generations/CID, bounded strongest-first projection, read-only zero-write behavior, освобождение heap, inactive outputs, zero input drops и final Home/lease 0. Physical gate S5 отложен, но не отменён: когда приедет заказанный replacement DIV, его read-only profile должен пройти до запуска сохранённой one-command матрицы IR→nRF24→OOK→FSK, закрывающей S5.3/S5.4/S5.6.
