...
- **Следующий gate:** реализовать offline-поиск companion и versioned deterministic export поверх уже принятого native-USB contract, с bounded pagination и без сети или прямого storage path. Physical HTTP parity отложена до появления отдельного idle network adapter либо внешнего client; активный Wi-Fi этого Mac исключён. Physical gate S5 остаётся отложенным до приезда replacement DIV и прохождения его read-only profile.
