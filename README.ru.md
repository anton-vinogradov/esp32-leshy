...
- **Проверенный checkpoint:** exact `0.181.0-companion-web-deferred-worker-restore` на source `6e0f2be76240e38d12805cfd654a7d70c61ae3d8` физически принят в `E-BUILD-151`/`E-AUTO-123`/`E-HIL-181`/`E-COMPANION-005`. В Targets→Detail→Actions→Local Web первое явное Right показывает consent overlay без запуска сети, второе Right создаёт один RAM-only credential WPA2/CCMP, listener на одного клиента и ограничения 10 минут idle/30 минут absolute. Runtime Targets и idle worker Survey освобождаются только на время admission Wi-Fi driver на portable profile без PSRAM; Stop уничтожает AP, listener, authorization и credential, сразу восстанавливает Targets, а worker Survey — при выходе из Targets. Exact CID и Session generation 161/59 observations сохраняются read-only, partition preflight совпадает без прошивки partition table, raw radio TX и storage writes равны zero, Cardputer не открывается, final lease равен 0. Два fail-closed firmware precursor сохраняют дефекты allocation Wi-Fi и преждевременного восстановления worker. Этот gate доказывает physical lifecycle connectivity/secrets, но ещё не HTTP-запрос с Mac.
- **Следующий gate:** физически проверить готовые по host offline-поиск companion и versioned deterministic snapshot, переиспользовав exact 0.195 поверх уже принятого native-USB contract. Delta обходит все bounded detail pages Target, проверяет counts/canonical SHA-256 и выполняет локальный поиск без сети, radio TX, writes storage устройства и повторной прошивки. Physical HTTP parity отложена до появления отдельного idle network adapter либо внешнего client; активный Wi-Fi этого Mac исключён. Physical gate S5 остаётся отложенным до приезда replacement DIV и прохождения его read-only profile.
