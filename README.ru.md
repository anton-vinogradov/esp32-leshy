...
- **Проверенный checkpoint:** `E-AUTO-220`/`E-HIL-238`/`E-EVIDENCE-002`/`RB-M254` принимают полную поддерживаемую цепочку `WF-11` на повторно использованном exact physical `1.0.0-dev.369`: понятное новичку объяснение проверки пароля ведёт через bounded запись своей Wi-Fi, atomic save, cold reopen в Библиотеке и canonical export `WPA*02`; затем `check_my_wifi_password.py` проверяет локальный common/vendor-default corpus с явным разрешением, конечными budgets, checkpoint/resume и privacy-safe отчётом. Анонимизированный physical fixture корректно остаётся negative control, официальный публичный PMKID-вектор Hashcat служит независимым positive control. Ни один report не сохраняет plaintext, raw evidence или identity сети; computer-side device/network/radio side effects равны zero, cleanup достигает Home/none/lease 0. Exact dev.357 остаётся periodic full anchor.
