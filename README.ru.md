...
- **Проверенный checkpoint:** `E-BUILD-196`/`E-AUTO-171`/`E-HIL-208`/`E-STORAGE-035`/`RB-M207` принимают exact physical `1.0.0-dev.280` — persistence/retry slice Device Lock CAP-052. На original board-01 изолированный disposable namespace NVS enroll-ит временный PIN, cold-restore-ит locked generation 1, сохраняет полные post-KDF задержки 5 и 15 секунд в generations 2/3, cold-restore-ит retry delay, unlock-ит generation 4 и явно очищает fixture. Два cold start доказывают, что pending cleanup нельзя забыть; product namespace остаётся virgin до, во время и после теста. Raw PIN, digest, whole-NVS copy, запись product state, radio, clone, Cardputer и Wi-Fi Mac не затронуты; final errors/drops input равны нулю, cleanup возвращает Home/none/lease 0.
- **Следующий gate:** физически доказать переход `recovery_only` после пятого неверного PIN и ordering destructive recovery, затем провести safe Stop/panic/update/factory reset и protected actions через access matrix без раскрытия protected content. Authenticated encryption at rest остаётся отдельным gate. Located/timed export CAP-050 всё ещё требует отдельно принадлежащего non-conflicting GPS profile; physical HTTP parity — dedicated client, который не затрагивает active Wi-Fi Mac или Cardputer; deferred S5 RF carrier gate остаётся заблокирован replacement hardware.
...
| **Active-confirmed** | **Блокировка устройства (CF-006):** local PIN, bounded retry/recovery и защита secrets/evidence без блокировки Stop, panic и recovery | 🟡 exact dev.280 физически принимает enrollment, cold restore locked/retry, полные post-KDF задержки 5/15 секунд и isolated cleanup; остаются recovery-only, action admission и encryption, S7 |
