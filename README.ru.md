...
- **Проверенный checkpoint:** `E-BUILD-178`/`E-AUTO-152`/`E-HIL-193`/`E-UX-057`/`RB-M189` физически принимают exact `1.0.0-dev.250` на оригинальном board-01, source `bfe646e4d9408b4cd0ec1dc58c7c4e9c38a4ac0d`. Общий Bluetooth renderer карточки сигнала теперь имеет delta-path, а bounded invalidation остаётся ответственностью экранов list/detail: за один live interval список перерисовал ровно 2 из 4 видимых строк без очистки всего content; карточка выбранного устройства изменила 929 pixels радара и zero static/chrome pixels, выполнив один delta repaint без нового full repaint или content clear. Passive BLE, zero driver drops, exact CID, автоматические screenshots и final Home/none/lease 0 проходят. Candidate включает принятую implementation persistence/export CAP-049 из dev.249, но этот focused repaint delta не заявляет physical acceptance Save/reopen/export.
- **Следующий gate:** переиспользовать exact image `1.0.0-dev.250`, уже прошитый в оригинальный DIV, и доказать явный Save, atomic commit SD, exact-generation cold reopen и экспорт PCAP; готовность `.hc22000` дополнительно обязана быть доказана полезным bounded PMKID или replay-consistent M1→M2 evidence, а ambient run без evidence должен оставаться честным и неэкспортируемым. Stop/commit/cold-recovery Product Survey остаётся отдельным pending после того, как прежняя попытка `running_degraded` fail closed до commit. Physical HTTP parity остаётся отложенным до dedicated client, не затрагивающего активный Wi-Fi Mac.
