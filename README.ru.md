...
- **Следующий gate:** `FF-2` остаётся активен. Exact physical `1.0.0-dev.343` (`E-BUILD-222`/`E-AUTO-198`/`E-HIL-226`/`E-UX-077`/`RB-M234`) принят владельцем: список Bluetooth и открытая карточка больше не мерцают. Host/build `1.0.0-dev.346` (`E-BUILD-224`/`E-AUTO-200`/`E-UX-079`/`RB-M236`) распространяет фиксированное контекстное дерево 62 возможностей с Wi-Fi на Bluetooth, 2,4 ГГц, Sub-GHz и Библиотеку: пользователь выбирает объект и понятную задачу, а GATT/OOK/FSK/CC1101/NRF24 остаются деталями реализации. Подсказки в карточке Bluetooth и записи радиоданных теперь честно ведут к Действиям. Затем остаются Protocol Workbench, честное empty state Lab и workflow screenshot → protected Library → USB export → cold reopen того же generation. RF TX запрещён.
