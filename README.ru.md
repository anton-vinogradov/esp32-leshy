...
- **Следующий gate:** `FF-2` остаётся активен. Exact physical `1.0.0-dev.343` (`E-BUILD-222`/`E-AUTO-198`/`E-HIL-226`/`E-UX-077`/`RB-M234`) принят владельцем: список Bluetooth и открытая карточка больше не мерцают. Host/build `1.0.0-dev.345` (`E-BUILD-223`/`E-AUTO-199`/`E-UX-078`/`RB-M235`) теперь фиксирует 62 возможности продукта без 62 пунктов меню: девять областей Home ведут через карточки объектов к контекстным задачам и одному общему guided-flow. Первый Wi-Fi slice показывает понятное объяснение проверки пароля до начала любой записи; обычный пользовательский путь больше не требует терминов реализации. Live-сортировка Wi-Fi продолжается после действий пользователя, а focus остаётся на выбранной identity. Затем остаются Protocol Workbench, честное empty state Lab и workflow screenshot → protected Library → USB export → cold reopen того же generation. RF TX запрещён.
