...
- **Проверенный checkpoint:** `E-BUILD-185`/`E-AUTO-160`/`E-HIL-199`/`RB-M196` принимают lifecycle визита Field Survey exact `1.0.0-dev.262` на оригинальном board-01, source `99aacd01336a065e18b52035ec243e2eb47abd92`. Preflight и оба committed visit автоматически переходят в pause ровно после одного прохода Wi-Fi+BLE с zero drops; generations идут 170→171→172, revisit сравнивает 52 текущие identity с baseline 46 как 42 seen again, 10 new и 4 missing, а deterministic incomplete input остаётся явно некоммиттабельным. Отдельный no-flash/no-scan cold delta восстанавливает exact generation 172/52 read-only с zero physical/blocked writes и final Home/none/lease 0. Retained precursor dev.260 81→64/17 drops теперь корректно классифицирован как cumulative pressure 11 повторных Wi-Fi cycles при недоступном BLE, а не один dense scan.
- **Следующий gate:** направить уже принятый bounded результат Field Survey в native Library/export и честный local WiGLE 1.6 output, затем добавить live passive observations Wi-Fi stations. Optional trusted GPS/UTC остаётся отдельным финальным slice CAP-050. Physical HTTP parity остаётся отложенным до dedicated client, не затрагивающего активный Wi-Fi Mac.
...
| **Passive audit** | **Полевой обзор (CF-003):** Wi-Fi AP/station и BLE с deduplication, сравнением повторного визита, optional GPS track и локальным WiGLE-compatible экспортом | 🟡 в работе, S7: есть setup/result UI, bounded catalog и exact preflight Wi-Fi→BLE; persistence first/revisit, routing export, stations и GPS остаются открыты |
