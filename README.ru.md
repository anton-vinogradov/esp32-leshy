...
- **Проверенный checkpoint:** exact `0.172.0-companion-target-mutate` на firmware source `6ec3a198562c2cffc998b18bbd5e0738dcae3428` и verification source `48d296537a8eb358663420918b19151e2aa19c09` физически принят в `E-BUILD-149`/`E-AUTO-121`/`E-HIL-180`/`E-COMPANION-003`. Тот же scoped companion по native USB теперь открывает чтение Target и Favorite/Name/Notes/Tag add/remove через preview→одноразовый confirm→status, exact optimistic revision и существующий supervised exact-CID atomic worker. Physical delta переключает один стабильный Target false→true→false, продвигает revision 10→11→12 и generation Target-state 15→16→17, затем cold-reopen-ит восстановленное значение. Оба commit используют 3 writes/3 file syncs/3 directory syncs; stale/no-op/unknown/replayed/changed-token/post-exit/Home случаи fail-close-ятся. Exact CID и Session generation 161/59 observations сохраняются, heap возвращается к 91 068 B, TX/input drops/discovery/Cardputer opens равны zero, final — Home/none/lease 0. Два rejected precursors остаются видимыми: неверное ожидание навигации runner и stale descriptor native USB в macOS; ни один не является отказом firmware. Reset helper теперь закрывается до re-enumeration ESP32-S3 и подключается заново к exact port; принятый rerun не требует physical power cycle.
- **Следующий gate:** связать принятый на host/build local Web presentation с явно разрешаемым пользователем lifecycle local connectivity без ambient access, persistent cleartext secrets или более широких grants, затем физически доказать parity USB/Web на оригинальном DIV. Physical gate S5 остаётся отложенным до приезда replacement DIV и прохождения его read-only profile.
