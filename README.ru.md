...
- **Проверенный checkpoint:** `E-BUILD-189`/`E-AUTO-164`/`RB-M200` принимают host/build foundation CAP-051 BLE Inspector в exact `1.0.0-dev.268`, source `52b2d1655b486ae029ce2402317c2f270bf88c0c`. Passive boundary теперь передаёт и копирует exact compatible legacy advertisement bytes только одной выбранной identity. Connected GATT не имеет surface pairing/read/write/subscribe: ему нужны свежий exact target, явные enumeration-only permission и confirmation token, после чего он держит отдельный ESP-RF lease до deterministic disconnect. Wrong identity, malformed/overflowed facts, timeout и failed cleanup остаются fail closed.
- **Следующий gate:** подключить bounded raw capture к workflow выбранного Bluetooth device и versioned export, затем реализовать platform adapter GATT enumeration и видимый UI permission/cleanup до focused physical HIL. Dev.268 — только foundation: product screen и live connection ещё отсутствуют. Physical located/timed export CAP-050 отложен до separately owned non-conflicting GPS profile; physical HTTP parity всё ещё требует client, не затрагивающий активный Wi-Fi Mac.
...
| **Passive audit** | **BLE Inspector, приём (CF-004):** сохраняет совместимые raw advertising records с provenance; это не обещание произвольного BLE link-layer sniffing | 🟡 host/build foundation в exact `1.0.0-dev.268`: bounded exact selected-target payload retention принято; product capture/export и physical HIL остаются открыты |
