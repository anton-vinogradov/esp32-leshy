...
- **Следующий gate:** повторить одно-командный exact flow `0.137.0-pulse-store-deadline` с fixture `0.2.5-shared-pin-safe`: два fixed NEC emission, normal IR save, injected pre-storage stall 10 s, trip 8 s, retained restart, recovery block, двухшаговый clear и final zero lease. Первый exact attempt и независимый regression на ранее принятом scenario отклонены: board-01 не увидела ни одного IR transition, хотя fixture `0.2.4` отчиталась о завершённой emission 68 ms. Review обнаружил, что GPIO14 одновременно моделировался как LEDC IR TX и как независимо управляемый CE третьего nRF; `0.2.5` оставляет у физически общего pad единственного LEDC-safe owner и добавляет source guard. Пока fresh-flash regression не пройдёт, это candidate cause, а не physical acceptance. Worker Sub-GHz пока покрыт только source/native; его positive physical gate и приостановленный nRF gate S5.3 всё ещё требуют исправного или заменённого RF carrier/device с plausible read-only identities.
...
| S5.5 | Полнота runtime: exact 0.136 принимает preparation/admission Product Survey, калиброванные workers Wi-Fi+BLE и Wi-Fi Capture Store; source/native/automation IR/Sub-GHz Store 0.137 готовы; первый two-board IR attempt отклонён, fixture 0.2.5 исправляет shared ownership GPIO14 IR/CE3 перед fresh physical rerun; low-voltage safe-write, sleep/resume и применимые явные assembly profiles GPS/PN532 остаются | 🟡 в работе |
