...
- **Проверенный checkpoint:** `E-BUILD-212`/`E-AUTO-187`/`E-HIL-220`/`E-UX-071`/`RB-M223` принимают первый physical slice `FF-1` на original board-01. Exact `1.0.0-dev.312` на firmware source `a2964985eff8bd33d8c429d4ec37350446d56834` сохраняет identity и канал выбранного passive Wi-Fi client стабильными при росте signal samples 29→32. Detail screen выполняет один initial full render, затем только atomic live-row/radar deltas: identity и chrome меняются на 0 pixels, live data — на 49 pixels, без новых content clear/full repaint. Два полных monitor lifecycle сохраняют heap byte-invariant, Device Lock использует и удаляет isolated HIL namespace, storage остаётся 8/54 с zero writes, cleanup завершает Home/none/lease 0. Exact dev.302 остаётся periodic full anchor.
- **Следующий gate:** завершить оставшуюся delta Targets Radar/localize в `FF-1` и провести согласованный interaction review Wi-Fi/BLE/Targets. Ранее принятые карточки Wi-Fi network и BLE остаются входами этого review; calibrated distance не заявляется. Automation/HID остаётся заморожен на принятом безопасном public-trust checkpoint dev.308 при отключённом execution; positive Serial Console traffic всё ещё ждёт explicitly reviewed no-RF fixture `mux56-3v3`, а optional GPS, physical HTTP parity и deferred S5 RF carrier gate остаются externally blocked.
...
| FF-1 | Radar Wi-Fi/BLE принят; завершить Targets/localize и согласованный cross-radio interaction review `FUNC-17` | 🟡 в работе |
