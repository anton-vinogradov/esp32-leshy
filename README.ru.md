...
- **Текущая фаза:** `S6.5 — functional-first пользовательские срезы`; сейчас Wi-Fi UX и проверки на собственных устройствах. ИК и другие радио отложены, а не завершены.
...
- **Проверенный checkpoint:** Dev.390: 13 снимков TFT; окно без имени 20,001 с, имя из кадра AP без подмены отсчётов сигнала, touch Stop через 1,056 с, свежие отсчёты 3→4 и 4→5 после возврата скана. Таймер: 26 динамических / ноль статических пикселей. Таймер AP: 60,314 с. Оба Home/none/lease 0, safety armed, без потерь scan-очереди и операций SD. Перед cleanup отдельная история на 64 записи удержала 64/747 наблюдений (683 capacity drops); это не запись без потерь, полная matrix или оптическая приёмка.
- **Следующий gate:** Прогресс **24/62**, Wi-Fi **17/21 готово, 4/21 осталось**. Переподключить питание оригинала, без перепрошивки проверить точный runtime dev.391, затем один раз установить тот же обычный образ на приёмник. Выполнить дельту пароля/приёмника, затем проверить клиентскую ветку своим телефоном. Ответ AP на probe может прийти раньше association: первый источник сохраняется, клиентский кадр отмечается отдельно. Причина молчания USB не установлена; Wi-Fi ноутбука не менялся.
...
| FF-5 | Поставить `FUNC-34` IR replay из одного выбранного immutable Capture с preview, confirmation и доказанным Stop/timeout; host/build основа принята на dev.373; replacement board-03 прибыла, optical IR qualification и exact two-board admission ещё открыты | 🟡 в работе |
...
| Privacy Identity рандомизирует STA/AP Leshy и даёт ephemeral provenance-labeled synthetic lab identities из owned Captures | S7 | 🟡 в работе |
