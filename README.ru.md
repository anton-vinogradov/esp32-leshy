...
- **Следующий gate:** выполнить exact `GET /` и общий `POST /api/v1/companion` через временную AP, сравнить те же projections Session/Target/Compare и confirmed mutation с native USB, затем восстановить прежнее состояние сети Mac. Physical gate S5 остаётся отложенным до приезда replacement DIV и прохождения его read-only profile.
