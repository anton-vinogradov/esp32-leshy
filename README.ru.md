...
- **Проверенный checkpoint:** `E-BUILD-199`/`E-AUTO-174`/`RB-M210` принимают host/build foundation CAP-053 в exact `1.0.0-dev.284` на source `82453d60c17647d894c8f988c35d9cb412883669`. Typed dispatcher проверяет schema, authorization, permission, hardware capability, fresh confirmation и atomic ownership `Console`+`Mux56` до запуска action. Общий CLI предоставляет только preview/run/status/cancel над named profiles; duplicate/unknown/missing fields, unsupported baud, raw GPIO и execution без confirmation fail closed. Этот checkpoint не заявляет platform UART adapter, product UI или physical serial traffic.
- **Следующий gate:** добавить bounded Arduino UART adapter, RX ring и UI Устройство → Serial Console над принятым Action boundary, затем физически принять receive-only negative/cleanup path на stock RF-shield и positive path только на отдельно объявленном no-RF fixture `mux56-3v3`. Located/timed export CAP-050 всё ещё требует отдельно принадлежащего non-conflicting GPS profile; physical HTTP parity — dedicated client, который не затрагивает active Wi-Fi Mac или Cardputer; deferred S5 RF carrier gate остаётся заблокирован replacement hardware.
