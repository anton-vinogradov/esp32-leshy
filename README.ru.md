...
- **Проверенный checkpoint:** host/build checkpoint `E-CORR-001` создаёт первый slice S6.2. Bounded proposal связывает одну ещё не принадлежащую Target exact radio identity/evidence с одним Target, показывает фиксированный вклад каждого feature и явный confidence `Low`/`Medium`/`High`/`Stale`, а само предложение никогда не выполняет mutation. Candidate и объясняющее evidence обязаны открывать exact сохранённые Observations. Versioned Actions `correlation.accept` и `correlation.reject` требуют явного решения, повторно проверяют evidence и optimistic revision Target и пишут immutable bounded decision record; пути automatic merge нет. Native, ASan/UBSan и production builds проходят. Decision log пока не persistent; merge/split, compare и UI явно не заявляются. Exact `0.145.0-interface-settings` остаётся последней физически принятой product baseline board-01; отложенный RF-positive gate S5 остаётся открыт.
- **Следующий gate:** атомарно сохранять correlation decisions вместе с каталогом Target, затем реализовать обратимую историю merge/split и доказать, что merge→split восстанавливает exact предыдущий graph identity/evidence без изменения source Observations. Physical gate S5 отложен, но не отменён: когда приедет заказанный replacement DIV, его read-only profile должен пройти до запуска сохранённой one-command матрицы IR→nRF24→OOK→FSK, закрывающей S5.3/S5.4/S5.6.
