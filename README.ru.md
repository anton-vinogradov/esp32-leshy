...
- **Проверенный checkpoint:** `E-BUILD-190`/`E-AUTO-165`/`E-HIL-203`/`E-UX-060`/`RB-M201` физически принимают receive-only product path CAP-051 BLE Inspector в exact `1.0.0-dev.270`, source `cd542afac79f696a2cdcf3d7abe4bd18f3bc4a82`. Из карточки выбранного Bluetooth device открываются «RAW пакеты»; bounded capture на 32 records сохраняет только exact selected identity, обновляет строки без очистки content, freeze-ится явно или по capacity и выдаёт versioned local export. Один fresh-flash gate удерживает Bluetooth route 15 265 ms, принимает два selected records/36 bytes с zero invalid/drop/allocation/fallback counters, автоматически проверяет четыре TFT frames и выходит Home/none/lease 0 с неактивным buzzer. Отклонённый predecessor dev.269 сохранён fail closed: его resident capture 1 856 B выбрасывал route в Home до live scan; dev.270 делит это storage с взаимоисключающим event Airspace Guard и возвращает NimBLE headroom.
- **Следующий gate:** реализовать live platform adapter GATT enumeration и видимый UI selected-target permission/confirmation/cleanup, затем физически доказать success, wrong-peer, timeout, resource-conflict и failed-cleanup paths. Pairing, characteristic read/write и subscription остаются вне этого slice. Passive path принят, но live BLE telemetry свободной heap после заполнения catalog достигает лишь 572 B/500 B largest block, поэтому connected adapter сначала обязан освободить или переиспользовать память passive scan и заново измерить bounded lifecycle. Physical located/timed export CAP-050 отложен до separately owned non-conflicting GPS profile; physical HTTP parity всё ещё требует client, не затрагивающий активный Wi-Fi Mac.
...
| BLE Inspector сохраняет raw compatible packets и входит в connected GATT только после explicit target/permission/lease confirmation | S7 | 🟡 в работе |
...
| **Active-confirmed** | **BLE Inspector, GATT (CF-004):** подключается только к явно выбранному устройству и перечисляет services/characteristics под отдельными permission и lease | 🟡 host/build foundation в exact `1.0.0-dev.268`: enumeration-only state machine permission/lease/cleanup принята; live transport, UI и physical HIL остаются открыты после acceptance passive path dev.270 |
