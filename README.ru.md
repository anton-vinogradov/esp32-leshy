...
- **Проверенный checkpoint:** exact `0.151.2-targets-favorite-compact` на source `02c55b61b0ec5e1efc82efce5423b5c34bf82bcb` физически принят в `E-AUTO-109`/`E-HIL-169`/`E-UX-046`. «Цель»→«Действия» переключает «Избранное» через typed Action boundary, повторно идентифицирует exact CID, атомарно публикует catalog-only schema-v3 generation 1→2 с 3 writes/3 file syncs/3 directory syncs и после cold reset открывает тот же 128-bit Target с изменённым значением. UI acknowledgement занимает 148 µs, supervised worker — 2,690 s; pre-mount workspace 16 КиБ помещается в largest block 34 804 B, heap возвращается к 97 012 B, финал — Home/none/lease 0. Два реальных отказа выделения 32 КиБ сохранены с zero writes; третий rejected run фиксирует исправную mutation, но ранний порядок наблюдения старого HIL runner. Отложенный RF-positive gate S5 открыт.
- **Следующий gate:** добавить следующий полный bounded metadata interaction — сначала имя, затем tags/notes — на том же exact-CID atomic path с сохранением выбранного Target. Далее идут correlation accept/reject; merge/split остаётся за явным confirmation с preview изменения ownership и сохранением reversal. Physical gate S5 остаётся отложенным до приезда replacement DIV и его read-only profile.
