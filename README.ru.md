...
- **Проверенный checkpoint:** `E-BUILD-176`/`E-AUTO-150`/`E-HIL-192`/`E-UX-056`/`RB-M187` принимают exact physical `1.0.0-dev.248` на source `e62599c1827e249845105405797cb75aedaa5226` на оригинальном DIV и exact CID `FE343253440000002000000055019CB7`. Fresh-flash smoke верхнего меню открывает все восемь пунктов Home, удерживает Bluetooth 15 секунд и возвращает каждый path в Home/none/lease 0. Прямой passive BLE regression завершает два lifecycle с 32 unique devices, strongest-first rows, offline company facts, identity-stable live radar, zero driver drops, exact bounded retention accounting и invariant heap. Fresh-flash CAP-049 затем завершает real ambient RX, автоматическую navigation `Результат → Действия → Устройство → Доказательства → Детали`, Повторить, Up/Down, Right/OK, Left/Back, synthetic golden evidence и replay rejection без управления Wi-Fi Mac, connect, raw TX, manual input или leaked lease. В ambient traffic не было EAPOL/PMKID, поэтому честный result остаётся `inconclusive`; это не заявляет полезный authentication evidence или persistence/export. Retained bundle CAP, acceptance pins и pins regressions входа/BLE в `tests/hil/evidence/` связывают exact image и runner hashes.
- **Следующий gate:** реализовать standard serializer authentication artifact, подключить provenance schema 8 к существующему atomic product persistence/export path и проверить его полезным bounded EAPOL/PMKID evidence. Stop/commit/cold-recovery Product Survey остаётся отдельным pending после того, как прежняя попытка `running_degraded` fail closed до commit. Physical HTTP parity остаётся отложенным до dedicated client, не затрагивающего активный Wi-Fi Mac.
