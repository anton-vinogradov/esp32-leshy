...
- **Проверенный checkpoint:** `E-BUILD-202`/`E-AUTO-177`/`E-HIL-212`/`E-UX-067`/`RB-M213` принимают exact physical `1.0.0-dev.288` на source `3d232d2e5aff8e914ccb7e7414194c1ce7e89bbf`. Compact read-only product route Лаборатория → Automation Inspector подключён без path Action/HID/resource, все девять верхнеуровневых route проходят автоматически. Регрессия первого запуска BLE исправлена compact только idle observation queue 64→32 перед NimBLE и restore 32→64 при выходе: 59 ready samples за 15 s сохраняют минимум 76 768 B free/31 732 B largest до init и 4 992/2 932 B после; 122 advertisements имеют zero driver drops и high-water 3/32. Rejected predecessor dev.287 пересекает UI revision 85→0 до запуска `ble_begin`. Privacy-minimal `E-HIL-212` сохраняет обе границы и final Home/none/lease 0.
- **Следующий gate:** 15-я принятая focused delta делает periodic full current-capability checkpoint обязательным. После сброса cadence физически провести bounded malformed/unsigned `.lhau` через nested Automation Inspector и сохранить стабильные EN/RU summary frames при zero output Action/HID/resource. Реальный P-256 trust store и каждый execution path остаются более поздними отдельными gates. Positive Serial Console traffic всё ещё ждёт explicitly reviewed no-RF fixture `mux56-3v3`; optional GPS, physical HTTP parity и deferred S5 RF carrier gate остаются externally blocked.
...
| **Passive audit** | **BadUSB inspection (CF-008):** разбирает подписанный Automation/HID script и показывает target, permissions, действия и пределы без исполнения | 🟡 exact host/build dev.286 принимает bounded zero-output parser и foundation admission; дальше compact product UI Inspector, real trust и execution не подключены |
...
| **Active-confirmed** | **Блокировка устройства (CF-006):** local PIN, bounded retry/recovery и защита secrets/evidence без блокировки Stop, panic и recovery | ✅ exact dev.283 физически принимает authenticated encrypted product storage и read-only cold reopen exact CID после принятого recovery/admission dev.281; CAP-052 завершён, S7 |
...
| **Active-confirmed** | **Automation/HID execution (CF-008):** выполняет только подписанный permissioned script после preview цели, действий, ceilings и конечной длительности | ⬜ запланировано, S7; dev.286 намеренно не имеет HID/Action output path |
