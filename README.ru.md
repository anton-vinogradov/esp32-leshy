...
- **Проверенный checkpoint:** `E-BUILD-198`/`E-AUTO-173`/`E-HIL-210`/`E-STORAGE-037`/`RB-M209` принимают exact physical `1.0.0-dev.283` и закрывают implementation boundary Device Lock CAP-052. Один normal product commit на original board-01 публикует encrypted generation 3 с валидным chunked envelope `LENC` AES-256-GCM: 615 plaintext bytes превращаются в 695 physical bytes, header и exact size валидны, ciphertext отличается от известного plaintext, catalog проходит authentication. Предыдущая generation 2 cold-reopen-ится read-only с exact CID и zero physical writes. Queues/drops, radio connect/raw-TX calls и leaked leases равны нулю; final state — Home/none/lease 0, safety armed. Retained watchdog failure dev.282 доказывает fail-closed reset/zero outputs, precursor dev.283 отклонён только из-за исправленного oracle token `ok` против `FR_OK`.
- **Следующий gate:** начать следующий незаблокированный implementation slice S7 — CAP-053 bounded Serial Console/shared Actions CLI; release hardening CAP-052 переносится в последующую matrix power-cut/signed-update/privacy. Located/timed export CAP-050 всё ещё требует отдельно принадлежащего non-conflicting GPS profile; physical HTTP parity — dedicated client, который не затрагивает active Wi-Fi Mac или Cardputer; deferred S5 RF carrier gate остаётся заблокирован replacement hardware.
