...
- **Проверенный checkpoint:** `E-BUILD-211`/`E-AUTO-186`/`E-HIL-219`/`E-UX-070`/`RB-M222` закрывают `FF-0` на original board-01. Exact dev.308 автоматически проходит все девять passive top-level routes и снимает реальные TFT states; focused dev.309 исправляет пустое состояние Automation, а exact `1.0.0-dev.311` на source `65402c4f2de77a8568e07e0e14ea9382d6123589` доказывает честное пустое состояние Targets после одной успешной exact-CID identity/mount попытки. Сырые кадры окружающих Wi-Fi/BLE сохраняются только хэшами; два безопасных исправленных кадра, exact lineage candidate/runner, cleanup и визуально отклонённый predecessor dev.310 с переносом machine checked. Exact dev.302 остаётся periodic full anchor.
- **Следующий gate:** поставить и физически принять первый vertical slice `FF-1`/`FUNC-17` Radar/localize со стабильным по identity выбором, честной историей RSSI/trend/range и content-only repaint. Automation/HID остаётся заморожен на принятом безопасном public-trust checkpoint dev.308 при отключённом execution; positive Serial Console traffic всё ещё ждёт explicitly reviewed no-RF fixture `mux56-3v3`, а optional GPS, physical HTTP parity и deferred S5 RF carrier gate остаются externally blocked.
...
| FF-1 | Завершить `FUNC-17` Radar/localize для Wi-Fi, BLE и Targets как одно согласованное interaction | 🟡 в работе |
