...
- **Проверенный checkpoint:** exact `0.149.0-targets-inplace-reset` на source `8d4a2e86d88807f0de21d5afd9680912d8b7797f` физически принят в `E-AUTO-107`/`E-HIL-167`/`E-UX-044`. Одна exact-прошивка сначала read-only открывает существующие поколения 111/112 и доказывает List→Compare с 16 Targets, mount error 0, zero writes/TX/drops и полным возвратом heap. Затем те же flashed bytes создают два реальных passive Wi-Fi визита поколений 113/114 с 21/18 observations и открывают List→Compare→Detail; все 16 строк классифицированы как 2 added + 14 unchanged. Пять TFT frames, exact CID, ELF stack frames 256/416/32/80/1 104 B, safe outputs и final Home/none/lease 0 сохранены machine-checked bundle. Три precursor failure 0.146/0.147/0.148 остаются видимыми как `E-HIL-164…166`; отложенный RF-positive gate S5 открыт.
- **Следующий gate:** завершить оставшуюся product-глубину S6.4: Compare становится стабильным class-sorted списком строк, каждая строка открывает exact baseline/current Observation и объясняет change mask, затем bounded metadata/correlation decisions используют существующие typed Actions без сокрытия source evidence. Сначала реализовать и host-review interaction строк/evidence; потратить одну delta HIL только после готовности полного slice 0.150. Physical gate S5 остаётся отложенным до приезда replacement DIV и его read-only profile.
