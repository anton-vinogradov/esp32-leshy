...
- **Следующий gate:** Прогресс **24/62**, Wi-Fi **17/21 готово, 4/21 осталось**. Нужно физически переподключить оригинальный board-01: после вывода SD retry перестали отвечать консоль и загрузчик dev.386, USB-reset их не восстановил. Dev.387 не записана: соединение оборвалось до flash. Board-03 не менялся, на нём dev.385. Сначала восстановить и проверить обычную загрузку, затем поставить одинаковую штатную прошивку на оба DIV и проверить AP-имя через меню. Первопричина ещё не подтверждена; двухплатная приёмка и клиентское обогащение скрытого имени не заявляются. Wi-Fi ноутбука не используется.
