...
- **Проверенный checkpoint:** `E-BUILD-236`/`E-AUTO-214`/`E-HIL-232`/`E-UX-087`/`RB-M248` принимают exact physical `1.0.0-dev.364`: понятный Wi-Fi Security Advisor. Одна fresh flash доказывает strongest-first live sorting, user-owned identity-stable focus, bounded repaint только строки сигнала, pixel-stable advisor без мерцания, два Wi-Fi lifecycle без drift heap и final Home/none/lease 0. Проверенный русский экран честно сообщает, что PMF не измерен, и каждое действие помещается в одну строку. Exact dev.357 остаётся periodic full anchor.
- **Следующий gate:** progress остаётся **23/62**, отдельный фиксированный Wi-Fi track теперь **11/21 готово, 10/21 осталось**. `WF-12` физически принят с privacy-minimal retained evidence; ambient SSID/BSSID/vendor и raw screenshots не сохраняются. Следующая focused delta — `WF-13`: именованные профили чувствительности Airspace Guard с понятными последствиями без ослабления существующей receive-only, evidence-first и fail-closed policy. Companion verification chain `WF-11` остаётся active параллельно; physical gates второй платы и dedicated client отложены, RF TX запрещён.
