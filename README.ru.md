...
- **Проверенный checkpoint:** `E-BUILD-204`/`E-AUTO-179`/`E-HIL-213`/`E-SAFETY-008`/`E-STORAGE-038`/`E-SURVEY-019`/`RB-M215` принимают exact physical `1.0.0-dev.301` на source `d34a9b0c6f61d43b462b1a638ce1c7781fe77947`. Существующий supervised worker Product Survey теперь владеет reopen exact media, atomic terminal commit и cleanup, поэтому UI task остаётся responsive для своего Task WDT. Два последовательных mixed pass Wi-Fi+BLE commits generations 7 и 8 по 54 observations с zero drops Wi-Fi/BLE/pipeline, exact CID и final Home/none/lease 0. Worker supervision завершает run armed, с 2 263 heartbeats и zero expiry/trip. Exact dev.299 и dev.300 сохраняются как fail-closed evidence для synchronous commit в UI task и для interstage feeds, которые не сделали один долгий SD primitive безопасным.
