...
- **Проверенный checkpoint:** `E-BUILD-179`/`E-AUTO-153`/`E-HIL-194`/`E-UX-058`/`RB-M190` физически принимают exact `1.0.0-dev.251` на оригинальном board-01, source `d84f8259c6781dcbe90ae00fba00f0c6f4379c32`. Карточка Bluetooth-устройства теперь сначала собирает каждую меняющуюся строку quality/dBm, range и trend в одном переиспользуемом монохромном sprite 648 B, затем одним push отправляет её на дисплей. Live update выполняет три атомарных push строк, один delta repaint радара, zero full/content clears, zero failures allocation sprite и zero direct fallbacks; меняются 805 pixels радара при неизменном static chrome. Passive BLE, zero driver drops, exact CID, два полных lifecycle, invariant heap после cleanup и final Home/none/lease 0 проходят.
- **Следующий gate:** переиспользовать exact image `1.0.0-dev.251`, уже прошитый в оригинальный DIV, и доказать явный Save, atomic commit SD, exact-generation cold reopen и экспорт PCAP; готовность `.hc22000` дополнительно обязана быть доказана полезным bounded PMKID или replay-consistent M1→M2 evidence, а ambient run без evidence должен оставаться честным и неэкспортируемым. Stop/commit/cold-recovery Product Survey остаётся отдельным pending после того, как прежняя попытка `running_degraded` fail closed до commit. Physical HTTP parity остаётся отложенным до dedicated client, не затрагивающего активный Wi-Fi Mac.
