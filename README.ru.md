...
- **Проверенный checkpoint:** `E-BUILD-246`/`E-AUTO-225`/`E-HIL-242`/`E-UX-093`/`RB-M259` принимают exact physical `1.0.0-dev.377`: Home теперь называет область **Wi-Fi** и поясняет **Сети · Устройства · Каналы**, поэтому переход одним действием в меню задач Wi-Fi видим. Из этого меню одно `OK` или одно `Вправо` открывает «Сети рядом». Последующее наблюдение физических клавиш после долгого простоя зарегистрировало оба намеренных действия по дереву с zero input errors, ambiguity и queue drops; потеря первого нажатия не воспроизвелась. Два passive lifecycle сохраняют strongest-first identity focus, zero chrome repaint, zero storage errors и финальный Home/none/lease 0. Fresh precursor честно оставлен rejected старым 2 KiB oracle cold Wi-Fi heap; измеренная одноразовая initialization 7 344 B укладывается в общую политику 8 KiB, оба post-warm endpoint точно равны 59 320 B.
- **Следующий gate:** progress остаётся **24/62**, отдельный фиксированный Wi-Fi track — **17/21 готово, 4/21 осталось**. Wi-Fi продолжается с `WF-18`; two-board gate IR replay `FF-5` отложен до приезда replacement DIV. `WF-18…WF-21` остаются за явным admission изолированного fixture/второй платы; RF TX запрещён.
