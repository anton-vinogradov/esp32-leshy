...
- **Проверенный checkpoint:** `E-BUILD-174`/`E-AUTO-148`/`E-HIL-191`/`RB-M185` сохраняют exact `1.0.0-dev.246` на source `54cf455810c15753220fb2bd0f497381dfabde48` (`firmware 8cdd2c01b9e3c8423d665e39b7a0581d0f5039ae4e96fa617fee934dc7ea3b6e`, embedded app identity `3c4643e014262722c3ef3ce640e2d9964f0df1431ee49cd897d331291927a221`) как первый physical checkpoint workflow CAP-049, переживающий оба product mount в одном boot. Fail-closed precursor dev.245 один раз смонтировал FAT при largest block 31 732 B, затем дошёл до всё ещё доступного drive с нулевой ошибкой SPI/bus, но не смог выделить второй VFS workspace при 17 396 B (`ESP_ERR_NO_MEM` 257, три попытки). Exact analysis ESP-IDF 5.5.4 показал, что неиспользуемый `max_files=5` требовал contiguous VFS workspace 29 512 B; dev.246 закрепляет один сериализованный direct-FatFs file продукта как `max_files=1`/12 968 B, и оба mount проходят с первой попытки и error zero. Retained receive-only run не наблюдает EAPOL/PMKID event, поэтому user result честно остаётся `inconclusive`/evidence-incomplete и FUNC-49 остаётся active; при этом независимо проходят exact candidate/CID, fixed target/channel, content-only repaint, privacy, cleanup без writes/connect/TX и final Home/none/lease 0.
- **Следующий gate:** завершить focused Wi-Fi-only Product Survey Stop/commit/cold-recovery run на тех же bytes dev.246, доказав исправленный второй mount через generation-advancing write и exact-CID reopen. Текущая mixed-source попытка дошла до `running_degraded`, потому что BLE был недоступен, затем fail closed до Stop/commit и очистилась в Home/none/lease 0; это не gate evidence. До заявления полезного authentication evidence CAP-049 всё ещё нужен известный или естественно наблюдаемый EAPOL/PMKID event, а physical HTTP parity остаётся отложенным до dedicated client, не затрагивающего активный Wi-Fi Mac.
