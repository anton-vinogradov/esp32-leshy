...
- **Следующий gate:** physical HTTP parity над принятым contract остаётся отложенной до отдельного idle adapter или внешнего client; активный Wi-Fi ноутбука остаётся запрещённым. До появления такого client продолжаются host-safe завершение S6.5 и подготовка integrated demo S6.6 без ослабления physical gate. Physical gate S5 остаётся отложенным, а не отменённым, до приезда replacement DIV и прохождения его read-only profile.
