...
- **Проверенный checkpoint:** `E-BUILD-206`/`E-AUTO-181`/`E-HIL-215`/`E-UX-068`/`E-STORAGE-039`/`RB-M217` принимают focused physical delta Automation Inspector на exact `1.0.0-dev.303`, firmware source `ca5bf300289b310dd39845a530aa3fe7a2acd9c2`. Одна exact прошивка candidate переиспользуется после трёх прозрачных fail-closed precursors harness. Accepted run создаёт только `malformed.lhau` и `unsigned.lhau` в exact-CID scratch `/leshy-hil/<run-id>` с 2 file и 1 directory barriers, показывает оба результата на EN/RU двумя byte-identical frames каждый, выдаёт zero output Action/HID/resource/RF, удаляет оба файла и isolated fixture Device Lock, восстанавливает русский язык и возвращается Home/none/lease 0. Periodic full anchor текущих возможностей остаётся exact dev.302.
- **Следующий gate:** подключить real verifier P-256 и видимый владельцу workflow enrollment/revocation trust store, включая cold restore и interaction Device Lock, сохраняя execution отключённым. Positive Serial Console traffic всё ещё ждёт explicitly reviewed no-RF fixture `mux56-3v3`; optional GPS, physical HTTP parity и deferred S5 RF carrier gate остаются externally blocked.
