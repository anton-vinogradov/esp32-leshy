...
| R-018 | Fatal fault main loop/worker оставляет software-controlled outputs активными или тихо перезагружается в то же unsafe operation | M | critical | permanent panic Task WDT main loop; IRAM quiesce GPIO2/14/15/47 до diagnostics; exact-app torn-write-resistant RTC latch плюс первый trace CPU/page/stage; no automatic clear; Safe Mode блокирует product workers и normal Actions | safety/platform; exact 0.103 принимает injected main-loop path; ночной exact dev.372 `E-SAFETY-088` сохраняет настоящий reset-reason-6 trip Wi-Fi cycle, а dev.374 ограничивает loop work без recurrence в `E-HIL-240`. Worker heartbeats и independent physical-stop HIL остаются closure work | reduced/open |
