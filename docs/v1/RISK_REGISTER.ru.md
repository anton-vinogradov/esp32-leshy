...
| R-020 | Defensive detector или channel/auth analysis показывает false conclusion как угрозу или proof | M | H | каждый вывод показывает detector/version/threshold/confidence/uncertainty и exact evidence; insufficient fixtures остаются inconclusive; automatic response отсутствует. Exact dev.210 фиксирует эти правила для первого Wi-Fi disconnect-burst detector и статически исключает dependencies driver/TX | analytics/product; `E-BUILD-159`/`E-AUTO-133` — partial control; live golden + negative corpora WF-06-A1/A2 и evidence drilldown всё ещё закрывают risk | reduced/open |
