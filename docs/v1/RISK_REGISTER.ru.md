...
| R-019 | Clone/DNP assembly сохраняет корпус и menu, но меняет module population, shared-radio wiring либо RF front ends; upstream issue #102 независимо сообщает ту же форму отказа всех radios из-за межплатных контактов | H | H | exact assembly profiles; сравнение BOM/photo/ROM; safe read-only identity до любого TX; community reports — corroboration, а не diagnosis; stock firmware не diagnostic fallback из-за full-power carrier paths | boards/RF; rail/continuity map board-02, plausible same-image identity, затем bounded regression | open |
