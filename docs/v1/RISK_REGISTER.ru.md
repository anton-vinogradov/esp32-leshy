...
| R-019 | Clone/DNP assembly сохраняет корпус и menu, но меняет module population, shared-radio wiring либо RF front ends; board-02 имеет valid idle rails, но clamps shared MISO LOW, а upstream issue #102 независимо сообщает ту же форму all-radio failure из-за interboard faults | H | H | exact assembly profiles; сравнение BOM/photo/ROM; safe read-only identity и pull characterization до любого TX; powered-off comparison 23/32 кОм отвергает hard short; stock только manual corroboration; board-02 остаётся RF-fault/no-cross-swap | boards/RF; exact isolated-main pull-characterization, localization clamping half и repaired plausible same-image identity до bounded regression | open |
