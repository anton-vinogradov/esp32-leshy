...
| R-007 | No-PSRAM heap pressure, fragmentation или unbounded queues ломают долгий Survey | H | H | RB-03/04, bounded queues/pools, finite per-loop drains, rate-limited passive data inspection, high-water/drop counters, отсутствие monotonic heap decline | kernel/services; `RB-M257`/`E-HIL-240` принимают exact трёхминутные delta Devices и Networks с zero post-warm drift/drops после ночного WDT; transient minimum 2 980 B остаётся активным pressure; size gate + release endurance с часовым бюджетом и optional extended qualification сохраняются | reduced/open |
...
| R-018 | Fatal fault main loop/worker оставляет software-controlled outputs активными или тихо перезагружается в то же unsafe operation, а причина исчезает до диагностики | M | critical | permanent panic Task WDT main loop; IRAM quiesce GPIO2/14/15/47 до diagnostics; exact-app torn-write-resistant RTC latch; первый safe boot проверяет одну deduplicated durable NVS cause; Safe Mode блокирует product workers/normal Actions и запись SD; после explicit clear exact-CID зеркало SD atomic и opportunistic | safety/platform; exact 0.103 принимает injected main-loop path; ночной exact dev.372 `E-SAFETY-088` сохраняет настоящий reset-reason-6 trip Wi-Fi cycle, а dev.374 ограничивает loop work без recurrence в `E-HIL-240`. Exact dev.376 `E-SAFETY-089` доказывает WDT → NVS → restart dedup → SD mirror → Home и сохраняет отклонённый stack failure dev.375. Worker heartbeats и independent physical-stop HIL остаются closure work | reduced/open |
