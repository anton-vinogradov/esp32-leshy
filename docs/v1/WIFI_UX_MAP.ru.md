...
Документ: **целевой UX; реализованная часть отмечена ниже**.
...
## Уже установлено — 14 сентября, dev.390

Dev.391 **собрана, физически не принята**: экран тестовой сети показывает случайный
пароль WPA2 только на время 60-секундной сессии; Stop/таймер убирает его.
Приёмник отдельно отмечает клиентский кадр, даже если имя первым раскрыл ответ AP.
Обе строки используют retained rendering.
[Деплой заблокирован](../../tests/hil/evidence/wifi-product-network-dev391-console-blocker.json)
молчанием USB оригинала; приёмник остаётся на dev.390.

Сети рядом → краткая карточка → OK поиск по сигналу / → действия →
оценка защиты, подготовка проверки пароля или четыре раздела сведений
(имя, защита, радио, наблюдения). Переходы клавишами/тачем сохраняют выбранную AP.
Подготовка объясняет запись обычного подключения и проверку на компьютере;
→ не запускает запись. Нижние подсказки не являются touch-кнопками.

Установлены общие дельты текста/радара/столбцов и безопасное отображение UTF-8 SSID
с обрезкой по пиксельной ширине. Восемь страниц и график прошли короткий HIL
на точном образе: 13 каналов, неподвижное обрамление, ноль drops/операций SD,
финальный Home/none/lease 0.
[Проверенный машиной итог](../../tests/hil/evidence/wifi-ui-1.0.0-dev.384.json).
Это не оптическая гарантия отсутствия мерцания на всех экранах.

Dev.385 добавляет четыре входа корня; **Наблюдение и запись** содержит обходы
и поиск аномалий. Вход в группу не запускает приём или запись. Десять состояний
переходов и пять снимков TFT прошли [дельту только меню](../../tests/hil/evidence/wifi-menu-1.0.0-dev.385.json):
тач, границы выбора, возврат с сохранением строки и освобождение ресурсов на Home.
Проверка карточек/графика выше — baseline dev.384, не повтор полного прогона.

Dev.388 на обоих DIV добавляет **Устройство → Самопроверка → Проверить Wi-Fi**.
Вход оставляет радио выключенным; явный Start создаёт свою тестовую сеть на
60 секунд. Выбор скрытого/открытого имени и Stop не продлевают таймер.
Второй DIV использует «Сети рядом». Только обнаружение, без интернета,
подключения клиента и SD. [Двухплатный итог](../../tests/hil/evidence/wifi-product-network-1.0.0-dev.388.json):
8 кадров TFT, ручной Stop/таймер, получение и сохранение имени той же AP;
таймер изменил 29 динамических / ноль статических пикселей. Оба DIV завершили
на Home с выключенным радио. Ветка самопроверки показана ниже; интерактивный
макет пока представляет основное дерево Wi-Fi, а не этот новый инструмент.

Dev.390 добавляет **Все сведения → Имя и устройство → Дослушать имя**:
сначала выбранный канал и ограничение 20 секунд, затем OK или настоящая кнопка
«Старт». Вправо только открывает страницу; OK/тач останавливает приём, влево
сначала останавливает, а повторное нажатие возвращает. Подсказка «Пуск/стоп»
не меняется; перерисовываются только изменённые области текста/кнопки.
Результат различает отсутствие имени, источник/подтверждение/конфликт и сбой;
после cleanup продолжается скан той же сети. Проверка имени из реального клиента
ещё отдельная: обещания раскрыть любой скрытый SSID нет.

Пока в проекте: контекстный переход в Lab, пауза радара, физическая проверка
клиентского имени и оставшиеся доработки сквозных мастеров.
«Наблюдения» сейчас объясняет RSSI и его ограничения; нового инспектора трафика
конкретной AP там ещё нет. Перестройка UI не добавляет готовую функцию в счётчик.
...
```mermaid
flowchart TD
    DEV["Устройство"] --> SELF["Самопроверка"] --> TEST["Проверить Wi-Fi"]
    TEST --> START["Создать свою сеть — 60 с / Стоп"]
    START --> CREDENTIAL["dev.391 собрана: пароль WPA2 на экране; без интернета"]
    CREDENTIAL -. "Свой телефон; физический gate открыт" .-> PHONE["Обычное подключение; проверяем услышанный кадр"]
    START -. "Приём на втором DIV" .-> N
    ...
    F --> H["Имя и источник"]
    H --> NL["Дослушать имя: канал / 20 с / OK Старт"]
    NL --> NE["Имя услышано / не услышано / сбой"]
    NE --> NS["Cleanup → продолжить скан той же сети"]
