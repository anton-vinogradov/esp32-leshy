...
| Clean 1.x measurement target | independent pinned build implements BoardProfile, HardwareInventory→AppCatalog projection, unified five-key display/input with non-color focus and a bounded observable PCF8574 boot probe, shared UI components and common-state cards, one EN/RU catalog with generated Roboto Condensed Medium 16/12 Cyrillic fonts and persistent Language selection, on-device Self-Test, buzzer-safe boot output, storage/runtime contracts, guarded SD persistence, app-bound reset-separated boot retries, bounded Product Start raw-identity retry, dual software/hardware boot-recovery watchdog tiers, and a persistent bounded Core-0 Product Survey worker with UI-owned terminal acknowledgement around the interactive exact-CID real passive Wi-Fi→FIFO→persistent SessionStore→cold-boot Library/export path | `E-BUILD-003…064`, `E-HIL-006…087`, `E-AUTO-022…027`, `E-GATE-002`; S2 is closed, exact 0.60 proves continuous Detail progress/generation 67→68 and removes the terminal race, exact 0.62 closes physical active-scan cancellation without commit/leak after retaining the 0.61 input-probe failure, and exact 0.63 closes the selected Cyrillic typography replacement with 254/254 fit and 18-state TFT evidence; missing-source real-TFT, physical power-cut, LittleFS parity, and final independent goldens remain |
...
| [UI automation contract](UI_AUTOMATION.md) | physical and diagnostic input share normalized Actions; actual TFT GRAM becomes reproducible PNG/state evidence without routine operator traversal | `UI-HIL-A1…A7`, `E-HIL-008/082/087`, `E-AUTO-022/027`, `E-UX-008`; the S2 platform gate is accepted, and exact 0.63 proves an idempotent 18-state EN/RU typography lane using the same Stage Demo/release evidence mechanism |
| [Input and accessibility map](UX_ACCESSIBILITY.md) | five physical keys and diagnostic Back share normalized Actions; focus has an outline/chevron and all current status classes remain explicit text rather than color-only signals | `E-BUILD-058/059`, `E-HIL-052/080/081`, `E-UX-006/007`; UX-06 and common-state UX-07 are accepted for current S2 screens |
...
| [Automated pre-release HIL](PRE_RELEASE_HIL.md) | the combined foreground runner flashes an exact candidate, executes enrolled product Survey plus an isolated generic suite, restores exact-CID read-only state, and emits hash-indexed evidence; product lanes prove async Detail progress and cancellation specifically during an observable active scan, bind runtime-emitted runner sources, and statically reject early worker-side terminal `Idle`; the typography lane normalizes persistent UI state, captures 18 exact EN/RU TFT frames, runs Quick/Full and restores Russian Home; the bounded endurance runner checkpoints repeated exact-candidate cycles and enforces an 8 h/32-cycle floor; malformed child metrics, retained incidents, and injected-timeout recovery are machine-checked | `E-AUTO-001…020/024…027`, `E-HIL-041…075/084…087`, ADR-005; deterministic hardware recovery, the 12-cycle/11,330.816 s engineering checkpoint, exact 0.59/0.60 worker regressions, exact 0.62 active cancellation, and exact 0.63 typography are proven without false release promotion. Full 8 h/32-cycle run remains NFR-004/DEMO-S4; camera calibration, destructive lane, and stable publish remain later gates |
