...
| [On-device Self-Test](SELF_TEST.md) | explicit last Home item offers read-only Quick and scoped Full/Guided; user and release HIL run the same versioned checks, while the host remains an independent oracle; absent profile modules are N/A and missing fixtures never become pass | PR-009, WF-01-A1…A6, `DEMO-S2/S5/S8`; S2 owns the shell/Quick/report, capability checks accumulate through S7, complete release verification closes in S8 |
