...
| Clean 1.x measurement target | independent pinned build implements BoardProfile, HardwareInventory→AppCatalog projection, unified display/input, buzzer-safe boot output, storage/runtime contracts, guarded SD identification/metadata/FAT persistence, throughput, reset instrumentation, shared recovery workspace, a real passive Wi-Fi→FIFO→persistent SessionStore→Library/export path, product SurveyWorkflow/SessionCatalog/SurveyPipeline plus fail-closed ProductStore/ProductSurvey admission, bounded Survey/Observation model, Session codec/store, and NDJSON evidence without legacy sources | `E-BUILD-003…042`, `E-HIL-006…048`; implementation evidence for PR-002 and slices of PR-003…007/009/012/NFR-002/003/005…010 plus lower bounds for PR-001/014, NFR-001/010, RB-02…06 |
...
| [Automated pre-release HIL](PRE_RELEASE_HIL.md) | the host runner retains and flashes a bundled exact candidate, requires equality of the full ELF SHA-256 across candidate descriptor/cold boot/metrics, binds manifest/device begin/end/run/local result with one random ID, traverses public Actions and bounded diagnostic queries, compares real-TFT RGB565 against reviewable goldens, and emits a hash-indexed bundle; one operator command controls cloud build and a service-free one-use runner; a GitHub workflow keyless-attests the complete candidate set and deterministic evidence archive through OIDC/Sigstore, then promotion/publish re-verifies provenance and the same bytes without rebuilding | `E-AUTO-001…010`, `E-HIL-041…052`, ADR-005; revision 6 asserts the deployed 5 ms frontend/64-entry press queue/batch path and retains fail-closed product admission, the full product vertical slice, and ten zero-mismatch frames; machine-checked physical UI-HIL-A8 binds 50/50 dispatched actions with high-water 6/64 and zero drops, while GitHub-native revision 6, destructive lane, external-camera subset, and stable publish remain open |
| [Storage atomicity HIL](STORAGE_HIL.md) | fixed dual heads publish only after sync; an exact-fingerprint guard prevents implicit use of unknown media; guarded FAT commit/remount/reopen, real-source queue/batching, current-boot Library admission/export, six-boundary software-reset recovery, and shared-workspace boundary-6 regression are physical | `E-STORAGE-001…005/019…022`, `E-SURVEY-003`, `E-LIBRARY-002`, `E-HIL-033…040`, `ST-HIL-A01…A08`; real Wi-Fi batching passes RB-06; boot catalog, power-cut, and LittleFS parity remain S3/S5/S8 |
...
| Passive Wi-Fi Survey ingress | passive-only plans reject active/directed operation; the explicit board adapter disables NVS/credentials and calls no connect/config/raw-TX API; under a combined lease the real path normalizes records into a fixed FIFO, commits a policy-selected Session, reopens it after remount, and passes the validated Session to Library/export | `E-SURVEY-001…003`, `E-BUILD-010/034/036/037`, `E-HIL-013/037/039/040`, WF-02-A1/A5; the latest run persists 52 observations with high-water 18, zero drops, and 12,957 B/s; instrumented no-TX, product Start/Stop, boot catalog, and endurance remain open |
| Product Survey workflow | simulated capability is explicit and RF-free; source ingress uses a bounded FIFO 64 with visible received/forwarded/drop/depth/high-water, then Setup→Running→List/Detail→Stop & Commit idempotently publishes/reopens one RAM generation, atomically replaces Library, and exports that same Session; real admission separately requires a passive plan, persistent commit permit, exact `/leshy/sessions/v1`, and combined lease without simulated/RAM fallback; failures preserve the prior Library | `E-SURVEY-004…006`, `E-STORAGE-023`, `E-LIBRARY-003`, `E-BUILD-011/040…042`, `E-HIL-014/046…048`, WF-02-A1/A2/A4/A5; board implementation evidence for PR-003…007 and NFR-002/003/006…010; real adapter lifecycle and boot read-only recovery remain open |
...
| Offline/persistent Library | a bounded controller admits only stopped/valid Sessions and exposes List/Detail with generation, integrity, simulated/persistent, and RF provenance; `SessionCatalog` staged-recovers the latest valid generation from one validated root, including corrupt-new fallback; the current-boot recovered SD Session replaces the simulated fixture and releases its Storage lease on Back | `E-LIBRARY-001…003`, `E-BUILD-014/037/040`, `E-HIL-017/040/046`, WF-03-A1/A3; implementation evidence for PR-006 and NFR-002/003/008/010; boot media discovery/mount and multi-root policy remain open |
