...
| [Automated pre-release HIL](PRE_RELEASE_HIL.md) | the host runner retains and flashes a bundled exact candidate, requires equality of the full ELF SHA-256 across candidate descriptor/cold boot/metrics, binds manifest/device begin/end/run/local result with one random ID, traverses public Actions and bounded diagnostic queries, compares real-TFT RGB565 against reviewable goldens, and emits a hash-indexed bundle; one operator command controls cloud build and a service-free one-use runner; a GitHub workflow keyless-attests the complete candidate set and deterministic evidence archive through OIDC/Sigstore, then promotion/publish re-verifies provenance and the same bytes without rebuilding | `E-AUTO-001…011`, `E-HIL-041…053`, ADR-005; revision 6 asserts the deployed keypad/product vertical slice and ten zero-mismatch frames; retained checkers bind physical keypad and isolated generic/product-boot states, while GitHub-native revision 6, automatic enrollment-state orchestration, destructive lane, external-camera subset, and stable publish remain open |
| [Storage atomicity HIL](STORAGE_HIL.md) | fixed dual heads publish only after sync; an exact-fingerprint guard prevents implicit use of unknown media; guarded FAT commit/remount/reopen, real-source queue/batching, current-boot Library admission/export, six-boundary software-reset recovery, shared-workspace boundary-6 regression, and exact-CID write-blocked product boot recovery are physical | `E-STORAGE-001…005/019…023`, `E-SURVEY-003`, `E-LIBRARY-002/003`, `E-HIL-033…040/053`, `ST-HIL-A01…A09`; real Wi-Fi batching passes RB-06 and product boot admits generation 1/17 with zero SD writes; power-cut and LittleFS parity remain S5/S8 |
