...
| [On-device Self-Test](SELF_TEST.md) | explicit last Home item offers read-only Quick and scoped Full/Guided; user and release HIL run the same versioned checks, while the host remains an independent oracle; absent profile modules are N/A and missing fixtures never become pass | PR-009, WF-01-A1…A6, `DEMO-S2/S5/S8`; Quick/report pass in `E-HIL-077`, plan-2 guided UI states pass in `E-HIL-081`/`E-UX-007`, capability checks accumulate through S7, and complete release verification closes in S8 |
