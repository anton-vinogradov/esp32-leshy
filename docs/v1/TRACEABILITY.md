...
| Clean 1.x measurement target | independent pinned build implements BoardProfile, HardwareInventory→AppCatalog projection, unified display/input, buzzer-safe boot output, storage/runtime contracts, guarded SD identification/metadata/FAT persistence, throughput, reset instrumentation, shared recovery workspace, bounded reset-separated product boot retry, and an interactive exact-CID real passive Wi-Fi→FIFO→persistent SessionStore→cold-boot Library/export path with fail-closed ProductStore/ProductSurvey admission, abort cleanup, bounded Survey/Observation model, Session codec/store, and NDJSON evidence without legacy sources | `E-BUILD-003…048`, `E-HIL-006…059`; implementation evidence for PR-002 and slices of PR-003…007/009/012/NFR-002/003/005…010 plus lower bounds for PR-001/014, NFR-001/010, RB-02…06 |
...
| [Automated pre-release HIL](PRE_RELEASE_HIL.md) | the combined foreground runner flashes an exact candidate, executes enrolled product Survey plus an isolated generic suite, restores exact-CID read-only state, and emits hash-indexed evidence; GitHub OIDC/Sigstore promotion re-verifies attested bytes without rebuilding; the external-camera verifier binds four physical panel states to GRAM; the bounded endurance runner checkpoints repeated exact-candidate cycles and enforces an 8 h/32-cycle release floor | `E-AUTO-001…015`, `E-HIL-041…059`, ADR-005; the GitHub-native combined lane, short repeatability, and endurance-runner smoke are proven; real camera calibration, complete endurance/destructive lanes, and stable publish remain open |
| [Storage atomicity HIL](STORAGE_HIL.md) | fixed dual heads publish only after sync; an exact-fingerprint guard prevents implicit use of unknown media; guarded FAT commit/remount/reopen, real-source queue/batching, current-boot Library admission/export, six-boundary software-reset recovery, exact-CID write-blocked boot recovery, bounded reset-separated readiness retry, and interactive product commit/abort are physical | `E-STORAGE-001…005/019…023`, `E-SURVEY-003`, `E-LIBRARY-002/003`, `E-HIL-033…040/053/054/058/059`, `ST-HIL-A01…A10`; real Wi-Fi batching passes RB-06, same-boot retry is rejected, and three fixed-candidate cycles advance 12→15; positive physical retry, power-cut, and LittleFS parity remain S5/S8 |
