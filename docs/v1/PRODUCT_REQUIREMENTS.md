...
| PR-004 | Shared List/Detail/Radar patterns Live network/device/graph browsing requires no SD; saving requires separate storage admission (ADR-008). | Back, filters, and units behave consistently across supported radios | P0 |
...
| PR-024 | Protect local secrets and evidence through Device Lock | First-run/local PIN setup, bounded retry and documented recovery cannot bypass safe cleanup, panic, update recovery, or factory reset; the lock overlay may continue a previously admitted safe Capture but controls, identities, and exports reveal no protected content; the owner can explicitly disable PIN protection without deleting or re-encrypting existing data and can enroll a new PIN later  Optional since ADR-007: a genuinely virgin device works without PIN after validated local key initialization; PIN enrollment is owner-selected, never mandatory onboarding. Existing locked/retry/recovery/corrupt or missing-expected credentials cannot fall back to this mode. No-PIN mode provides no PIN-based protection against possession of the device.| P0 before a release stores credentials or sensitive captures |
...
| PR-027 | Ship only named, individually accepted wireless Lab recipes | Every Wi-Fi/BLE/nRF/IR recipe declares owned fixture/target, region, channel/frequency, explicit power profile, duration, expected evidence and hardware stop path; targeted handshake-assist, synthetic iBeacon/identity emulation, MouseJack injection, selected-channel RF-interference robustness, other bounded robustness/crash and IR-camera tests are admitted only when target and containment are proven. An interference recipe may select full qualified hardware output, requires an isolated fixture/interlock and fails closed if containment cannot be verified; UI/evidence distinguish the requested radio power setting from independently measured power. Unbounded or indiscriminate ambient output and secret harvesting are rejected | P0 for any shipped active output |
...
| PR-033 | Verify owned evidence without disguising cracking as observation | Leshy validates explicitly owned PMKID/complete EAPOL evidence and exports canonical `hc22000`; bounded computer-companion verification performs the versioned common/weak-password and vendor-default corpus comparison with preview, budget, pause/stop, checkpoint and provenance. The companion may reveal the match to its owner, but plaintext is never returned to Leshy or persisted in Leshy logs/screenshots/reports/exports; the durable result records weakness class, corpus version/source/rank, cost and evidence references. Identity-linked leaked credential collections are never bundled. NFC/Sub-GHz/fixed-code verification may remain local or companion-assisted | P1 |
