...
| RB-M56 | measured | hardware-backed timeout boot recovery | 1 062 900 B linked flash; 125 464 B static RAM; app/factory images 1 063 312/1 128 848 B; RTC no-init 20 B; software tier 4 000 ms плюс panic-enabled Task WDT tier 5 000 ms; runtime heap total/free/min 276 040/227 588/192 128 B; injected timeout ready 6 697,964 ms, normal maximum ready 923,202 ms | `0.51.0-hardware-boot-watchdog-measure`, `E-HIL-072…074`/`E-AUTO-019/020`/`E-BUILD-053`; +1 048 B linked flash, +8 B static RAM и +1 056 B images к 0.50; IRAM placement handler/atomic, physical Task WDT recovery, zero heap drift за три product cycles и final lease 0 доказаны; это не 8 h gate |
...
| Runtime heap/queues | lease lifecycle измерен на 1 000 UI cycles; fixed FIFO 64 прошёл real Wi-Fi→SD→Library path с high-water 18 и zero drops; final 0.48 сохраняет 276 304/227 864/192 432 B через шесть cold boots и три commits | завершить product run 8 h/32 cycles, затем измерить отдельные concurrent receiver/storage workers | lower bound S1, endurance S3/S4 |
...
| Storage throughput/atomicity | bounded SessionStore matrices, guarded FAT/remount/reset evidence, passive Wi-Fi ingress, product workers/Stop и three-cycle product continuity измерены; natural Product Start/boot identity retries и injected lock-free timeout restart сохраняют prior generations с zero SD writes; real FIFO/batch path даёт 12 957 encoded B/s против required 2 184 B/s | завершить endurance lane, затем отдельно измерить LittleFS и physical power-cut | блокирует полную verification PR-005 и RB-06 |
