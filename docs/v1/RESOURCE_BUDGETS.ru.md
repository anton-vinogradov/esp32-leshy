...
| Storage throughput/atomicity | bounded SessionStore matrices, guarded FAT/remount/reset evidence, passive Wi-Fi ingress, product workers/Stop и 12-cycle continuity измерены; natural retries и injected timeout restart сохраняют prior generations; real FIFO/batch path даёт 12 957 encoded B/s против required 2 184 B/s | отдельно измерить LittleFS и physical power-cut; повторить 8 h на S4 cross-radio workload | блокирует полную verification PR-005, но не S1/S2 |
