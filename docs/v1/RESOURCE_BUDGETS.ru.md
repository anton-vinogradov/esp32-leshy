...
| Runtime heap/queues | lease lifecycle измерен на 1 000 UI cycles; fixed FIFO 64 прошёл real Wi-Fi→SD→Library path с high-water 18 и zero drops; product runner сохраняет invariant 276 312/227 876/194 956 B через шесть cold boots | завершить активный product run 8 h/32 cycles, затем измерить отдельные concurrent receiver/storage workers | lower bound S1, endurance S3/S4 |
...
| Storage throughput/atomicity | bounded SessionStore matrices, guarded FAT/remount/reset evidence, passive Wi-Fi ingress, product workers/Stop и three-cycle product continuity измерены; real FIFO/batch path даёт 12 957 encoded B/s против required 2 184 B/s и reopens после remount | наблюдать reset-between-attempts при natural transient, затем отдельно LittleFS и physical power-cut; завершить активный endurance lane | блокирует полную verification PR-005 и RB-06 |
