...
(36816 B сверх 512 KiB). Зависимости от PSRAM или полного framebuffer нет.
[Проверенная дельта имени](../../tests/hil/evidence/wifi-name-listen-1.0.0-dev.390.json):
13 кадров TFT; таймер 26 динамических / ноль статических пикселей; пассивное окно
без имени 20,001 с, touch Stop через 1,056 с, возврат скана той же сети.
Final heap total/free/min источника **139556/63704/20756 B**,
приёмника **139556/58064/14940 B**. Оба Home/none/lease 0; без операций SD и
потерь scan-очереди. Перед cleanup отдельная ограниченная история удержала
**64/747** наблюдений, отбросив **683** из-за ёмкости; живой каталог отдельный.
Конечные нулевые счётчики не означают запись без потерь. Heap-invariance,
client-association и широкая matrix не приняты.
[Прежние отказы](../../tests/hil/evidence/wifi-name-listen-dev389-390-failures.json)
сохраняют ошибку identity в runner, невыясненный тайм-аут запроса и неполный
USB-кадр с некорректным сравнением после повтора за пределами таймера.
Повтор снимка в timed-runner отключён; позднейший pass не устанавливает причину USB-сбоя.

## Предыдущая product-дельта — RB-M267 (14 сентября 2026)

Обычная dev.388 на обоих DIV: static RAM **237864 B**, linked flash **3624264 B**,
app **3624768 B**, OTA free **569536 B** (45248 B сверх 512 KiB).
Зависимости от PSRAM и полного framebuffer не добавлено. Source прошивки/runner `a42deb5`.
[Проверенный двухплатный итог](../../tests/hil/evidence/wifi-product-network-1.0.0-dev.388.json):
8 кадров TFT; таймер изменил 29 динамических / 0 статических пикселей; Stop по
таймеру замечен хостом через 60,395 с, включая задержку опроса. Final heap
total/free/min источника **140428/64576/21632 B**, приёмника **140428/58944/16344 B**.
Оба Home/none/lease 0, ноль drops/операций SD. Это конечные измерения, не
heap-invariance, endurance, оптическая проверка мерцания или полная matrix.
Настроенный предел 2 dBm применяется после старта Wi-Fi; запуск использует PHY default.

## Предыдущий заблокированный деплой — RB-M266 (14 сентября 2026)

Штатная тестовая Wi-Fi сеть: dev.386 static RAM **237864 B** (+512),
app **3624096 B**, OTA free **570208 B**. Полный host-suite прошёл;
загрузка оригинала остановилась до AP-теста. Follow-up dev.387 с этапами
загрузки в журнале и feed после SD recovery: static RAM **237864 B**,
linked flash **3624272 B**, app **3624768 B**, OTA free **569536 B**
(45248 B выше floor). Дельта и сборка прошли; до записи dev.387 дело не дошло —
загрузчик оригинала не ответил. После физического переподключения dev.387 и
dev.388 загрузились; RB-M267 фиксирует последующую узкую приёмку, не доказательство причины.
[Сохранённый blocker](../../tests/hil/evidence/wifi-product-network-dev386-boot-blocker.json).
Heap Safe Mode не сравнивается с работающей AP. Двухплатной приёмки,
heap-invariance, новой готовой функции или обнуления cadence нет.

## Предыдущая дельта меню — RB-M265 (14 сентября 2026)

Dev.385, группировка меню: static RAM **237352 B** (без изменений), linked flash **3618768 B**,
app **3619264 B** (+208 B), OTA free **575040 B** (50752 B сверх порога).
[Дельта меню на точном образе](../../tests/hil/evidence/wifi-menu-1.0.0-dev.385.json):
10 состояний переходов / 5 снимков TFT; final heap total/free/min 140940/66948/66840 B,
Home/none/lease 0, ноль drops/операций SD. Радио не запускалось: сравнивать этот heap
с активным сканированием или выдавать его за endurance нельзя.

## Предыдущий baseline карточек/графика — RB-M264
...
Bound foundation real passive trust P-256 `RB-M218`: exact host/build
`1.0.0-dev.304` на source `96a55a4d46f3171690cb3778d99acc2a95cbd0c2`
использует 233 048 B static RAM, 3 546 516 B linked flash и app/factory images
3 546 672/3 612 208 B, оставляя 647 632 B в OTA slot 4 MiB. SHA-256
app/factory/ELF/map —
...
Относительно dev.303 это +424 B static RAM и +4 720 B linked/app/factory с
уменьшением OTA headroom на 4 720 B. Delta финансирует fixed NVS record на четыре
ключа, real verification P-256 mbedTLS и production wiring; resident package,
...
Bound защищённого screenshot `RB-M227`: host/build `1.0.0-dev.329` на firmware
source `54d787ab982fc85948321f7642d3d3b658682ddd` использует 234 768 B static RAM,
3 592 880 B linked flash и app/factory images 3 593 040/3 658 576 B, оставляя
601 264 B в OTA slot 4 MiB. SHA-256 app/factory/ELF/map —
...
Относительно dev.328 это +704 B static RAM, +14 788 B linked flash, +14 448 B в
обоих app/factory images и на 14 448 B меньше OTA headroom. Delta оплачивает generic
ScreenshotStore с двумя heads, bounded encrypted stream adapters, Screenshot entry
Библиотеки, exact framebuffer capture/export и boot recovery. Frame 153 600 B никогда
не находится в static RAM: product capture использует PSRAM и chunks по четыре строки,
а encrypted filesystem IO — независимые bounded workspaces plaintext/ciphertext по
256 B с последующей очисткой. Native atomic fallback и policy tests проходят. Physical
run exact candidate остановился на намеренно внешнем precondition owner Device Lock до
...
Bound periodic full checkpoint и повторяемого BLE entry `RB-M243`: exact physical
`1.0.0-dev.357` на firmware source
`56e6ebe67b2b4d2e77e28bbf2cf0d29bed17a568` использует 234 976 B static RAM,
3 636 516 B linked flash и app/factory images 3 637 024/3 702 560 B, оставляя
557 280 B фактического headroom application image в OTA slot 4 MiB.
SHA-256 app/factory/ELF/map:
...
Относительно dev.354 static RAM растёт на 952 B, linked flash — на 16 832 B,
app/factory images — на 17 184/17 184 B; product delta включает task-first wiring
Protocol Workbench из dev.355, bounded diagnostics BLE и три persistent compositor
UI. Их выделение до запуска controller сохраняет оба complete passive lifecycle BLE
точно на 143 828/72 412 B total/free heap после cleanup. Последующие matrices Home/RF
на 21 screen, Targets и USB companion сохраняют тот же warm heap, generation 8/54,
zero input errors/drops, zero storage writes, zero RF TX и final Home/none/lease 0.
Исторический `heap_min_free` 508 B измерен, пока NimBLE владел runtime arena, и не
является доступным allocation headroom; admission по-прежнему требует 72 000 B free
