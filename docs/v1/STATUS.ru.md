...
| E-AUTO-001 | ADR-005 prerelease runner/suite/bundle verifier host tests | pass: manifest/action/assertion/mask bounds, missing-only golden bootstrap, refusal to overwrite, exact/masked pixel mismatch, artifact/candidate tamper, safe internal/escaping tar links и unsigned-default rejection покрыты; full `tools/test.sh` зелёный | queue/quarantine и camera/power остаются открыты; live GitHub HIL закрыт E-AUTO-005/E-HIL-044 |
...
| E-AUTO-005 | GitHub-native build-once/HIL/promotion trust workflow | live pass: `hil-production` разрешает только `main`; run `31975573475` на `97e7145` GitHub-attests exact candidate/factory/ELF/map, проверяет provenance до flash, выполняет physical HIL, attests evidence и повторно проверяет все artifacts/same bytes; unique-label one-job runner удалил credentials/registration и exit 0, repository runners 0; preceding run `31975374875` безопасно cancelled до registration/flash на overly-strict archive-link check; `publish` допускает только stable 1.x и rebuild отсутствует | measurement `0.37…` non-publishable; queue/quarantine, camera/power и полный release-candidate suite остаются открыты |
