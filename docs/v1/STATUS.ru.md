...
| S1 | `active` | vision, конкурентный срез, draft PRD 0.2, product-reviewed `CAP-001…047`, UX-01/02 и Stage Demo contracts, workflows, constrained hardware unknowns, partial HIL/budgets, risk register и пять ADR; automatic device-smoke v5 проверяет bounded physical-keypad frontend и product source→FIFO→Survey→commit→Library→export vertical slice, visible queue/drop/storage state и fail-closed real/persistent admission до hardware I/O | guided physical burst кнопок, остальные physical/storage evidence и review baseline PRD |
...
| E-AUTO-009 | debounced bounded physical-keypad frontend + `device-smoke` revision 5 | pass: host tests покрывают short pulse, invalid I2C read, held key без duplicate, stable release, все пять mappings, ambiguous chord и wrap `millis()`; static checks требуют 5 ms task и producer/consumer очередь 16; HIL требует deployed task/config/counters с пустой очередью и zero drops до serial navigation | automation может проверить frontend, но не создать physical switch edges; guided UI-HIL-A8 остаётся обязательным |
| E-HIL-049 | board-01 keypad-aware `device-smoke` revision 5 | automatic pass/gate-eligible local evidence: exact app `03dc165c…70c05c5`, ELF `21f31ab2…ae8958`, run ID `490608019ef55ae5c230ed1254a82fad`; ready 503,916 ms; frontend task даёт 930 valid/0 erroneous samples, maximum gap 5 ms, queue 0/16 с 0 drops; прежний workflow и десять TFT comparisons сохраняют zero mismatch; final owner none/lease 0, heap total/free/min 281 184/233 556/228 160 B, GPIO2 LOW; `run.json` `ab29096a…b97ee`, index `3b3a3ccb…a0f8ce` | physical burst 10×5 кнопок pending; local evidence unsigned, реальный product RF/SD lifecycle/boot recovery не выполнялись |
