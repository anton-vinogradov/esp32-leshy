...
| Storage throughput/atomicity | bounded SessionStore matrices, guarded FAT/remount/reset evidence, passive Wi-Fi ingress, product workers/Stop, and three-cycle product continuity are measured; the real FIFO/batch path delivers 12,957 encoded B/s against required 2,184 B/s and reopens after remount | observe the reset-between-attempts path on a natural transient, then separately measure LittleFS and physical power-cut; complete the active endurance lane | blocks full verification of PR-005 and RB-06 |
