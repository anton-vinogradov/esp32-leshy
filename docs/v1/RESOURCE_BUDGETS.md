...
Offline Field Survey export bound `RB-M197`: exact physical `1.0.0-dev.263` at
product source `0f46b4db840bf38e3beac6424623c33fca8e749e` keeps static RAM at
231,624 B. Native Field Survey CSV formatting is allocation-free and uses bounded
caller storage; the Library serial path streams native or WiGLE output without
starting receivers or mutating storage. Linked flash is 3,455,384 B. App/factory
sizes are 3,455,888/3,521,424 B with SHA-256
...
The 4 MiB OTA slot retains 738,416 B, above the mandatory 524,288 B floor.
Physical HIL read-only reopens exact generation 172/52 on attempt 1 and exports
52 native rows/4,650 B plus 52 WiGLE rows/3,573 B. It performs zero scans, commits,
physical or blocked writes, writes no raw CSV to host evidence, and exits
Home/none/lease 0. WiGLE remains `untimed_unlocated` and not upload-ready. The
compact [export evidence](../../tests/hil/evidence/board-01-field-survey-export-1.0.0-dev.263.json)
accepts routing/truthfulness, not live station capture or trusted GPS/UTC.

Offline Field Survey station bound `RB-M198`: exact physical `1.0.0-dev.266` at
source `ffd8f8f23cd3153a5de415049fc2665544af66c1` keeps static RAM at 231,624 B.
The receive-only station observer adds no persistent heap allocation and is bounded
to three complete 1–13 sweeps before it removes the promiscuous callback and releases
Wi-Fi for BLE. Linked flash is 3,458,056 B. App/factory sizes are
3,458,560/3,524,096 B with SHA-256
...
The 4 MiB OTA slot retains 735,744 B, still 211,456 B above the mandatory floor.
Physical first/revisit runs retain 51 bounded records each with 2 stations and zero
drops, then cold-recover exact generation 175/51 read-only. The no-reflash export
delta preserves both station rows in 51 native rows/4,593 B and excludes exactly
those rows from 49 WiGLE rows/3,362 B with zero scans/commits/writes and final
Home/none/lease 0. The compact
...
Device Lock persistence bound `RB-M207`: exact `1.0.0-dev.280` at firmware source
`c79e1d24d65e38e3fd1878f2893c8879f43a1b6e` uses 230,968 B static RAM and a
3,497,472 B app image, leaving 696,832 B in the 4 MiB OTA slot. The
app/factory/ELF/map SHA-256 values are
...
The eight static bytes and 2,208 padded app bytes beyond dev.278 add only the HIL
namespace selector and cleanup-continuity flags; production continues to default to
`leshy1-lock`. On original board-01 the final cold boot retains 74,332/74,104 B
free/minimum heap after four 7.47–7.48 s KDFs, two cold retry restores and explicit
...
Long-idle first-action correction bound `RB-M260`: exact `1.0.0-dev.378` at
firmware source `c4293a4adaabb1c46ca3cc66f84805dd2938a8d3` uses 236,008 B
static RAM, 3,612,752 B linked flash and 3,613,248/3,678,784 B app/factory
images, leaving 581,056 B actual application-image headroom in the 4 MiB OTA
slot. Firmware/factory/ELF/map SHA-256 are
...
Relative to dev.377, static RAM is unchanged, linked flash rises 44 B and both
app/factory images rise 32 B. OTA margin is 56,768 B above the 512 KiB floor.
The retained physical-key observation covers at least 42,551,505 ms and
