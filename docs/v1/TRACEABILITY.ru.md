...
| Clean measurement target 1.x | independent pinned build реализует BoardProfile, projection HardwareInventory→AppCatalog, unified display/input, buzzer-safe boot output, storage/runtime contracts, bounded reset-separated product boot retry и interactive exact-CID real passive Wi-Fi→FIFO→persistent SessionStore→cold-boot Library/export path с fail-closed ProductStore/ProductSurvey admission, abort cleanup, bounded Survey/Observation model, Session codec/store и NDJSON evidence без legacy sources | `E-BUILD-003…048`, `E-HIL-006…059`; implementation evidence для PR-002 и slices PR-003…007/009/012/NFR-002/003/005…010 плюс lower bounds для PR-001/014, NFR-001/010, RB-02…06 |
...
| [Автоматический предрелизный HIL](PRE_RELEASE_HIL.ru.md) | combined foreground runner прошивает exact candidate, выполняет enrolled product Survey и isolated generic suite, восстанавливает exact-CID read-only состояние и создаёт hash-indexed evidence; GitHub OIDC/Sigstore promotion повторно проверяет attested bytes без rebuild; внешний camera verifier связывает четыре физических panel states с GRAM; bounded endurance runner сохраняет repeated exact-candidate cycles и требует release floor 8 h/32 cycles | `E-AUTO-001…015`, `E-HIL-041…059`, ADR-005; GitHub-native combined lane, short repeatability и endurance-runner smoke подтверждены; real camera calibration, полный endurance/destructive lane и stable publish открыты |
| [HIL атомарности storage](STORAGE_HIL.ru.md) | fixed dual heads публикуют только после sync; exact-fingerprint guard запрещает unknown media; guarded FAT commit/remount/reopen, real-source batching, six-boundary recovery, exact-CID write-blocked boot, bounded reset-separated readiness retry и interactive product commit/abort проверены physical | `E-STORAGE-001…005/019…023`, `E-SURVEY-003`, `E-LIBRARY-002/003`, `E-HIL-033…040/053/054/058/059`, `ST-HIL-A01…A10`; same-boot retry отклонён, три fixed-candidate cycles продвигают 12→15; positive physical retry, power-cut и LittleFS parity остаются S5/S8 |
