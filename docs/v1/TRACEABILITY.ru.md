...
| Clean measurement target 1.x | independent pinned build реализует BoardProfile, projection HardwareInventory→AppCatalog, unified five-key display/input с non-color focus, bounded observable boot probe PCF8574 и компактной инкрементальной навигацией Left Back / Right-or-OK Enter / Up-Down Select с non-blocking physical dispatch «одно событие — один кадр», общие UI components и common-state cards, единый каталог EN/RU со сгенерированными Cyrillic fonts Roboto Condensed Medium 16/12 и persistent выбором языка, встроенный Self-Test, buzzer-safe boot output, storage/runtime contracts, guarded SD persistence, app-bound reset-separated boot retries, bounded Product Start raw-identity retry, два software/hardware tier boot-recovery watchdog и persistent bounded Core-0 Product Survey worker с UI-owned terminal acknowledgement вокруг interactive exact-CID real passive Wi-Fi→FIFO→persistent SessionStore→cold-boot Library/export path | `E-BUILD-003…068`, `E-HIL-006…091`, `E-AUTO-022…031`, `E-GATE-002`; S2 закрыт; exact 0.62 закрывает physical active-scan cancellation, 0.63 — Roboto typography, 0.64 возвращает spatial convention, 0.65 доказывает compact rendering, 0.66 обнаруживает post-render serial backpressure incident, а 0.67 удаляет его с 75 подтверждёнными пользователем presses, high-water 1/64 и max queue latency 1,256 ms; missing-source real-TFT, physical power-cut, LittleFS parity и final independent goldens остаются открыты |
...
| [Автоматический предрелизный HIL](PRE_RELEASE_HIL.ru.md) | combined runners связывают exact candidates, product Survey, TFT evidence и final cleanup; product lanes доказывают async progress и active-scan cancellation, typography снимает 18 EN/RU states, navigation проверяет видимые controls и bounded non-blocking physical dispatch без ручного прохода; endurance требует floor 8 h/32 cycles | `E-AUTO-001…020/024…031`, `E-HIL-041…075/084…091`, ADR-005; recovery и worker/cancel/typography/navigation regressions до exact 0.67 доказаны без ложного release promotion; full endurance и stable publish остаются later gates |
