...
| [Встроенный Self-Test](SELF_TEST.ru.md) | явный последний пункт Home предлагает read-only Quick и scoped Full/Guided; user и release HIL выполняют одинаковые versioned checks, host остаётся independent oracle; отсутствующие по profile modules дают N/A, missing fixtures никогда не становятся pass | PR-009, WF-01-A1…A6, `DEMO-S2/S5/S8`; Quick/report проходят в `E-HIL-077`, guided UI states plan 2 — в `E-HIL-081`/`E-UX-007`, capability checks накапливаются до S7, полный release verification закрывается в S8 |
