...
| Clean measurement target 1.x | independent pinned build реализует BoardProfile, projection HardwareInventory→AppCatalog, unified five-key display/input с non-color focus и bounded observable boot probe PCF8574, общие UI components и common-state cards, единый каталог EN/RU со сгенерированными Cyrillic fonts Roboto Condensed Medium 16/12 и persistent выбором языка, встроенный Self-Test, buzzer-safe boot output, storage/runtime contracts, guarded SD persistence, app-bound reset-separated boot retries, bounded Product Start raw-identity retry, два software/hardware tier boot-recovery watchdog и persistent bounded Core-0 Product Survey worker с UI-owned terminal acknowledgement вокруг interactive exact-CID real passive Wi-Fi→FIFO→persistent SessionStore→cold-boot Library/export path | `E-BUILD-003…064`, `E-HIL-006…087`, `E-AUTO-022…027`, `E-GATE-002`; S2 закрыт, exact 0.60 доказывает continuous Detail progress/generation 67→68 и удаляет terminal race, exact 0.62 закрывает physical active-scan cancellation без commit/leak после retained input-probe failure 0.61, exact 0.63 закрывает выбранную замену Cyrillic typography с fit 254/254 и TFT evidence 18 states; missing-source real-TFT, physical power-cut, LittleFS parity и final independent goldens остаются открыты |
...
| [UI automation contract](UI_AUTOMATION.ru.md) | physical и diagnostic input используют normalized Actions; actual TFT GRAM становится воспроизводимым PNG/state evidence без рутинного участия оператора | `UI-HIL-A1…A7`, `E-HIL-008/082/087`, `E-AUTO-022/027`, `E-UX-008`; platform gate S2 принят, exact 0.63 доказывает идемпотентный typography lane 18 states EN/RU тем же механизмом Stage Demo/release evidence |
| [Карта input и accessibility](UX_ACCESSIBILITY.ru.md) | пять физических кнопок и diagnostic Back используют общие normalized Actions; focus имеет outline/chevron, все текущие классы status остаются явным текстом, а не color-only signals | `E-BUILD-058/059`, `E-HIL-052/080/081`, `E-UX-006/007`; UX-06 и common-state UX-07 приняты для текущих экранов S2 |
...
| [Автоматический предрелизный HIL](PRE_RELEASE_HIL.ru.md) | combined foreground runner прошивает exact candidate, выполняет enrolled product Survey и isolated generic suite, восстанавливает exact-CID read-only состояние и создаёт hash-indexed evidence; product lanes подтверждают async Detail progress и cancellation именно при observable active scan, связывают runtime-emitted runner sources и statically отклоняют ранний worker-side terminal `Idle`; typography lane нормализует persistent UI state, снимает 18 exact EN/RU TFT frames, выполняет Quick/Full и возвращает русский Home; bounded endurance runner сохраняет repeated exact-candidate cycles и требует floor 8 h/32 cycles; malformed child metrics, retained incidents и injected-timeout recovery machine-checked | `E-AUTO-001…020/024…027`, `E-HIL-041…075/084…087`, ADR-005; deterministic hardware recovery, 12-cycle/11 330,816 s engineering checkpoint, exact worker regressions 0.59/0.60, exact active cancellation 0.62 и exact typography 0.63 подтверждены без ложного release promotion. Полный 8 h/32-cycle run остаётся NFR-004/DEMO-S4, camera calibration/destructive lane/stable publish — более поздними gates |
| [HIL атомарности storage](STORAGE_HIL.ru.md) | fixed dual heads публикуют только после sync; exact-fingerprint guards, guarded FAT lifecycle, real-source batching, six-boundary recovery, lower-clock bounded Product Start identity retry, app-bound boot retry, bounded recovery timeouts, persistent async source ownership, UI-owned terminal acknowledgement и interactive product commit/abort проверены physical | `E-STORAGE-001…005/019…023`, `E-SURVEY-003`, `E-LIBRARY-002/003`, `E-HIL-033…040/053/054/058…075/084…086`, `ST-HIL-A01…A10`; E-HIL-085 подтверждает normal Detail/commit/reboot recovery, E-HIL-086 — active-scan cancel с неизменными 68/25 и zero writes/leases; physical power-cut, full cross-radio endurance и LittleFS parity остаются S3/S4/S5/S8 |
...
| Product Survey workflow | simulated capability остаётся explicit/RF-free без enrollment; после exact-card boot admission AppCatalog выбирает real/persistent provenance и lease 15. Explicit Start ставит intent в очередь до validation/retry raw CID и filesystem access; persistent bounded worker владеет identity/mount/source/commit work, а UI владеет pipeline и единолично подтверждает terminal `Idle` после cleanup/commit. Live List/Detail/Back сохраняет и продвигает source, Stop запрашивает source shutdown до publish/reopen ровно следующей generation, bounded boot recovery/reboot/export подтверждают её; failures сохраняют prior Library при наличии cleanup evidence и fail closed при unknown final state | `E-SURVEY-004…006`, `E-STORAGE-023`, `E-LIBRARY-003`, `E-AUTO-023…026`, `E-BUILD-011/040…042/046…053/061…063`, `E-HIL-014/046…048/053/054/057…075/083…086`, WF-02-A1…A5; exact 0.60 подтверждает 67→68, live Detail progress и удаление early-Idle race; exact 0.62 физически отменяет active scan без изменения 68/25, writes/leaks; physical power-cut, missing-source TFT, LittleFS, dense RF и полный S4 endurance открыты |
