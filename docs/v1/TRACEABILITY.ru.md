...
| Clean measurement target 1.x | independent pinned build реализует BoardProfile, projection HardwareInventory→AppCatalog, unified display/input, buzzer-safe boot output, storage/runtime contracts, guarded SD persistence, app-bound reset-separated boot retries, bounded Product Start raw-identity retry и два software/hardware tier boot-recovery watchdog вокруг interactive exact-CID real passive Wi-Fi→FIFO→persistent SessionStore→cold-boot Library/export path | `E-BUILD-003…053`, `E-HIL-006…075`; implementation evidence для PR-002 и slices PR-003…007/009/012/NFR-002/003/005…010 плюс lower bounds для PR-001/014, NFR-001/010, RB-02…06; 12-cycle checkpoint подтверждает 51→63/144 observations/zero drift, но S2 закрывается UX-03…07 и DEMO-S2 |
