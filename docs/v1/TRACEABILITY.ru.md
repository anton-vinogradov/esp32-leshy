...
| Clean measurement target 1.x | independent pinned build реализует BoardProfile, projection HardwareInventory→AppCatalog, unified display/input, общие UI components, единый каталог EN/RU со сгенерированными Cyrillic fonts и persistent выбором языка, встроенный Self-Test, buzzer-safe boot output, storage/runtime contracts, guarded SD persistence, app-bound reset-separated boot retries, bounded Product Start raw-identity retry и два software/hardware tier boot-recovery watchdog вокруг interactive exact-CID real passive Wi-Fi→FIFO→persistent SessionStore→cold-boot Library/export path | `E-BUILD-003…057`, `E-HIL-006…079`; implementation evidence для PR-002 и slices PR-003…007/009/011/012/NFR-002/003/005…010 плюс lower bounds для PR-001/014, NFR-001/010, RB-02…06; UX-03…05 приняты, 12-cycle checkpoint подтверждает 51→63/144 observations/zero drift, а S2 закрывается UX-06/07 и DEMO-S2 |
