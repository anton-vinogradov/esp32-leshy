...
| Clean measurement target 1.x | independent pinned build реализует BoardProfile, projection HardwareInventory→AppCatalog, unified five-key display/input с non-color focus, общие UI components и common-state cards, единый каталог EN/RU со сгенерированными Cyrillic fonts и persistent выбором языка, встроенный Self-Test, buzzer-safe boot output, storage/runtime contracts, guarded SD persistence, app-bound reset-separated boot retries, bounded Product Start raw-identity retry и два software/hardware tier boot-recovery watchdog вокруг interactive exact-CID real passive Wi-Fi→FIFO→persistent SessionStore→cold-boot Library/export path | `E-BUILD-003…060`, `E-HIL-006…083`, `E-AUTO-022/023`, `E-GATE-002`; S2 закрыт, exact S3 progress подтверждает live List→Detail→Back и generation 65→66 reboot/export, а missing-source real-TFT, physical power-cut, LittleFS parity и final independent goldens остаются открыты |
...
| [Автоматический предрелизный HIL](PRE_RELEASE_HIL.ru.md) | combined foreground runner прошивает exact candidate, выполняет enrolled product Survey и isolated generic suite, восстанавливает exact-CID read-only состояние и создаёт hash-indexed evidence; bounded endurance runner сохраняет repeated exact-candidate cycles и требует floor 8 h/32 cycles; malformed child metrics, retained incidents и injected-timeout recovery machine-checked | `E-AUTO-001…020`, `E-HIL-041…075`, ADR-005; deterministic hardware recovery и 12-cycle/11 330,816 s engineering checkpoint подтверждены без ложного release promotion. Полный 8 h/32-cycle run остаётся NFR-004/DEMO-S4, camera calibration/destructive lane/stable publish — более поздними gates |
| [HIL атомарности storage](STORAGE_HIL.ru.md) | fixed dual heads публикуют только после sync; exact-fingerprint guards, guarded FAT lifecycle, real-source batching, six-boundary recovery, lower-clock bounded Product Start identity retry, app-bound boot retry, bounded recovery timeouts и interactive product commit/abort проверены physical | `E-STORAGE-001…005/019…023`, `E-SURVEY-003`, `E-LIBRARY-002/003`, `E-HIL-033…040/053/054/058…075`, `ST-HIL-A01…A10`; E-HIL-073…075 подтверждают hardware recovery и 12-cycle continuity; power-cut, full cross-radio endurance и LittleFS parity остаются S4/S5/S8 |
...
| Passive Wi-Fi Survey ingress | passive-only plans отклоняют active/directed operation; explicit adapter отключает NVS/credentials, owns/cleans event loop и не вызывает connect/config/raw-TX API; под lease 15 product path нормализует records в FIFO 64, commits одну authorized Session и передаёт reboot-validated Session в Library/export | `E-SURVEY-001…003`, `E-BUILD-010/034/036/037/046…052`, `E-HIL-013/037/039/040/053/054/059/064/067/068/069/071/075`, WF-02-A1/A5; E-HIL-075 принимает/forward 144/144 с zero drops через 12 cycles; instrumented physical no-TX и full S4 endurance открыты |
| Product Survey workflow | simulated capability остаётся explicit/RF-free без enrollment; после exact-card boot admission AppCatalog выбирает real/persistent provenance и lease 15. Explicit Start validates/retries только raw CID до filesystem access, затем проверяет bounded cached space/root; passive scan drain FIFO 64, live List/Detail/Back сохраняет running worker, Stop publishes/reopens ровно следующую generation, bounded boot recovery/reboot/export подтверждают её; failures сохраняют prior Library при наличии cleanup evidence и fail closed при unknown final state | `E-SURVEY-004…006`, `E-STORAGE-023`, `E-LIBRARY-003`, `E-AUTO-023`, `E-BUILD-011/040…042/046…053`, `E-HIL-014/046…048/053/054/057…075/083`, WF-02-A1…A5; exact 0.58 подтверждает 65→66 и Detail→Back за 102,636 ms; physical power-cut, missing-source TFT, LittleFS, dense RF и полный S4 endurance открыты |
