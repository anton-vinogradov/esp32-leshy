...
| [Автоматический предрелизный HIL](PRE_RELEASE_HIL.ru.md) | combined foreground runner прошивает exact candidate, выполняет enrolled product Survey и isolated generic suite, восстанавливает exact-CID read-only состояние и создаёт hash-indexed evidence; bounded endurance runner сохраняет repeated exact-candidate cycles и требует floor 8 h/32 cycles; malformed child metrics, retained incidents и injected-timeout recovery machine-checked | `E-AUTO-001…020`, `E-HIL-041…074`, ADR-005; GitHub-native combined HIL, natural reset-separated retries, deterministic software/hardware timeout recovery, retained release failures и regression 0.51 подтверждены. E-HIL-072 остаётся rejected; real camera calibration, passing full endurance/destructive lane и stable publish открыты |
| [HIL атомарности storage](STORAGE_HIL.ru.md) | fixed dual heads публикуют только после sync; exact-fingerprint guards, guarded FAT lifecycle, real-source batching, six-boundary recovery, lower-clock bounded Product Start identity retry, app-bound boot retry, bounded recovery timeouts и interactive product commit/abort проверены physical | `E-STORAGE-001…005/019…023`, `E-SURVEY-003`, `E-LIBRARY-002/003`, `E-HIL-033…040/053/054/058…074`, `ST-HIL-A01…A10`; same-boot retry, recovery на трёх attempts, увеличенный R1 polling и software-watchdog-only recovery отклонены retained evidence. E-HIL-073/074 подтверждают hardware Task WDT recovery и следующий product regression; power-cut, passing full endurance и LittleFS parity остаются S5/S8 |
