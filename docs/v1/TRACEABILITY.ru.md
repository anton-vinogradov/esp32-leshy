...
| Clean measurement target 1.x | independent pinned build реализует BoardProfile, projection HardwareInventory→AppCatalog, unified display/input, buzzer-safe boot output, storage/runtime contracts, guarded SD persistence, app-bound reset-separated boot retries, bounded Product Start raw-identity retry и lock-free boot-recovery watchdog вокруг interactive exact-CID real passive Wi-Fi→FIFO→persistent SessionStore→cold-boot Library/export path | `E-BUILD-003…052`, `E-HIL-006…071`; implementation evidence для PR-002 и slices PR-003…007/009/012/NFR-002/003/005…010 плюс lower bounds для PR-001/014, NFR-001/010, RB-02…06 |
...
| [Автоматический предрелизный HIL](PRE_RELEASE_HIL.ru.md) | combined foreground runner прошивает exact candidate, выполняет enrolled product Survey и isolated generic suite, восстанавливает exact-CID read-only состояние и создаёт hash-indexed evidence; bounded endurance runner сохраняет repeated exact-candidate cycles и требует floor 8 h/32 cycles; malformed child metrics, retained incidents и injected-timeout recovery machine-checked | `E-AUTO-001…018`, `E-HIL-041…071`, ADR-005; GitHub-native combined HIL, natural reset-separated retries, deterministic no-OS watchdog recovery, retained release failures и three-cycle regression подтверждены; real camera calibration, passing full endurance/destructive lane и stable publish открыты |
| [HIL атомарности storage](STORAGE_HIL.ru.md) | fixed dual heads публикуют только после sync; exact-fingerprint guards, guarded FAT lifecycle, real-source batching, six-boundary recovery, lower-clock bounded Product Start identity retry, app-bound boot retry, lock-free recovery timeout и interactive product commit/abort проверены physical | `E-STORAGE-001…005/019…023`, `E-SURVEY-003`, `E-LIBRARY-002/003`, `E-HIL-033…040/053/054/058…071`, `ST-HIL-A01…A10`; same-boot retry, недостаточный boot recovery на трёх attempts и увеличенный R1 polling отклонены retained evidence; eight-attempt reset-separated policy восстанавливается с zero writes в short regression; power-cut, passing full endurance и LittleFS parity остаются S5/S8 |
