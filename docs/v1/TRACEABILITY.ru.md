...
| Clean measurement target 1.x | independent pinned build реализует BoardProfile, projection HardwareInventory→AppCatalog, unified five-key display/input с non-color focus и bounded observable boot probe PCF8574, общие UI components и common-state cards, единый каталог EN/RU со сгенерированными Cyrillic fonts и persistent выбором языка, встроенный Self-Test, buzzer-safe boot output, storage/runtime contracts, guarded SD persistence, app-bound reset-separated boot retries, bounded Product Start raw-identity retry, два software/hardware tier boot-recovery watchdog и persistent bounded Core-0 Product Survey worker с UI-owned terminal acknowledgement вокруг interactive exact-CID real passive Wi-Fi→FIFO→persistent SessionStore→cold-boot Library/export path | `E-BUILD-003…063`, `E-HIL-006…086`, `E-AUTO-022…026`, `E-GATE-002`; S2 закрыт, exact 0.60 доказывает continuous Detail progress/generation 67→68 и удаляет terminal race, а exact 0.62 закрывает physical active-scan cancellation без commit/leak после retained input-probe failure 0.61; missing-source real-TFT, physical power-cut, LittleFS parity и final independent goldens остаются открыты |
...
| [UI automation contract](UI_AUTOMATION.ru.md) | physical и diagnostic input используют normalized Actions; actual TFT GRAM становится воспроизводимым PNG/state evidence без рутинного участия оператора | `UI-HIL-A1…A7`, `E-HIL-008/082`, `E-AUTO-022`; platform gate S2 принят, тот же механизм остаётся путём Stage Demo/release evidence |
...
| [Автоматический предрелизный HIL](PRE_RELEASE_HIL.ru.md) | combined foreground runner прошивает exact candidate, выполняет enrolled product Survey и isolated generic suite, восстанавливает exact-CID read-only состояние и создаёт hash-indexed evidence; product lanes подтверждают async Detail progress и cancellation именно при observable active scan, связывают runtime-emitted runner sources и statically отклоняют ранний worker-side terminal `Idle`; bounded endurance runner сохраняет repeated exact-candidate cycles и требует floor 8 h/32 cycles; malformed child metrics, retained incidents и injected-timeout recovery machine-checked | `E-AUTO-001…020/024…026`, `E-HIL-041…075/084…086`, ADR-005; deterministic hardware recovery, 12-cycle/11 330,816 s engineering checkpoint, exact worker regressions 0.59/0.60 и exact active cancellation 0.62 подтверждены без ложного release promotion. Полный 8 h/32-cycle run остаётся NFR-004/DEMO-S4, camera calibration/destructive lane/stable publish — более поздними gates |
