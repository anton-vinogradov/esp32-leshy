...
| [Автоматический предрелизный HIL](PRE_RELEASE_HIL.ru.md) | combined foreground runner прошивает exact candidate, выполняет enrolled product Survey и isolated generic suite, восстанавливает exact-CID read-only состояние и создаёт hash-indexed evidence; bounded endurance runner сохраняет repeated exact-candidate cycles и требует floor 8 h/32 cycles; retained incidents и injected-timeout recovery machine-checked | `E-AUTO-001…016`, `E-HIL-041…064`, ADR-005; GitHub-native combined HIL, natural reset-separated retries, deterministic no-OS watchdog recovery и three-cycle regression подтверждены; real camera calibration, full endurance/destructive lane и stable publish открыты |
| [HIL атомарности storage](STORAGE_HIL.ru.md) | fixed dual heads публикуют только после sync; exact-fingerprint guards, guarded FAT lifecycle, real-source batching, six-boundary recovery, bounded Product Start identity retry, app-bound boot retry, lock-free recovery timeout и interactive product commit/abort проверены physical | `E-STORAGE-001…005/019…023`, `E-SURVEY-003`, `E-LIBRARY-002/003`, `E-HIL-033…040/053/054/058…064`, `ST-HIL-A01…A10`; same-boot retry и blocking watchdog diagnostics отклонены; natural identity retry и injected timeout восстанавливаются с zero writes; power-cut, full endurance и LittleFS parity остаются S5/S8 |
