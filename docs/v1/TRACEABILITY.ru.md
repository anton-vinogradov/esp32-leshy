...
| Clean measurement target 1.x | independent pinned build реализует BoardProfile, projection HardwareInventory→AppCatalog, unified five-key display/input с non-color focus, общие UI components и common-state cards, единый каталог EN/RU со сгенерированными Cyrillic fonts и persistent выбором языка, встроенный Self-Test, buzzer-safe boot output, storage/runtime contracts, guarded SD persistence, app-bound reset-separated boot retries, bounded Product Start raw-identity retry, два software/hardware tier boot-recovery watchdog и persistent bounded Core-0 Product Survey worker с UI-owned terminal acknowledgement вокруг interactive exact-CID real passive Wi-Fi→FIFO→persistent SessionStore→cold-boot Library/export path | `E-BUILD-003…062`, `E-HIL-006…085`, `E-AUTO-022…025`, `E-GATE-002`; S2 закрыт, exact 0.60 S3 progress подтверждает continuous source work через Detail и generation 67→68 reboot/export, одновременно удаляя race repeated-Start terminal; missing-source real-TFT, physical cancel/power-cut, LittleFS parity и final independent goldens остаются открыты |
...
| [Автоматический предрелизный HIL](PRE_RELEASE_HIL.ru.md) | combined foreground runner прошивает exact candidate, выполняет enrolled product Survey и isolated generic suite, восстанавливает exact-CID read-only состояние и создаёт hash-indexed evidence; product lane подтверждает async worker progress через Detail, связывает runtime-emitted source runner и statically отклоняет ранний worker-side terminal `Idle`; bounded endurance runner сохраняет repeated exact-candidate cycles и требует floor 8 h/32 cycles; malformed child metrics, retained incidents и injected-timeout recovery machine-checked | `E-AUTO-001…020/024/025`, `E-HIL-041…075/084/085`, ADR-005; deterministic hardware recovery, 12-cycle/11 330,816 s engineering checkpoint и exact worker regressions 0.59/0.60 подтверждены без ложного release promotion. Полный 8 h/32-cycle run остаётся NFR-004/DEMO-S4, camera calibration/destructive lane/stable publish — более поздними gates |
| [HIL атомарности storage](STORAGE_HIL.ru.md) | fixed dual heads публикуют только после sync; exact-fingerprint guards, guarded FAT lifecycle, real-source batching, six-boundary recovery, lower-clock bounded Product Start identity retry, app-bound boot retry, bounded recovery timeouts, persistent async source ownership, UI-owned terminal acknowledgement и interactive product commit/abort проверены physical | `E-STORAGE-001…005/019…023`, `E-SURVEY-003`, `E-LIBRARY-002/003`, `E-HIL-033…040/053/054/058…075/084/085`, `ST-HIL-A01…A10`; E-HIL-085 подтверждает source progress через Detail, stop-before-commit, terminal cleanup и read-only reboot recovery; physical cancel/power-cut, full cross-radio endurance и LittleFS parity остаются S3/S4/S5/S8 |
