...
## Конкурентная полнота, защита устройства и автоматизация
...
| CAP-061 | Owned Evidence Verification проверяет собственные evidence с preview, budget, pause/stop/checkpoint и provenance: Leshy захватывает и валидирует Wi-Fi PMKID/EAPOL, экспортирует canonical `hc22000`, а компьютерный companion сравнивает его с corpus распространённых/слабых паролей и vendor-default patterns; проверки NFC/Sub-GHz/fixed-code могут быть local или companion-assisted. Identity-linked leaked corpora не поставляются, plaintext match не возвращается и не сохраняется на Leshy, а durable report содержит только класс слабости, provenance/rank corpus и ссылки на evidence | P1 | J-03, J-06, PR-033, NFR-011 | S7 |
