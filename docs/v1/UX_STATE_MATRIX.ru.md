...
| UX-S23/S24 Device/Diagnostics | module state always declared/detected/available/conflicted/fault/unknown; no fake percentage | read-only probe/report progress cancellable | live metrics не запускают TX | unknown/partial evidence показаны как такие | probe fault изолирован от соседних modules | risky self-test отсутствует в Diagnostics | export report имеет build/profile/checksum | Back завершает probe и освобождает leases |
