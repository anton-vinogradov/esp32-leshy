...
- Отказ Device Lock на Home или в защищённых настройках Устройства открывает экран
  защиты с понятным действием разблокировки/восстановления до запуска функции.
  Новое устройство с инициализированным ключом сразу открывает функции без PIN;
  добровольная настройка доступна в Устройство → Блокировка. Первое отклонённое
  нажатие не вводит PIN и не меняет состояние защиты. Влево возвращает к прежнему
  пункту меню; retry/recovery/fault остаются fail-closed. Успешная настройка или
  разблокировка не запускает автоматически ранее выбранное радио или операцию.
...
| UX-S30 Захват Wi-Fi-аутентификации | invalid target/channel или unavailable receive source имеют exact reason; отсутствие EAPOL/PMKID не success и не error | fixed target/channel и remaining time; live counts отделяют candidates от retained/dropped input и никогда не показывают terminal EAPOL до Stop | стабильный bounded capture; перерисовывается только изменённый content | любая loss, invalid accounting или unsupported evidence видима и принудительно даёт `inconclusive` | inconsistent report, zero-mask peer или полностью нулевой Key MIC в M2/M3/M4 fail closed; cleanup предшествует result | нет: receive-only Повтор не требует active confirmation | volatile/RAM-only/not saved result ставит `inconclusive` раньше Full/PMKID/Partial; `exportEligibility=NotEvaluated`; Действия→Устройство→Доказательства→Детали открывают только valid immutable evidence | Up/Down остаются в уровне; Right/OK входят; Left/Back возвращают на уровень; Повтор запускает тот же bounded capture; final Back оставляет zero leases |
