...
2.4 ГГц предлагает «Обзор эфира» / «Найти сигнал», Sub-GHz — chooser CC, а Устройство остаётся
...
| Поиск сигнала 2.4 ГГц | найти частоту пульта, метки или датчика | two-step инструкция фон/действие, точная частота, ближайший канал Wi-Fi при применимости и полноширинный график отклика | windows, sweeps, thresholds, response-bin counts и wire counters приёмника |
| Поиск частоты Sub-GHz | найти, на какой частоте работает свой пульт, метка или датчик | two-step инструкция фон/действие, точные кГц, подсказка ближайшего стандартного диапазона и полноширинный график отклика | bins, sweeps, calibration strategy, threshold, harmonic mask и wire counters CC1101 |
...
Exact 0.109 расширяет правило на `Wi-Fi→Каналы`. Экран использует всю ширину
content для тринадцати подписанных столбцов каналов на чёрном фоне без сигнала и
показывает только честную нижнюю шкалу `занятость ~0–8%` плюс наименее занятый
измеренный основной канал из 1/6/11. Live refresh рисует только изменившийся
столбец и, при необходимости, рекомендацию. Header, scale, channel axis и footer
