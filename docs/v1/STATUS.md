...
| E-AUTO-005 | GitHub-native build-once/HIL/promotion trust workflow | live pass: `hil-production` permits only `main`; run `31975573475` on `97e7145` GitHub-attests exact candidate/factory/ELF/map files, verifies provenance before flash, executes physical HIL, attests evidence, and re-verifies every artifact/same bytes; the unique-label one-job runner removed credentials/registration and exited 0, leaving zero repository runners; preceding run `31975374875` cancelled safely before registration/flash on an overly strict archive-link check; `publish` admits stable 1.x only and never rebuilds | measurement `0.37…` is non-publishable; queue/quarantine, camera/power, and the full release-candidate suite remain open |
