...
| S1 | `active` | vision, competitive snapshot, draft PRD 0.2, product-reviewed `CAP-001…047`, UX-01/02 and Stage Demo contracts, workflows, constrained hardware unknowns, partial HIL/budgets, risk register, five ADRs, and automatic device-smoke v5 covering the bounded physical-keypad frontend plus a product source→FIFO→Survey→commit→Library→export vertical slice, visible queue/drop/storage state, and fail-closed real/persistent admission before hardware I/O | guided physical-key burst, remaining physical/storage evidence, and PRD baseline review |
...
| E-AUTO-009 | debounced bounded physical-keypad frontend + `device-smoke` revision 5 | pass: host tests cover short pulse, invalid I2C read, held key without duplicate, stable release, all five mappings, ambiguous chord, and `millis()` wrap; static checks require the 5 ms task and 16-entry producer/consumer queue; HIL requires deployed task/config/counters with empty queue and zero drops before serial navigation | automation can assert the frontend but cannot generate physical switch edges; guided UI-HIL-A8 remains required |
| E-HIL-049 | board-01 keypad-aware `device-smoke` revision 5 | automatic pass/gate-eligible local evidence: exact app `03dc165c…70c05c5`, ELF `21f31ab2…ae8958`, run ID `490608019ef55ae5c230ed1254a82fad`; ready 503.916 ms; frontend task reports 930 valid/0 erroneous samples, maximum gap 5 ms, queue 0/16 with 0 drops; prior workflow and ten TFT comparisons remain zero mismatch; final owner none/lease 0, heap total/free/min 281,184/233,556/228,160 B, GPIO2 LOW; `run.json` `ab29096a…b97ee`, index `3b3a3ccb…a0f8ce` | physical 10×5-key burst is pending; local evidence is unsigned and real product RF/SD lifecycle/boot recovery were not exercised |
