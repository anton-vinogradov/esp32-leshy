...
Реализация 14 сентября: dev.390 включает четыре раздела сведений, безопасный
UTF-8/escaped SSID с обрезкой по пикселям и **Имя и устройство → Дослушать имя**.
Явный OK/тач запускает пассивный приём выбранного канала максимум на 20 секунд.
Другие каналы приостанавливаются; Stop/таймер после cleanup возвращает обычный
скан. Вход на страницу и нажатие вправо приём не запускают.
...
Декодер подключён к этому живому окну и той же карточке AP. Карточка показывает
источник AP/клиент, подтверждение точкой, возраст и предупреждение о конфликте;
первое имя сохраняется без подмены RSSI и счётчика отсчётов скана.
Используется прежний foreground lease, без очереди пакетов, записи на SD и
зависимости от PSRAM. Static RAM выросла на 872 B относительно dev.388.
Физический positive из association клиента — отдельный открытый gate;
одних host-тестов недостаточно для его закрытия.

## Как это выглядит пользователю

Подготовка dev.391 (собрана; деплой заблокирован): обычный экран тестовой сети
показывает новый случайный пароль WPA2 только пока сеть активна, не в USB и не на SD.
Приёмник независимо сохраняет первый клиентский кадр: ответ AP на направленный
probe может прийти раньше, поэтому AP-first не переименовывается в client-first.
Надпись сообщает, что **услышан кадр подключения**, а не доказывает успешную
аутентификацию, DHCP или интернет. Физическая проверка телефоном открыта.

[Физическая дельта dev.390](../../tests/hil/evidence/wifi-name-listen-1.0.0-dev.390.json):
20,001 с без имени, имя из живого кадра AP, touch Stop, свежий скан после возврата
и 13 снимков TFT; таймер меняет 26 динамических / ноль статических пикселей.
Оба DIV завершают Home/none/lease 0. Ветку реального клиента это не доказывает.
[Прежние отказы](../../tests/hil/evidence/wifi-name-listen-dev389-390-failures.json)
сохранены; надёжность USB-диагностики не объявлена исправленной.
...
отдельный тестовый бинарник. На прежней обычной dev.388
...
с нулевыми конечными счётчиками drops и операций SD. Сброшенные конечные счётчики
не доказывают отсутствие потерь за весь сеанс. Переподключение восстановило загрузку; прежний
...
Принятый baseline источника проверяет только обнаружение. Dev.391 готовит проверку
кадра подключения своего телефона, без DHCP, веб-сервера, интернета, raw injection,
deauthentication и сети ноутбука.
