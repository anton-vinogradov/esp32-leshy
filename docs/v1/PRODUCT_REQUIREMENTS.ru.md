...
Статус документа: **черновик 0.2 после product review**, 16 августа 2026 года.
...
Требования будут уточнены после карты аппаратуры и проверки эталонных сценариев, но
их идентификаторы уже стабильны для трассировки решений и тестов.
...
| PR-009 | Diagnostics проверяет ресурсы и базовые модули | Self-test не начинает передачу и сохраняет экспортируемый отчёт | P0 |
...
| NFR-004 | 8-часовой passive Survey не имеет монотонного роста heap, зависания UI или повреждения Session |
...
## Условия заморозки требований

Черновик становится baseline 1.0 после четырёх проверок:

- [карта аппаратуры](HARDWARE_ENVELOPE.ru.md) подтверждена схемой, кодом 0.x и
  выполнением `HW-T01…HW-T11` на ESP32-DIV v2;
...
- flash/RAM/storage/power бюджеты измерены либо явно ограничены в
