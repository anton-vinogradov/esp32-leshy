...
| R-018 | A fatal loop/worker fault leaves software-controlled outputs active or silently reboots into the same unsafe operation | M | critical | permanent panic Task WDT on the main loop; IRAM GPIO2/14/15/47 quiesce before diagnostics; exact-app torn-write-resistant RTC latch plus first CPU/page/stage trace; no automatic clear; Safe Mode blocks product workers and normal Actions | safety/platform; exact 0.103 accepts the injected main-loop path; exact dev.372 overnight `E-SAFETY-088` retains a real reset-reason-6 Wi-Fi-cycle trip and dev.374 bounds loop work with zero recurrence in `E-HIL-240`. Worker heartbeats and independent physical-stop HIL remain closure work | reduced/open |
