...
| R-007 | No-PSRAM heap pressure, fragmentation, or unbounded queues fail long Survey runs | H | H | RB-03/04, bounded queues/pools, finite per-loop drains, rate-limited passive data inspection, high-water/drop counters, no monotonic heap decline | kernel/services; `RB-M257`/`E-HIL-240` accept 3-minute exact Devices and Networks deltas with zero post-warm drift/drops after the overnight WDT; 2,980 B transient minimum remains active pressure; size gate + one-hour-budget release endurance and optional extended qualification remain | reduced/open |
...
| R-018 | A fatal loop/worker fault leaves software-controlled outputs active or silently reboots into the same unsafe operation, while the cause disappears before diagnosis | M | critical | permanent panic Task WDT on the main loop; IRAM GPIO2/14/15/47 quiesce before diagnostics; exact-app torn-write-resistant RTC latch; first safe boot verifies one deduplicated durable NVS cause; Safe Mode blocks product workers/normal Actions and SD writes; after explicit clear an exact-CID SD mirror is atomic and opportunistic | safety/platform; exact 0.103 accepts the injected main-loop path; exact dev.372 overnight `E-SAFETY-088` retains a real reset-reason-6 Wi-Fi-cycle trip and dev.374 bounds loop work with zero recurrence in `E-HIL-240`. Exact dev.376 `E-SAFETY-089` proves WDT → NVS → restart dedup → SD mirror → Home and retains the rejected dev.375 stack failure. Worker heartbeats and independent physical-stop HIL remain closure work | reduced/open |
...
| R-020 | Defensive detector or channel/auth analysis presents a false conclusion as a threat or proof | M | H | every conclusion exposes detector/version/threshold/confidence/uncertainty and exact evidence; insufficient fixtures remain inconclusive; no automatic response. Exact dev.210 pins these rules for the first Wi-Fi disconnect-burst detector and excludes driver/TX dependencies; dev.211 validates reports and freezes evidence-strength order; dev.212 rejects contradictory status/counters/evidence bounds and provides bounded EN/RU text so color is never the only meaning; dev.213 renders empty input explicitly as capture-not-started; dev.214 adds bounded live passive Wi-Fi evidence while observed/drop counters, zero coverage and incomplete cleanup fail closed; dev.215 adds a second complete-evidence SSID/security conflict indicator, labels it medium-confidence investigation rather than evil-twin proof and retains both exact BSSIDs/security/source references; dev.216 partitions the one live buffer between reserved disconnect slots and deduplicated exact identity profiles, enables the detector only after complete cleanup and converts malformed/oversized/capacity loss into incomplete evidence; dev.217 adds a third same-BSSID rapid visible-name-change indicator, treats legitimate multi-SSID behavior as a medium-confidence investigation, ignores repeated names, rejects split/stale/malformed evidence, retains exact references and makes no PineAP or attack claim; dev.218 adds default-off BLE tracker-compatible presence over exact protocol/address-type/identity groups, calls confidence repeated presence rather than ownership or unwanted tracking and fails closed on ambiguous/incomplete evidence; dev.219 gives that finding dedicated owner-unknown EN/RU copy and source-record/time/RSSI details, validates Bluetooth identity and zero-channel evidence independently from Wi-Fi; dev.220 preserves exact repeated BLE reports only for this detector, separates raw/retained/lost accounting and fails closed on malformed, queue or capacity loss; dev.221 sequences complete Wi-Fi cleanup before BLE on the existing supervised worker, rejects stale generations and requires exact accounting plus cleanup before publication, while Back/safety/deadline and a pre-start latch make cancellation fail closed; dev.222 adds a bounded same-channel elevated-noise indicator over plausible ESP32-S3 RX metadata, always labels it Low-confidence possible interference with unknown cause, never identifies a source or proves jamming, and turns invalid/missing/lost metadata into incomplete evidence; there is still no automatic response | analytics/product; `E-BUILD-159…171`/`E-AUTO-133…145` are partial control; WF-06-A1/A2 live golden + negative corpora and consolidated physical TFT evidence drilldown/cleanup still close the risk | reduced/open |
...
| R-024 | NFC/EMV, captive portal, or evidence verification retains a real submitted credential/payment secret or a matched Wi-Fi candidate | M | critical | NFR-011 data-minimization schemas, redacted views and forbidden-field/log/screenshot/export guards; Wi-Fi plaintext remains on the computer companion and only a non-secret weakness result crosses back; evidence inventory and persistence/export negative tests prove the boundary; a versioned curated common/default corpus is allowed, identity-linked leaked credentials are not | privacy/storage; PR-030/033/034 | open |
| R-025 | A bounded robustness/interference/network recipe affects an ambient or wrong target | M | critical | NFR-012 selected target, explicit channel/frequency and power profile including full qualified output, qualified isolated fixture/interlock, preview, deadline, physical Stop, RF/network containment oracle and fail-closed admission; requested power setting is never mislabeled as measured dBm | safety/Lab; PR-027/028/031/034 | open |
