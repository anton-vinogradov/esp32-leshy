...
Дополнение по доработке Wi-Fi: [карта экранов и интерактивные макеты](WIFI_UX_MAP.ru.md),
включая контракт пяти клавиш и [обогащение сведений о сети](WIFI_NETWORK_FACTS.ru.md).
Это проект для реализации/ревью, а не ретроспективное заявление, что принятая
прошивка уже рисует эти новые экраны.

Фиксированный знаменатель 1.0 — **62 возможности (`Y = 62`)**. Это не 62 пункта
меню. Количество строк меню и количество возможностей принципиально независимы:
одна понятная пользовательская задача собирает несколько CAP за одним контекстным
переходом.

- Home содержит только девять устойчивых входов по области интереса.
- Следующий уровень выбирает наблюдаемый объект: сеть, устройство, канал, сигнал,
  сохранённую запись или собственный стенд.
- Карточка объекта предлагает только уместные для него задачи. Например,
  `Сеть → Проверить пароль`, `Устройство → Найти рядом`,
  `Запись сигнала → Разобраться в сигнале`.
- Названия реализации (`PMKID`, `EAPOL`, `ESB`, `GATT`, `extcap`, chip names,
  file formats) никогда не требуются для выбора пути. Они допустимы только в
  необязательном листе **Технические детали** и в названиях экспортных форматов.
- Новая capability не получает строку Home, если её естественно открыть из уже
  выбранного объекта или результата.

Каждая исполняемая задача использует один guided-flow, а не собственный набор
непредсказуемых экранов:

```text
Задача → что произойдёт → что нужно подготовить → просмотр границ и цели
       → Старт → прогресс с постоянным Стоп → понятный итог
       → дальнейшие действия → Технические детали (необязательно)
```

Первый экран отвечает «что я получу», подготовка — «что потребуется и безопасно ли
продолжать», а результат — «что это значит и что делать дальше». Пользователь может
пройти весь путь, не зная названий протоколов, радио-чипов и инструментов компьютера.
...
Каждый живой список радиообъектов постоянно упорядочен по убыванию принятого
сигнала: текущий самый сильный RSSI находится сверху, более слабые записи идут ниже.
При равном RSSI сохраняется прежний взаимный порядок. До первого ручного действия
focus может следовать за первой/самой сильной строкой. Первый Navigate/Open отдаёт
focus пользователю: последующие refresh продолжают сортировать список, но
переякоривают cursor на ту же identity и никогда не утягивают его обратно на первую
строку. Новые объекты появляются без повторного входа. Поэтому порядок остаётся
честным и живым, а пересортировка не подменяет выбранный пользователем объект.
...
├─ Wi-Fi рядом
│  ├─ Сети → карточка сети
│  │  ├─ понять защиту и качество
│  │  ├─ найти источник по живому сигналу
│  │  └─ проверить пароль → объяснение → запись → сохранить/открыть → экспорт
│  │     └─ компьютер: выбрать локальный список → подтвердить право → проверить → результат
│  ├─ Устройства → карточка → кто это / что ищет / найти рядом
│  ├─ Каналы → загрузка / средний фон / рекомендация
│  ├─ Записать визит → сравнить с прошлым визитом
│  └─ Проверить подозрительную активность → вывод → исходные данные
├─ Bluetooth рядом
│  └─ Устройства → карточка → найти рядом
│     ├─ записать радиоданные для анализа
│     └─ узнать возможности устройства → явное разовое подключение
├─ Эфир 2,4 ГГц
│  ├─ увидеть активность: график ↔ водопад
│  └─ найти сигнал пульта, метки или датчика
├─ Эфир Sub-GHz
│  ├─ выбрать привычный диапазон → график ↔ водопад
│  └─ найти / записать сигнал → сохранить → при необходимости повторить в Lab
├─ Записать
│  ├─ эфир Wi-Fi → сохранить для анализа на компьютере
│  ├─ кнопку ИК-пульта → сохранить / разобрать / сравнить
│  ├─ метку NFC при наличии модуля → прочитать / сохранить / восстановить свою
│  └─ снимок экрана → открыть нужный экран → Снимок
├─ Мои цели
│  └─ цель → имя / заметки / связи / сравнить визиты / найти рядом / записать
├─ Сохранённое
│  └─ запись → понятная сводка → сравнить / экспортировать / открыть в Lab
│     └─ Технические детали: форматы, протокол, сырые поля
├─ Лаборатория
│  ├─ выбрать собственную сеть, устройство или изолированный стенд
│  ├─ выбрать понятную проверку устойчивости
│  └─ границы → мощность и время → подтверждение → Стоп → отчёт
│     ├─ Wi-Fi / Bluetooth / пульты и датчики / ИК / USB
│     └─ сценарии и автоматизация → сначала объяснение разрешений
└─ Устройство
   ├─ Настройки / Блокировка / Обновление
   ├─ Самопроверка: Быстрая / Полная пошаговая
   ├─ Диагностика: что работает / почему недоступно / что сделать
   ├─ Читать проводное устройство → подключение / просмотр / сохранить текст
   ├─ Доверенные сценарии → кто может добавлять и запускать
   └─ О системе

UX-S28 — общий слой: недоступность, подготовка, прогресс, подтверждение,
ошибка и аварийный Стоп. UX-S02…S62 — переиспользуемые экраны и контекстные
переходы внутри дерева, а не отдельные строки Home.
```
...
## UX-S32 «Смотреть запись Wi-Fi на компьютере»

Новый технический пункт Home не создаётся. Пользователь проходит существующее дерево
задач: `Главная → Захват → Wi-Fi → Старт`. Setup и Running показывают компактную
подсказку `USB: МОЖНО СМОТРЕТЬ НА ПК`; после этого Wireshark предлагает один source
`Леший: эфир Wi-Fi по USB`. Только устройство запускает и останавливает приём.
Wireshark получает read-only live radiotap PCAP, может независимо остановить свой
просмотр и никогда не меняет сеть Mac или tuning устройства. Выход из Capture
отзывает stream. Меняется только live data region; title, footer и explanatory note
используют общее smart-repaint rule и не должны мерцать.
...
2. **Выполняется** использует всю content area для времени, общего числа unique,
   ...
   strongest-first список AP/станций/BLE; live-сортировка продолжается, а cursor
   остаётся привязан к выбранной пользователем identity. Implementation counters, totals redraw
...
Host/build dev.352 добавляет безопасную основу annotations: до двенадцати
непересекающихся диапазонов Заголовок/Адрес/Команда/
...
Exact physical dev.353 принимает на реальном TFT task-first путь Actions → начало →
...
Этот абзац сохраняет только принятое поведение 0.114. Текущее нормативное правило
выше отменяет фиксацию порядка: strongest-first порядок остаётся живым, а после
действия пользователя focus переякоривается по identity.
...
Историческая фиксация identity строк здесь также заменена текущим контрактом live
order + identity-anchored focus; exact выбранные устройство и источник Radar
сохраняются.
