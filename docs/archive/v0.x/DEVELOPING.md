...
> **Archived 0.x development guide.** Commands remain useful while the 1.x bootstrap
> still builds through the legacy PlatformIO target. New development policy is in
> [the 1.x documentation](../../README.md).
...
Run the hardware-independent kernel tests before the slower firmware build:

```bash
tools/test.sh
```

They use the host C++ compiler and cover navigation, resource arbitration, and the
application lifecycle without requiring PlatformIO or a connected board.
